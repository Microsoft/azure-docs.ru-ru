...
10. Выберите вторую задачу **Azure IOT Edge** и настройте ее со следующими значениями:
    ...
    Если в задаче используется образ, который находится в частном доверенном реестре DOCKER, который не отображается в общедоступном облаке, можно задать для переменной среды **SKIP_MODULE_IMAGE_VALIDATION** значение, чтобы `true` пропустить проверку изображения. 
