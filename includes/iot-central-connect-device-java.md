...
* Компьютер для разработки с пакетом SDK для Java SE версии 8 или более поздней версии. Пакеты JDK Java 8 (LTS) для различных платформ можно скачать в разделе [Скачивание сборок Zulu для OpenJDK](https://www.azul.com/downloads/zulu-community/).
...
В копии пакета SDK Интернета вещей Microsoft Azure для Java, которую вы скачали ранее, откройте файл *azure-iot-sdk-java/device/iot-device-samples/pnp-device-sample/temperature-controller-device-sample/src/main/java/samples/com/microsoft/azure/sdk/iot/device/TemperatureController.java* в текстовом редакторе.
...
Класс `TargetTemperatureUpdateCallback` содержит метод `TwinPropertyCallBack` для передачи обновлений свойств, доступных для записи, в компонент из IoT Central. Этот метод использует класс `PnpConvention` для создания ответа:
...
Класс `MethodCallback` содержит метод `call` для обработки команд компонента, вызываемых из IoT Central:
...
Чтобы запустить пример приложения, откройте среду командной строки и перейдите в папку *azure-iot-sdk-java/device/iot-device-samples/pnp-device-sample/temperature-controller-device-sample*, содержащую папку *src* с примером файла *TemperatureController.java*.
