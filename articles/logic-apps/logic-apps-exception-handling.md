...
Хотя перехват ошибок в области очень эффективен, вам также может понадобиться контекст, чтобы понять, какие действия завершились сбоем, и узнать возвращенные ошибки и коды состояния. [ `result()` Функция](../logic-apps/workflow-definition-language-functions-reference.md#result) возвращает результаты действий верхнего уровня в действии с областью действия, принимая один параметр, который является именем области, и возвращает массив, содержащий результаты из этих действий первого уровня. Эти объекты действий включают те же атрибуты, что и возвращаемые `actions()` функцией, такие как время начала действия, время окончания, состояние, входы, идентификаторы корреляции и выходные данные. 

> [!NOTE]
> `result()`Функция возвращает результаты *только* из действий первого уровня, а не из более глубоких вложенных действий, таких как действия Switch или Condition.

Чтобы получить контекст о действиях, которые завершились сбоем в области, можно использовать `@result()` выражение с именем области и `runAfter` свойством. Чтобы отфильтровать возвращенный массив по действиям, имеющим `Failed` состояние, можно добавить [действие « **Фильтрация массива**](logic-apps-perform-data-operations.md#filter-array-action)». Чтобы выполнить действие для возвращенного действия, завершившееся сбоем, возьмите возвращенный отфильтрованный массив и используйте [цикл **for each**](../logic-apps/logic-apps-control-flow-loops.md).

Ниже приведен пример с подробным объяснением, который отправляет запрос HTTP POST с текстом ответа для всех действий, которые не удалось выполнить в действии Scope с именем "My_Scope":
