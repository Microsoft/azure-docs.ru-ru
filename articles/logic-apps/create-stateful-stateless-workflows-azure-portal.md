...
1. На странице **Создание приложения логики (Предварительная версия)** на вкладке **основы** укажите эти сведения о приложении логики.
   ...
   | **Опубликовать** | Да | <*Развертывание — среда*> | Назначение развертывания для приложения логики. Вы можете развернуть в Azure, выбрав **Рабочий процесс** или **контейнер DOCKER**. <p><p>В этом примере используется **Рабочий процесс**, который развертывает ресурс **приложения логики (Предварительная версия)** в портал Azure. <p><p>**Примечание**. прежде чем выбрать **контейнер DOCKER**, убедитесь, что вы создаете образ контейнера DOCKER. Например, вы можете создать этот образ с помощью [реестра контейнеров Azure](../container-registry/container-registry-intro.md), [службы приложений](../app-service/overview.md)или [экземпляра контейнера Azure](../container-instances/container-instances-overview.md). Таким образом, после выбора **контейнера DOCKER** можно [указать контейнер, который вы хотите использовать в параметрах приложения логики](#set-docker-container). |
...
1. В портал Azure в меню рабочего процесса выберите **мониторинг**.
   ...
   | **Клонения** | Запуск запущен и запущен, но получен запрос на отмену. |
