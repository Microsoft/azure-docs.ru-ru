...
1. Создайте новое назначение RFC со следующими параметрами:
    ...
    * На вкладке **технические параметры** в поле **тип активации** выберите пункт **зарегистрированный серверная программа**. В качестве **идентификатора программы** введите значение. В SAP триггер приложения логики будет зарегистрирован с использованием этого идентификатора.
    ...
    > [!IMPORTANT]
    > **Идентификатор программы** SAP учитывает регистр. При настройке приложения логики и сервера SAP следует всегда использовать тот же формат регистра для **идентификатора программы** . В противном случае при попытке отправить IDoc в SAP могут возникать следующие ошибки в мониторе Трфк (T-Code SM58):
    >
...
1. Прежде чем начать, вам потребуется приложение логики с триггером SAP. Если вы еще этого не сделали в приложении логики, выполните описанные выше действия в этом разделе, чтобы [настроить приложение логики с помощью триггера SAP](#receive-message-from-sap).
...
1. [Добавьте действие ответа в приложение логики](/azure/connectors/connectors-native-reqres#add-a-response-action) , чтобы ответить немедленно, с состоянием вашего запроса SAP. Рекомендуется добавить это действие сразу после триггера, чтобы освободить канал связи с сервером SAP. Выберите один из следующих кодов состояния ( `statusCode` ) для использования в действии ответа:
    ...
    * **204 нет содержимого**. Это означает, что сервер успешно выполнил запрос и в теле полезных данных ответа нет дополнительного содержимого для отправки. 

    * **200 ОК**. Этот код состояния всегда содержит полезную нагрузку, даже если сервер создает текст полезной нагрузки с нулевой длиной. 
...
* Как правило, триггер SAP не поддерживает кластеры шлюза данных. В некоторых случаях отработки отказа узел шлюза данных, взаимодействующий с системой SAP, может отличаться от активного узла, что приводит к непредвиденному поведению.

  * Для сценариев отправки поддерживаются кластеры шлюза данных в режиме отработки отказа. 

  * Кластеры шлюза данных в режиме балансировки нагрузки не поддерживаются действиями SAP с отслеживанием состояния. Эти действия включают **Создание сеанса с отслеживанием состояния**, **фиксацию транзакции BAPI**, **откат транзакции BAPI**, **закрытие сеанса с отслеживанием состояния** и все действия, в которых указывается значение **идентификатора сеанса** . Связь с отслеживанием состояния должна остаться на одном узле кластера шлюза данных. 

  * Для действий SAP с отслеживанием состояния используйте шлюз данных в режиме, отличном от кластера, или в кластере, настроенном для отработки отказа.
