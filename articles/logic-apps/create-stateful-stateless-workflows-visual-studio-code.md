...
1. Если вы используете macOS или Linux, настройте доступ к учетной записи хранения, выполнив следующие действия, которые необходимы для локального запуска проекта:
   ...
   1. Когда все будет готово, убедитесь, что изменения сохранены.
...
При отмене сеанса отладки для запуска рабочего процесса, использующего локальное выполнение триггеров или действий на основе веб-перехватчика, существующие регистрации подписок не удаляются. Для отмены регистрации необходимо вручную удалить или удалить регистрации подписки.
...
* [Azure Logic Apps выполнение любого монитора с Application Insights. часть 1](https://techcommunity.microsoft.com/t5/integrations-on-azure/azure-logic-apps-running-anywhere-monitor-with-application/ba-p/1877849)
...
* Файл DOCKER для рабочего процесса, который используется при создании контейнера DOCKER

  Например, этот пример файла DOCKER развертывает приложение логики. Указывает строку подключения, которая содержит ключ доступа для учетной записи хранения Azure, которая использовалась для публикации приложения логики в портал Azure. Чтобы найти эту строку, см. раздел [Получение строки подключения учетной записи хранения](#find-storage-account-connection-string).
...
Перед сборкой и запуском образа контейнера DOCKER необходимо получить строку подключения, содержащую ключ доступа к вашей учетной записи хранения. Ранее вы создали эту учетную запись хранения как для использования расширения в macOS или Linux или при развертывании приложения логики в портал Azure.

Чтобы найти и скопировать эту строку подключения, выполните следующие действия.
...
1. Сохраните строку подключения в безопасном месте, чтобы можно было добавить эту строку в файл DOCKER, используемый для развертывания. 

<a name="find-storage-account-master-key"></a>

### <a name="find-master-key-for-storage-account"></a>Найти главный ключ для учетной записи хранения

Если рабочий процесс содержит триггер запроса, необходимо [получить URL-адрес обратного вызова триггера](#get-callback-url-request-trigger) после сборки и запуска образа контейнера DOCKER. Для этой задачи также необходимо указать значение главного ключа для учетной записи хранения, используемой для развертывания.

1. Чтобы найти этот главный ключ, в проекте откройте **Azure-веб-задания — секреты/{Deployment-Name}/host.jsв** файле.

1. Найдите `AzureWebJobsStorage` свойство и скопируйте значение ключа из этого раздела:
...
1. Сохраните это значение ключа в безопасном месте для дальнейшего использования.
...
Для рабочего процесса, использующего триггер запроса, получите URL-адрес обратного вызова триггера, отправив этот запрос:
...
`{trigger-name}`Значение — это имя триггера запроса, которое отображается в определении JSON рабочего процесса. Это `{master-key}` значение определяется в учетной записи хранения Azure, заданной для `AzureWebJobsStorage` свойства в файле, **Azure-веб-заданиях (секреты)/{Deployment-name}/host.json**. Дополнительные сведения см. в разделе [Поиск главного ключа учетной записи хранения](#find-storage-account-master-key).
...
### <a name="designer-fails-to-open"></a>Не удается открыть конструктор

При попытке открыть конструктор появляется сообщение об ошибке **"не удалось запустить время разработки рабочего процесса"**. Если вы ранее пытались открыть конструктор, а затем не настроили или удалили проект, пакет расширений может быть загружен неправильно. Чтобы проверить причину проблемы, выполните следующие действия.
  ...
  1. Просмотрите выходные данные и проверьте, отображается ли это сообщение об ошибке:
   ...
   Чтобы устранить эту ошибку, удалите папку **екстенсионбундлес** в этом расположении **. папка \Users \{ имя пользователя} \аппдата\локал\темп\функтионс\екстенсионбундлес** и повторите попытку открытия файла **workflow.js** в конструкторе.
...
### <a name="debugging-session-fails-to-start"></a>Не удается запустить сеанс отладки

При попытке запустить сеанс отладки появляется сообщение об ошибке **"ошибка возникает после выполнения выполнении предварительной задачи ' женератедебугсимболс '"**. Чтобы устранить эту проблему, измените **tasks.jsдля** файла в проекте, чтобы пропустить создание символов.

1. В проекте разверните папку **vscode** и откройте **tasks.jsв** файле.

1. В следующей задаче удалите строку, `"dependsOn: "generateDebugSymbols"` вместе с запятой, завершающей предыдущую строку, например:
