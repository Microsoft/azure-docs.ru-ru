...
* **Сопутствующие рекомендации**
  ...
  * Регион, выбранный для установки шлюза, необходимо будет выбрать при последующем создании ресурса шлюза Azure для приложения логики. По умолчанию этот регион совпадает с расположением клиента Azure AD, который управляет учетной записью пользователя Azure. Однако расположение можно изменить во время установки шлюза или более поздней версии.

    > [!IMPORTANT]
    > Во время настройки шлюза команда **изменить регион** недоступна, если вы вошли в учетную запись Azure для государственных организаций, которая связана с клиентом Azure Active Directory (Azure AD) в [облаке Azure для государственных организаций](../azure-government/compare-azure-government-global-azure.md). Шлюз автоматически использует тот же регион, что и клиент Azure AD вашей учетной записи пользователя.
    > 
    > Чтобы продолжить использовать учетную запись Azure для государственных организаций, но настройте шлюз для работы в глобальном коммерческом облаке Azure с несколькими клиентами, а затем сначала войдите во время установки шлюза с `prod@microsoft.com` именем пользователя. Это решение заставляет шлюз использовать глобальное многоклиентское облако Azure, но по-прежнему позволяет продолжать использовать учетную запись Azure для государственных организаций.
