...
Спецификации шаблонов предоставляют следующие преимущества.

* Для спецификации шаблона используются стандартные шаблоны ARM.
* Вы управляете доступом с помощью Azure RBAC, а не токенов SAS.
* Пользователи могут развернуть спецификацию шаблона без доступа на запись к шаблону.
* Вы можете интегрировать спецификацию шаблона в существующий процесс развертывания, например сценарий PowerShell или конвейер DevOps.

Спецификации шаблонов позволяют создавать канонические шаблоны и совместно использовать их с командами в Организации. Спецификации шаблонов безопасны, так как они доступны для Azure Resource Manager развертывания, но недоступны пользователям без правильного разрешения. Пользователям требуется только доступ для чтения к спецификации шаблона, чтобы развернуть его шаблон, чтобы можно было предоставить общий доступ к шаблону, не позволяя другим пользователям изменять его.

Если у вас есть шаблоны в репозитории GitHub или учетной записи хранения, при попытке совместного использования шаблонов и их применения возникают некоторые трудности. Чтобы развернуть шаблон, необходимо либо сделать шаблон общедоступным, либо управлять доступом с помощью маркеров SAS. Чтобы обойти это ограничение, пользователи могут создавать локальные копии, которые в конечном итоге расходят от исходного шаблона. Спецификации шаблонов упрощают общий доступ к шаблонам.
