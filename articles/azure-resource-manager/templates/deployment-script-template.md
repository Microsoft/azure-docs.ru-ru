...
> [!NOTE]
> _Скриптконтент_ показывает сценарий с несколькими строками.  Портал Azure и конвейере Azure DevOps не могут анализировать скрипт развертывания с несколькими строками. Можно либо связать команды PowerShell (с помощью точек с запятой или _\\ r \\ n_ или _\\ n_) в одну строку, либо использовать `primaryScriptUri` свойство со внешним файлом скрипта. Доступно множество бесплатных инструментов escape-последовательности и переключения по строке JSON. Например, [https://www.freeformatter.com/json-escape.html](https://www.freeformatter.com/json-escape.html) .
