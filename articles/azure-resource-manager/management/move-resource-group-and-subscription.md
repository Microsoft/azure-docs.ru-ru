...
## <a name="changed-resource-id"></a>Измененный идентификатор ресурса

При перемещении ресурса изменяется его идентификатор ресурса. Стандартный формат для идентификатора ресурса — `/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/{resourceProviderNamespace}/{resourceType}/{resourceName}` . При перемещении ресурса в новую группу ресурсов или подписку вы изменяете одно или несколько значений в этом пути.

Если вы используете идентификатор ресурса в любом месте, это значение необходимо изменить. Например, если на портале есть [Настраиваемая панель мониторинга](../../azure-portal/quickstart-portal-dashboard-azure-cli.md) , которая ссылается на идентификатор ресурса, необходимо обновить это значение. Найдите скрипты или шаблоны, которые необходимо обновить для нового идентификатора ресурса.
...
1. При перемещении ресурса, для которого роль Azure назначена непосредственно ресурсу (или дочернему ресурсу), назначение роли не перемещается и становится потерянным. После перемещения необходимо повторно создать назначение ролей. В конечном итоге назначение потерянной роли автоматически удаляется, но рекомендуется удалить назначение роли перед перемещением.
...
Для выполнения запроса на перемещение может потребоваться не более четырех часов. Чтобы предотвратить изменение перемещаемых ресурсов, во время перемещения ресурса блокируются как исходная, так и Целевая группы ресурсов.
