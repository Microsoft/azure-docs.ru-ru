...
Найдите проект для этого примера на сайте [GitHub](https://github.com/Azure-Samples/communication-services-web-calling-hero). Версию примера с [взаимодействием между командами](../../concepts/teams-interop.md) можно найти в отдельной [ветви](https://github.com/Azure-Samples/communication-services-web-calling-hero/tree/teams-interop).
...
> [!NOTE]
> Из-за ограничений пакета SDK для вызовов через Интернет поддерживается только один удаленный видеопоток. Дополнительные сведения см. в разделе [Поддержка потоков в пакете SDK для вызовов](https://docs.microsoft.com/azure/communication-services/concepts/voice-video-calling/calling-sdk-features#calling-sdk-streaming-support).
