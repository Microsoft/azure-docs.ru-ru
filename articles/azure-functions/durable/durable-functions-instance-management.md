...
> [!NOTE]
> Orchestrator не помечается как `Completed` до завершения всех запланированных задач _и_ возврата Orchestrator. Иными словами, Orchestrator не имеет достаточных прав на доступ к своей `return` инструкции, чтобы она была помечена как `Completed` . Это особенно важно в тех случаях, когда `WhenAny` используется; эти оркестрации часто `return` выполняются до выполнения всех запланированных задач.
