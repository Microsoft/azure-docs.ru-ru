...
Если приложение версии 1.х было разработано локально, можно внести изменения в приложение, чтобы обеспечить совместимость с версией 2.x.  Но все же, рекомендуется создать новый проект "Функции Azure" с версией 2.x и перенести в него существующий код.  Хотя Вы можете вручную обновить существующий проект и код (обновление "на месте"), однако есть ряд других изменений между версиями 1 и 2, которые, возможно, придется внести. (Например, в C# объект для отладки `TraceWriter` был заменен на `ILogger`).  

Если же Вы создаете новый проект версии 2.x, функции будут создаваться по самым последним обновленным шаблонам функций версии 2.x.

#### <a name="visual-studio-runtime-versions"></a>Версии среды выполнения Visual Studio

В Visual Studio вы выбираете версию среды выполнения при создании проекта.  Средства работы с Функциями Azure для Visual Studio поддерживает обе основные версий. Динамически используется подходящая из них для проекта.  Эти параметры определяеются в файле `.csproj`.  Для приложений 1.x свойства проекта такие:
...
Во время отладки или публикации используется правильная версия среды выполнения.
