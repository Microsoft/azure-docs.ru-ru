...
>[!NOTE]
>Приложения функции .NET закреплены, чтобы `~2.0` отказаться от автоматического обновления до .NET Core 3,1. Чтобы узнать больше, ознакомьтесь с [рекомендациями по Функциям версии 2.x](functions-dotnet-class-library.md#functions-v2x-considerations).  
