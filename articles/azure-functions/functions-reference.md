...
| Имя расширения | версия расширения;                                                                                     | Поддерживается в плане потребления |
|----------------|-------------------------------------------------------------------------------------------------------|---------------------------------------|
...
| Центры событий Azure    | [Версия 5.0.0-beta1 или более поздняя](./functions-bindings-event-hubs.md#event-hubs-extension-5x-and-higher) | Нет                                    |
...
| Свойство.    | Требуется для расширений | Переменная среды | Описание |
|---|---|---|---|
| URI службы | Большой двоичный объект Azure, очередь Azure | `<CONNECTION_NAME_PREFIX>__serviceUri` |  Универсальный код ресурса (URI) плоскости данных службы, к которой выполняется подключение. |
| Полное пространство имен | Центры событий | `<CONNECTION_NAME_PREFIX>__fullyQualifiedNamespace` | Полное пространство имен концентратора событий. |
...
Чтобы подключиться с помощью субъекта-службы Azure Active Directory с ИДЕНТИФИКАТОРом клиента и секретом, определите подключение со следующими обязательными свойствами в дополнение к [свойствам соединения](#connection-properties) , приведенным выше.

| Свойство.    | Переменная среды | Описание |
|---|---|---|
| Tenant ID | `<CONNECTION_NAME_PREFIX>__tenantId` | Идентификатор Azure Active Directory клиента (каталога). |
...
Пример `local.settings.json` свойств, необходимых для подключения на основе удостоверений с помощью большого двоичного объекта Azure: 
```json
{
  "IsEncrypted": false,
  "Values": {
