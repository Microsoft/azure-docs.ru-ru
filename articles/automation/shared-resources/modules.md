...
* При явном импорте модуля модулем Runbook с [помощью оператора using Module](/powershell/module/microsoft.powershell.core/about/about_using#module-syntax) . Оператор using поддерживается начиная с Windows PowerShell 5,0, поддерживает классы и импорт типов Enum.
* если runbook импортирует другой зависимый модуль.

Вы можете импортировать модули az в учетную запись службы автоматизации из портал Azure. Не забудьте импортировать только необходимые модули AZ, а не каждый доступный модуль AZ. Поскольку от модуля [Az.Accounts](https://www.powershellgallery.com/packages/Az.Accounts/1.1.0) зависят другие модули Az, обязательно импортируйте его перед остальными.
