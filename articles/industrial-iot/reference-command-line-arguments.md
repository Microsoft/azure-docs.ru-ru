---
title: Аргументы командной строки издателя Microsoft OPC
description: В этой статье представлен обзор аргументов командной строки издателя OPC.
author: jehona-m
ms.author: jemorina
...
ms.locfileid: "104787593"
---
# <a name="command-line-arguments"></a>Аргументы командной строки

В следующем примере описано несколько аргументов командной строки, которые можно использовать для установки глобальных параметров для издателя OPC.

## <a name="opc-publisher-command-line-arguments-for-version-25-and-below"></a>Аргументы командной строки издателя OPC для версии 2,5 и ниже

* Использование: opcpublisher.exe \<applicationname> [ \<iothubconnectionstring> ] [ \<options> ]
    
* ApplicationName: имя приложения OPC UA, которое необходимо использовать. имя приложения также используется для регистрации издателя с этим именем в реестре устройств центра Интернета вещей.
    
* iothubconnectionstring. необязательный параметр ConnectionString для владельца центра Интернета вещей. Как правило, значение ConnectionString IoTHub Owner указывается только в первом запуске приложения. Строка подключения шифруется и хранится в хранилище сертификатов платформ.
При последующих вызовах он считывается и повторно используется. Если указать ConnectionString для каждого запуска, устройство, созданное для приложения в реестре устройств центра Интернета вещей, будет удалено и создано заново.
    
Существует несколько переменных среды, которые можно использовать для управления приложением:
```
...
## <a name="opc-publisher-command-line-arguments-for-version-26-and-above"></a>Аргументы командной строки издателя OPC для версии 2,6 и более поздней
```
      --pf, --publishfile=VALUE
                                       the filename to configure the nodes  to publish.
...
Дополнительные ресурсы можно найти в репозиториях GitHub:

> [!div class="nextstepaction"]
> [Репозиторий издателя OPC](https://github.com/Azure/Industrial-IoT)

> [!div class="nextstepaction"]
> [Репозиторий GitHub платформы промышленному](https://github.com/Azure/iot-edge-opc-publisher)