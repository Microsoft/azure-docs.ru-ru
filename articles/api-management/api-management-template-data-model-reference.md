...
|`State`|Число|Текущее состояние приложения.<br /><br /> — 0: зарегистрировано.<br /><br /> — 1: отправлено.<br /><br /> — 2: опубликовано.<br /><br /> — 3: отклонено.<br /><br /> — 4: не опубликовано.|  
|`RegistrationDate`|Дата/время|Дата и время регистрации приложения.|  
|`CategoryId`|Число|Категория приложения (финансы, развлечения и т. д.)|  
...
|`PostedOn`|Дата/время|Дата и время, когда был оставлен комментарий.|  
...
|`ReportedOn`|Дата/время|Дата и время, когда была обнаружена проблема.|  
...
|`PageSize`|Число|Максимальное число результатов, отображаемых на одной странице.|  
|`TotalItemCount`|Число|Число элементов для отображения.|  
|`ShowAll`|Логическое|Указывает, следует ли показывать все результаты на одной странице.|  
|`PageCount`|Число|Число страниц с результатами.|  
...
|`kind`|Число|Указывает, является ли этот параметр параметром пути (1) или параметром строки запроса (2).|  
...
|`ProductState`|Число|Указывает, опубликован ли продукт. Опубликованные продукты доступны разработчикам на портале разработчика. Неопубликованные продукты видны только администраторам.<br /><br /> Допустимые значения для состояния продукта:<br /><br /> - `0 - Not Published`<br /><br /> - `1 - Published`<br /><br /> - `2 - Deleted`|  
...
|`MultipleSubscriptionsCount`|Число|Максимальное число подписок для этого продукта, которые может иметь пользователь.|  
...
|`Password`|строка|Пароль учетной записи пользователя.|  
|`PasswordVerdictLevel`|Число|Значение, используемое элементом управления для [регистрации](api-management-page-controls.md#sign-up).|  
|`UserRegistrationTerms`|строка|Условия, которые пользователь должен принять перед входом.|  
|`UserRegistrationTermsOptions`|Число|Значение, используемое элементом управления для [регистрации](api-management-page-controls.md#sign-up).|  
