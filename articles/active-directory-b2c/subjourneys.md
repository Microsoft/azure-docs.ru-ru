...
| --------- | -------- | ----------- |
| Идентификатор | Да | Идентификатор вспомогательного пути, который может быть использован в пути взаимодействия пользователя для ссылки на подсвойства в политике. Элемент **субжаурнэйреференцеид** элемента [Candidate](userjourneys.md#journeylist) указывает на этот атрибут. |
| Type | Да | Возможные значения: `Call` или `Transfer` . Дополнительные сведения см. в разделе [ветвь пути взаимодействия пользователя](#user-journey-branching) .|
