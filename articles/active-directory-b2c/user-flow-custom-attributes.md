...
Каталог Azure AD B2C прилагается ко [встроенному набору атрибутов](user-profile-attributes.md). Однако часто приходится создавать собственные атрибуты для управления конкретным сценарием, например:
...
1. Вставьте ранее записанный **идентификатор приложения** между открывающим `<Item Key="ClientId">` и закрывающим `</Item>` элементами.
1. Вставьте ранее записанный идентификатор **ObjectID приложения** между открывающим `<Item Key="ApplicationObjectId">` и закрывающим `</Item>` элементами.
