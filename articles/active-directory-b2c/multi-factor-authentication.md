...
1. В разделе **многофакторная проверка подлинности** выберите нужный **метод MFA**, а затем в разделе **принудительное применение MFA** выберите **Always on** или **Conditional (рекомендуется)**.
   > [!NOTE]
   >
   > - Если выбран параметр **Условный (рекомендуется)**, необходимо также [Добавить условный доступ к потокам пользователей](conditional-access-user-flow.md)и указать приложения, к которым должна применяться политика.
