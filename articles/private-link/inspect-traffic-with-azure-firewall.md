...
* Группы безопасности сети (NSG) обрабатываются трафиком, поступающим от частных конечных точек.
* Определяемые пользователем маршруты (UDR) обрабатываются трафиком, поступающим от частных конечных точек.
...
> [!IMPORTANT] 
> Использование правил приложения в правилах сети рекомендуется при проверке трафика, предназначенного для частных конечных точек, в целях поддержания симметрии потока. Если используются сетевые правила или используется NVA, а не брандмауэр Azure, для трафика, предназначенного для частных конечных точек, необходимо настроить SNAT.
...
Если виртуальные сети являются равноправными, то при подключении из виртуальной сети клиента к брандмауэру Azure в виртуальной сети будет взиматься плата. Подключения из брандмауэра Azure в виртуальной сети концентратора к частным конечным точкам в одноранговой виртуальной сети не начисляются.
...
Используйте этот шаблон, если переход на центральную и лучевую архитектуру невозможен. Те же вопросы, что и в сценарии 2. В этом сценарии плата за пиринг виртуальных сетей не применяется.
...
2. Установите [SQL Server программ командной строки](/sql/linux/quickstart-install-connect-ubuntu#tools).
