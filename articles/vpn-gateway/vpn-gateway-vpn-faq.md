...
### <a name="can-i-specify-private-dns-servers-in-my-vnet-when-configuring-vpn-gateway"></a>Можно ли указать частные DNS-серверы в виртуальной сети при настройке VPN-шлюза?

Если вы указали DNS-сервер или серверы при создании виртуальной сети, VPN-шлюз будет использовать указанные DNS-серверы. Если указан DNS-сервер, убедитесь, что DNS-сервер может разрешить доменные имена, необходимые для Azure.
...
Шлюзы на основе маршрутов реализуют VPN на основе маршрутов. VPN на основе маршрутов используют «маршруты» в таблице маршрутизации и переадресации для направления пакетов в интерфейсы соответствующих туннелей. Затем интерфейсы туннелей шифруют пакеты в туннели или расшифровывают их из туннелей. Политики или селекторы трафика для виртуальных частных сетей на основе маршрутов настраиваются как любые (или) подстановочные знаки.
...
Нет. Тип шлюза нельзя изменить с помощью политики на маршрут или на основе маршрута на основе политики. Чтобы изменить тип шлюза, необходимо удалить шлюз и создать его заново. Этот процесс занимает около 60 минут. При создании нового шлюза нельзя хранить IP-адрес исходного шлюза.

1. Удалите все подключения, связанные с этим шлюзом.

1. Удалите шлюз, используя одну из следующих статей:
...
1. Создайте шлюз, используя нужный тип шлюза, а затем завершите установку VPN. Инструкции см. в [руководстве "сеть — сеть](./tutorial-site-to-site-portal.md#VNetGateway)".
...
VPN Azure использует проверку подлинности с помощью общего ключа (PSK). Во время создания VPN-туннеля генерируется общий ключ (PSK). Можно изменить автоматически сформированный PSK на собственный командлет Set pre-shared key PowerShell или REST API.
...
## <a name="bgp-and-routing"></a><a name="bgp"></a>BGP и маршрутизация
