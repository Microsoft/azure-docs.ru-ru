...
Аудит изменений конфигурации осуществляется в таблице **гатевайдиагностиклог** . Обратите внимание, что выполнение изменений в журналах может занять несколько минут.
...
:::image type="content" source="./media/troubleshoot-vpn-with-azure-diagnostics/image-26-set-gateway.png" alt-text="Укажите пример операции шлюза.":::
...
:::image type="content" source="./media/troubleshoot-vpn-with-azure-diagnostics/image-16-tunnel-connected.png" alt-text="Пример события с подключением к туннелю.":::
...
:::image type="content" source="./media/troubleshoot-vpn-with-azure-diagnostics/image-31-bgp-route.png" alt-text="Примеры маршрутов BGP.":::
...
|---        | ---               |
|**TimeGenerated** | Метка времени каждого события в часовом поясе UTC|
| **RemoteIP** | IP-адрес локального VPN-устройства. В реальных сценариях полезно выполнять фильтрацию по IP-адресу соответствующего локального устройства. |
|**LocalIP** | IP-адрес VPN-шлюза, для которого выполняется устранение неполадок. В реальных сценариях полезно выполнять фильтрацию по IP-адресу соответствующего VPN-шлюза, если в подписке есть больше одного. |
|**Событие** | содержит диагностическое сообщение, полезное для устранения неполадок. Обычно они начинаются с ключевого слова и обращаются к действиям, выполняемым **\[ отправкой \]** шлюза Azure. указывает на событие, вызванное пакетом IPSec, отправленным **\[ \] полученным** шлюзом Azure указывает, что в результате пакет, полученный от **\[ локального \]** устройства, указывает на действие, выполняемое локально шлюзом Azure. |
...
- Если не удается установить туннель IPsec, Azure повторит попытку через несколько секунд. По этой причине неполадки "VPN Down" очень удобны в Икедиагностиклог, так как нет необходимости ждать определенного времени для воспроизведения проблемы. Кроме того, эта ошибка будет всегда одинаковой при каждом походе, поэтому можно просто увеличить одно «примерное» согласование в любое время.
...
Параметры [IPSec/IKE по умолчанию](https://docs.microsoft.com/azure/vpn-gateway/vpn-gateway-about-vpn-devices#ipsec) содержит список параметров IPsec, поддерживаемых шлюзом Azure с параметрами по умолчанию


## <a name="p2sdiagnosticlog"></a><a name="P2SDiagnosticLog"></a>P2SDiagnosticLog

Последняя доступная таблица для диагностики VPN — **P2SDiagnosticLog**. Будет выполнен трассировка действия для точки на сайт.
...
:::image type="content" source="./media/troubleshoot-vpn-with-azure-diagnostics/image-28-p2s-log-event.png" alt-text="Пример журналов P2S.":::
...
:::image type="content" source="./media/troubleshoot-vpn-with-azure-diagnostics/image-29-eap.png" alt-text="Пример журналов EAP.":::
