...
Службы мультимедиа Azure можно использовать для доставки потоков, которые были зашифрованы с помощью лицензий Microsoft PlayReady, Google Widevine или Apple FairPlay. Подробное описание см. в статье [Защита содержимого с помощью динамического шифрования Служб мультимедиа](drm-content-protection-concept.md).
...
![AMS с видео, защищенным DRM, в Проигрывателе мультимедиа Azure](./media/drm-protect-with-drm-tutorial/ams_player.png)
...
* Просмотрите статью [Обзор системы защиты содержимого](drm-content-protection-concept.md).
* Ознакомьтесь со статьей [Проектирование системы для защиты содержимого с несколькими подсистемами DRM и управлением доступом](architecture-design-multi-drm-system.md).
* Установите Visual Studio Code или Visual Studio.
* Создайте учетную запись Служб мультимедиа Azure, как описано в [этом руководстве](./account-create-how-to.md).
...
Как было указано выше, объект **преобразования** является набором инструкций, а [задание](transform-jobs-concept.md) — фактическим запросом к Службам мультимедиа для применения этого **преобразования** к данному видео и аудио. **Задание** указывает такую информацию, как расположение входного и выходного видео.
...
Ключ содержимого обеспечивает безопасность доступа к ресурсам. При шифровании содержимого с помощью DRM необходимо создать [политику ключа содержимого](drm-content-key-policy-concept.md). Политика настраивает способ доставки ключа содержимого конечным клиентам. Ключ содержимого связан с указателем потоковой передачи. Службы мультимедиа также включают в себя службу доставки ключей, которая доставляет ключи шифрования и лицензии авторизованным пользователям.
...
* Конфигурация
    ...
    Лицензии [PlayReady](drm-playready-license-template-concept.md) и [Widevine](drm-widevine-license-template-concept.md) являются настраиваемыми, что позволяет доставлять их с помощью службы доставки лицензий для служб мультимедиа. Даже если в этом примере лицензия [FairPlay](drm-fairplay-license-overview.md) не настраивается, он содержит метод, который можно использовать для ее настройки. Конфигурацию FairPlay можно добавить в качестве другого параметра.
...
1. Создайте [указатель потоковой передачи](stream-streaming-locators-concept.md).
...
> [!IMPORTANT]
> При использовании пользовательской [политики потоковой передачи](stream-streaming-policy-concept.md) следует разработать ограниченный набор таких политик для учетной записи Служб мультимедиа и повторно использовать их для компонентов StreamingLocator каждый раз, когда требуются те же параметры шифрования и протоколы. У вашей учетной записи Служб мультимедиа есть квота на количество записей StreamingPolicy. Вы не должны создавать StreamingPolicy для каждого StreamingLocator.
...
![Защита с помощью DRM](./media/drm-protect-with-drm-tutorial/playready_encrypted_url.png)
...
> [Использование динамического шифрования AES-128 и службы доставки ключей](drm-playready-license-template-concept.md)
