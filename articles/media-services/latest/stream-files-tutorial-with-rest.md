...
Чтобы начать потоковую передачу видео, сначала вам потребуется запустить [конечную точку потоковой передачи](./stream-streaming-endpoint-concept.md), из которой предполагается транслировать видео.
