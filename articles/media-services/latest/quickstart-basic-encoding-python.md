---
title: Краткое руководство по базовому кодированию Python для Служб мультимедиа
description: В этом кратком руководстве показано, как выполнить базовое кодирование с помощью Python и Служб мультимедиа Azure.
...
ms.locfileid: "102500841"
---
# <a name="media-services-basic-encoding-with-python"></a>Базовое кодирование Служб мультимедиа с помощью Python

[!INCLUDE [media services api v3 logo](./includes/v3-hr.md)]

## <a name="introduction"></a>Введение

В этом кратком руководстве показано, как выполнить базовое кодирование с помощью Python и Служб мультимедиа Azure. В нем используется API Службы мультимедиа версии 3 от 1 мая 2020 года.

## <a name="prerequisites"></a>Предварительные требования

- Если у вас еще нет подписки Azure, [создайте бесплатную учетную запись](https://azure.microsoft.com/free/?WT.mc_id=A261C142F), прежде чем начинать работу.
- Создайте группу ресурсов для выполнения инструкций из этого краткого руководства.
- Создайте учетную запись Служб мультимедиа версии 3.
- Получите ключ учетной записи хранения.
- Создайте субъекта-службу и секрет.

## <a name="get-the-sample"></a>Получение примера

Создайте вилку и скопируйте пример, расположенный в [репозитории примеров для Python](https://github.com/Azure-Samples/media-services-v3-python). В этом кратком руководстве мы будет использовать пример BasicEncoding.

## <a name="create-the-env-file"></a>Создание ENV-файла

Получите значения из своей учетной записи, чтобы создать *ENV*-файл. Сохраните файл без имени (только с расширением).  Используйте файл *sample.env* в качестве шаблона, а затем сохраните *ENV*-файл в папку BasicEncoder в локальной копии.

## <a name="try-the-code"></a>Использование кода

В приведенный ниже код добавлены подробные комментарии.  В собственном сценарии вы можете использовать все примеры из этого сценария или некоторые его части.

В этом примере создается случайное число для именования вещей. Вы можете определить их как группу, которая была создана при запуске сценария.  Случайное число необязательное. Вы можете удалить его по завершении тестирования сценария.

В этом примере мы не используем URL-адрес SAS для входного ресурса.
...
Завершив работу с кратким руководством, удалите ресурсы, созданные в группе ресурсов.

## <a name="next-steps"></a>Дальнейшие действия

Общие сведения о [пакете SDK для Python Служб мультимедиа](https://docs.microsoft.com/python/api/azure-mgmt-media/)