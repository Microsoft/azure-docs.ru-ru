...
* [Подключение центра Интернета вещей к Azure Stack границе](../../databox-online/azure-stack-edge-gpu-deploy-configure-compute.md#configure-compute)
...
* **Метрики Pod и node**
    ...
    Используйте [kubectl Top](https://kubernetes.io/docs/reference/generated/kubectl/kubectl-commands#top)  , чтобы просмотреть метрики Pod и Node.
    <br/>`kubectl top pods -n iotedge` 

* **Сетевые подключения модулей**   
Для обнаружения модулей на Azure Stack ребро требуется, чтобы модуль имел привязку порта узла в Креатеоптионс. Затем модуль будет адреснее `moduleName:hostport` .
...
* **Общая память при использовании gRPC**
...
Kubernetes поддерживает [сходство Pod](https://kubernetes.io/docs/concepts/scheduling-eviction/assign-pod-node/#inter-pod-affinity-and-anti-affinity)  , что может запланировать модули питания на одном узле. Раздел Подаффинити, ссылающийся на модуль Live Video Analytics, можно добавить в манифест Pod для определения для достижения совместного размещения.
    ...
    ```
* **код ошибки 404 при использовании `rtspsim` модуля**  
Контейнер будет считывать видео только из одной папки в контейнере. Если вы сопоставляете или привязываете внешнюю папку к той, которая уже существует в образе контейнера, Docker будет скрывать файлы, присутствующие в образе контейнера.  
 
    Например, без привязок контейнер может содержать следующие файлы:  
    ```
    ...
    И ваш узел может иметь следующие файлы:
    ```    
    C:\MyTestVideos> dir
    Test1.mkv
    Test2.mkv
    ...
    Но при добавлении следующей привязки в файл манифеста развертывания DOCKER перезапишет содержимое/Ливе/медиасервер/медиа в соответствии с тем, что находится на узле.
