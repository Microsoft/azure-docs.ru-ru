...
* В Azure AD DS используется отдельный встроенный объект групповой политики для контейнеров "Пользователи" и "Компьютеры". Для дополнительного контроля можно [создать пользовательские объекты групповой политики][create-gpo] и выбрать их для пользовательских подразделений.
...
В этом сценарии Azure AD DS позволяет приложениям выполнять привязку LDAP в процессе проверки подлинности. Устаревшие локальные приложения могут быть перенесены в Azure и продолжать проверять подлинность пользователей без каких-либо изменений в конфигурации или в пользовательском интерфейсе.
...
* Убедитесь, что приложению не нужно изменять или записывать в каталог. Доступ на запись LDAP к управляемому домену не поддерживается.
* Убедитесь, что приложению не требуется пользовательская или расширенная схема Active Directory. Расширения схемы не поддерживаются в Azure AD DS.
...
Некоторые приложения включают в себя несколько уровней, где один из уровней должен выполнять вызовы, прошедшие проверку, на уровень сервера, например в базу данных. В этих сценариях обычно используются учетные записи служб Active Directory. При перетаскивании приложений посредством метода lift-and-shift в Azure служба Azure AD DS позволяет продолжать использовать учетные записи служб тем же самым способом. Вы можете использовать ту же учетную запись службы, которая синхронизируется из локального каталога с Azure AD, или создать настраиваемое подразделение, а затем создать отдельную учетную запись службы в этом подразделении. В обоих этих случаях приложения продолжают работать в одном и том же режиме и отправлять вызовы, прошедшие проверку, на другие уровни и в службы.
