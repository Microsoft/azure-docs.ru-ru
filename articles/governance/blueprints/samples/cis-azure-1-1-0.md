---
title: Пример схемы CIS Microsoft Azure Foundations Benchmark версии 1.1.0
description: Общие сведения о примере схемы CIS Microsoft Azure Foundations Benchmark 1.1.0. Этот пример схемы помогает клиентам оценить определенные средства управления.
ms.date: 03/11/2021
ms.topic: sample
...
ms.locfileid: "103197803"
---
# <a name="cis-microsoft-azure-foundations-benchmark-v110-blueprint-sample"></a>Пример схемы CIS Microsoft Azure Foundations Benchmark версии 1.1.0

В примере схемы CIS Microsoft Azure Foundations Benchmark версии 1.1.0, используемом со службой [Политика Azure](../../policy/overview.md), предоставляются средства для обеспечения соответствия требованиям, чтобы вы могли оценить рекомендации этого теста. Схема помогает клиентам развернуть основной набор политик для любой развернутой в Azure архитектуры, в которой необходимо реализовать рекомендации согласно CIS Microsoft Azure Foundations Benchmark версии 1.1.0.

## <a name="recommendation-mapping"></a>Сопоставление рекомендаций

[Сопоставление рекомендаций службы "Политика Azure"](../../policy/samples/cis-azure-1-1-0.md) позволяет получить подробные сведения об определениях политик, включенных в эту схему, и о сопоставлении этих определений с **рекомендациями** в CIS Microsoft Azure Foundations Benchmark версии 1.1.0. Назначаемые архитектуре ресурсы оцениваются службой "Политика Azure" на предмет соответствия назначенным определениям политик. Дополнительные сведения см. в статье [Что такое служба "Политика Azure"?](../../policy/overview.md)

## <a name="deploy"></a>Развертывание

Чтобы развернуть пример схемы CIS Microsoft Azure Foundations Benchmark версии 1.1.0 в Azure Blueprints, необходимо выполнить следующие действия:
...
|-|-|-|-|
|Аудит рекомендаций CIS Microsoft Azure Foundations Benchmark версии 1.1.0 и развертывание определенных расширений виртуальных машин для его поддержки|Назначение политики|Список регионов, где должен быть включен Наблюдатель за сетями|Разделенный точками с запятой список регионов. Список всех регионов можно просмотреть с помощью Get-AzLocation. Пример: eastus; eastus2|
|Аудит рекомендаций CIS Microsoft Azure Foundations Benchmark версии 1.1.0 и развертывание определенных расширений виртуальных машин для его поддержки|Назначение политики|Список утвержденных для использования расширений виртуальной машины|Разделенный точками с запятой список расширений. Список всех расширений виртуальной машины можно просмотреть с помощью Get-AzVMExtensionImage. Например, AzureDiskEncryption; IaaSAntimalware|
