...
Устройства IoT самонастраивающийся отправляют по крайней мере одно сообщение телеметрии для каждого интерфейса, включая корень, что может увеличить количество сообщений, подсчитанных в отношении квоты сообщений.
...
При инициации операций с удостоверениями с помощью [операций обновления реестра](https://docs.microsoft.com/rest/api/iothub/service/bulkregistry/updateregistry) (а *не* заданий импорта и экспорта) применяются те же ограничения регулирования. Например, если вы хотите отправить групповые операции для создания устройств 50 и у вас есть центр Интернета вещей S1 с 1 единицей, то в минуту будут приняты только два из этих запросов. Это связано с тем, что регулирование операций идентификации для центра Интернета вещей S1 с 1 единицей равно 100/min/Unit. Кроме того, в этом случае третий запрос (и больше) в той же минуте будет отклонен, так как уже достигнут предел. 
