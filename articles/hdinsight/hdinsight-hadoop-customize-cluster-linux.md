...
- URI Bash-скрипта (расположение для доступа к файлу) должен быть доступен из поставщика ресурсов HDInsight и кластера.
- Возможные места хранения:
   ...
   - Для обычных кластеров (не ESP):
     ...
     - Data Lake Storage 1-го поколения: служба HDInsight субъекта-службы использует для доступа к Data Lake Storage должен иметь доступ для чтения к скрипту. Формат URI Bash-скрипта — `adl://DATALAKESTOREACCOUNTNAME.azuredatalakestore.net/path_to_file` . 

     - Data Lake Storage 2-го поколения не рекомендуется использовать для действий скриптов. `abfs://` не поддерживается для URI Bash-скрипта. `https://` Идентификаторы URI возможны, но они работают для контейнеров, имеющих общий доступ, и брандмауэра, открытого для поставщика ресурсов HDInsight, и поэтому не рекомендуется.

     - Общедоступная служба общего доступа к файлам, доступная через `https://` пути. Примеры: BLOB-объект Azure, GitHub или OneDrive. Примеры URI см. в разделе [Пример сценариев действий сценария](#example-script-action-scripts).

  - Для кластеров с ESP `wasb://` `wasbs://` `http[s]://` поддерживаются URI или или.

- Действия скрипта могут быть ограничены для запуска только на определенных типах узлов. например головных или рабочих.
- Действия скрипта могут быть постоянными или *нерегламентированными*.
...
- Действия сценария могут принимать параметры, используемые скриптом во время выполнения.
- Действия сценария выполняются с правами корневого уровня на узлах кластера.
- Действия скрипта можно использовать с помощью портал Azure, Azure PowerShell, Azure CLI или пакета SDK для HDInsight .NET.
...
:::image type="content" source="./media/hdinsight-hadoop-customize-cluster-linux/cluster-provisioning-states.png" alt-text="Этапы создания кластера" border="false":::
...
1. Начните создавать кластер, как описано в статье [Создание кластеров под управлением Linux в HDInsight с помощью портал Azure](hdinsight-hadoop-create-linux-clusters-portal.md). На вкладке **Настройка и цены** выберите **+ Добавить действие скрипта**.
   ...
   :::image type="content" source="./media/hdinsight-hadoop-customize-cluster-linux/azure-portal-cluster-configuration-scriptaction.png" alt-text="Действие сценария кластера портал Azure":::
...
1. Используйте запись __Выберите сценарий__, чтобы выбрать готовый сценарий. Чтобы использовать настраиваемый сценарий, выберите __Настраиваемый__. Затем укажите __имя__ и __универсальный код ресурса (URI) Bash-сценария__ своего сценария.
   ...
   :::image type="content" source="./media/hdinsight-hadoop-customize-cluster-linux/hdinsight-select-script.png" alt-text="Добавление скрипта в форме выбора скрипта":::
...
1. Чтобы сохранить скрипт, нажмите кнопку __Создать__. Чтобы добавить еще один сценарий, вы можете использовать элемент __+ Submit new__ (+ Отправить новый).
   ...
   :::image type="content" source="./media/hdinsight-hadoop-customize-cluster-linux/multiple-scripts-actions.png" alt-text="Действия нескольких скриптов HDInsight":::
...
1. В верхней части страницы **Действия скрипта** выберите **+ Отправить новое**.
   ...
   :::image type="content" source="./media/hdinsight-hadoop-customize-cluster-linux/add-script-running-cluster.png" alt-text="Добавление скрипта в работающий кластер":::
...
1. Журнал сценариев для этого кластера отображается в разделе "Действия сценария". Эти сведения включают в себя список сохраняемых скриптов. На следующем снимке экрана показано, что в этом кластере был выполнен сценарий Solr. Сохраняемых сценариев на нем не видно.
   ...
   :::image type="content" source="./media/hdinsight-hadoop-customize-cluster-linux/script-action-history.png" alt-text="Журнал отправки действий сценария портала":::
...
1. При выборе сценария в журнале отображается соответствующий раздел **Панель свойств**. В верхней части экрана можно повторно запустить скрипт или изменить его тип.
   ...
   :::image type="content" source="./media/hdinsight-hadoop-customize-cluster-linux/promote-script-actions.png" alt-text="Продвижение свойств действий скрипта":::

1. Можно также нажать кнопку с многоточием ( **...**) справа от элемента записи в разделе действия скрипта, чтобы выполнить действия.

   :::image type="content" source="./media/hdinsight-hadoop-customize-cluster-linux/hdi-delete-promoted-sa.png" alt-text="Удаление сохраненных действий сценария":::
