...
1. На вкладке **Configs** (Конфигурации) Hive в поле фильтра введите **execution engine**.
    ...
    :::image type="content" source="./media/optimize-hive-ambari/ambari-search-execution.png" alt-text="Механизм выполнения поиска Apache Ambari" border="true":::

1. Свойство **Optimization** (Оптимизация) по умолчанию имеет значение **Tez**.

    :::image type="content" source="./media/optimize-hive-ambari/optimization-apache-tez.png" alt-text="Оптимизация — Apache Tez Engine" border="true":::
...
1. Задайте для обоих параметров значение **33 554 432** байтов (32 МБ).
    ...
    :::image type="content" source="./media/optimize-hive-ambari/apache-tez-grouping-size.png" alt-text="Размеры группирования Tez Apache Ambari" border="true":::
...
1. Чтобы изменить этот параметр, перейдите на вкладку **Configs** (Конфигурации) Hive и найдите параметр **Data per Reducer** (Данные на модуль сжатия) на странице "Settings" (Параметры).
    ...
    :::image type="content" source="./media/optimize-hive-ambari/ambari-data-per-reducer.png" alt-text="Данные Apache Ambari для каждого из них" border="true":::
...
1. Выберите **Edit** (Изменить), чтобы изменить значение параметра на 128 МБ (134 217 728 байтов), и нажмите клавишу **ВВОД**, чтобы сохранить изменение.
    ...
    :::image type="content" source="./media/optimize-hive-ambari/data-per-reducer-edited.png" alt-text="Ambari данные для каждого уменьшения" border="true":::
...
1. Чтобы ограничить число заданий, выполняемых параллельно, измените `hive.exec.parallel.thread.number` свойство. Значение по умолчанию: 8.
    ...
    :::image type="content" source="./media/optimize-hive-ambari/apache-hive-exec-parallel.png" alt-text="Параллельное отображение Apache Hive Exec" border="true":::
...
1. Чтобы включить векторизированное выполнение сжимаемой части запроса, задайте для параметра `hive.vectorized.execution.reduce.enabled` значение true. Значением по умолчанию является false.
    ...
    :::image type="content" source="./media/optimize-hive-ambari/hive-vectorized-execution.png" alt-text="Apache Hive векторное выполнение" border="true":::
...
:::image type="content" source="./media/optimize-hive-ambari/hdinsight-cbo-config.png" alt-text="Оптимизатор на основе затрат HDInsight" border="true":::
...
* `hive.compute.query.using.stats`
    ...
    :::image type="content" source="./media/optimize-hive-ambari/hive-compute-query-using-stats.png" alt-text="Apache Hive вычисление запроса с помощью статистики" border="true":::
...
* `hive.stats.fetch.column.stats`
    ...
    :::image type="content" source="./media/optimize-hive-ambari/hive-stats-fetch-column-stats.png" alt-text="Статистика по столбцам набора Apache Hive статистики" border="true":::
...
* `hive.stats.fetch.partition.stats`
    ...
    :::image type="content" source="./media/optimize-hive-ambari/hive-stats-fetch-partition-stats.png" alt-text="Статистика по секциям Hive" border="true":::
...
1. Чтобы включить промежуточное сжатие, перейдите на вкладку **Configs** (Конфигурации) Hive, а затем установите для параметра `hive.exec.compress.intermediate` значение true. Значением по умолчанию является false.
    ...
    :::image type="content" source="./media/optimize-hive-ambari/hive-exec-compress-intermediate.png" alt-text="&quot;Средний уровень сжатия файла Hive&quot;" border="true":::
...
1. Вот как можно добавить пользовательский параметр.
    ...
    :::image type="content" source="./media/optimize-hive-ambari/hive-custom-property.png" alt-text="' Apache Hive пользовательское свойство Add '" border="true":::
...
1. Чтобы выбрать кодек для сжатия выходных данных, добавьте пользовательское свойство `mapred.output.compression.codec` в область "Custom hive-site" (Настраиваемый сайт Hive), как описано на шаге 3 в предыдущем разделе.
    ...
    :::image type="content" source="./media/optimize-hive-ambari/hive-custom-property2.png" alt-text="Apache Hive пользовательское свойство ADD2" border="true":::
...
* Чтобы включить спекулятивное выполнение, перейдите на вкладку **Configs** (Конфигурации) Hive, а затем установите для параметра `hive.mapred.reduce.tasks.speculative.execution` значение true. Значением по умолчанию является false.
    ...
    :::image type="content" source="./media/optimize-hive-ambari/hive-mapred-reduce-tasks-speculative-execution.png" alt-text="' Hive mapred уменьшить задачи, выполняющие упреждающее выполнение &quot;" border="true":::
...
:::image type="content" source="./media/optimize-hive-ambari/hive-exec-mode-local-auto.png" alt-text="Локальный автоматический режим Apache Hive Exec" border="true":::
...
:::image type="content" source="./media/optimize-hive-ambari/hive-multigroupby-singlereducer.png" alt-text="Настройка отдельного запроса MapReduce MultiGROUP BY в Hive" border="true":::
