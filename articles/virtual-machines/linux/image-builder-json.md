...
- Расположения скриптов должны быть общедоступными, если не используется [MSI](./image-builder-user-assigned-identity.md).
...
#### <a name="super-user-privileges"></a>Привилегии суперпользователя
Чтобы команды выполнялись с привилегиями суперпользователя, их необходимо `sudo` Добавить в скрипты или использовать встроенные команды, например:
```json
                "type": "Shell",
                "name": "setupBuildPath",
                "inline": [
...
```
Пример скрипта с использованием sudo, на который можно сослаться с помощью Скриптури:
```bash
#!/bin/bash -e
...
> [!NOTE]
> Если sourceUri является учетной записью хранения Azure, независимо от того, помечен ли этот большой двоичный объект общедоступным, вы будете предоставлять управляемому удостоверению пользователя разрешения на чтение BLOB-объекта. Ознакомьтесь с этим [примером](./image-builder-user-assigned-identity.md#create-a-resource-group) , чтобы задать разрешения для хранилища.
...
В библиотеке [GitHub в разделе Конструктора образов Azure](https://github.com/azure/azvmimagebuilder) имеются примеры JSON-файлов для разных сценариев.