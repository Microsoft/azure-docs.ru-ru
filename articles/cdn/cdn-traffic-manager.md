...
1. Перейдите на веб-сайт поставщика домена для своего пользовательского домена, например GoDaddy, и создайте две записи DNS CNAME. 
    ...
    > [!NOTE]
    > Для реализация после отработки отказа сценерио обе конечные точки должны находиться в разных профилях, а разные профили могут быть разными поставщиками CDN, чтобы избежать конфликтов доменных имен.
    > 
