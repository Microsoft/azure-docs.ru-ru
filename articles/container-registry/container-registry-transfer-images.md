...
* **Проблемы с извлечением образа в физически изолированной среде**
  * Если вы видите сообщения об ошибках внешних слоев или пытаетесь разрешить mcr.microsoft.com при попытке извлечь образ в физической изолированной среде, скорее всего, манифест образа имеет нераспространяемые слои. Из-за природы физической изолированной среды эти образы часто не запрашиваются. Чтобы убедиться, что это так, проверьте манифест образа на наличие ссылок на внешние реестры. В этом случае вам потребуется отправить нераспространяемые слои в общедоступный облачный элемент контроля доступа, прежде чем развертывать конвейер экспорта — запустить для этого образа. Инструкции по выполнению этой задачи см. в разделе [разделы справки принудительная отправка нераспространяемых слоев в реестр?](./container-registry-faq.md#how-do-i-push-non-distributable-layers-to-a-registry)
