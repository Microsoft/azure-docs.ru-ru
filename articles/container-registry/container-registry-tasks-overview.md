...
Используя привычный `docker build` Формат, команда [AZ запись контроля][az-acr-build] доступа в Azure CLI принимает [контекст](#context-locations) (набор файлов для сборки), отправляет его в список задач записи контроля доступа и по умолчанию передает созданный образ в реестр после завершения.
