...
> [!IMPORTANT]
> После завершения операции смены ключа старая версия ключа больше не используется для шифрования новых данных. Возможны случаи, когда для доступа к старым данным может потребоваться старый ключ. Чтобы разрешить чтение таких старых данных, не удаляйте старый ключ.