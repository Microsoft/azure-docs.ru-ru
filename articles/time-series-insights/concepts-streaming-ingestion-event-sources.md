...
Источник события — это связь между центром и средой Gen2 "аналитика временных рядов Azure", а `Time Series Insights event source` в группе ресурсов создается отдельный ресурс типа. Центр Интернета вещей или ресурсы концентратора событий могут находиться в той же подписке Azure, что и среда Gen2 для аналитики временных рядов Azure, или другую подписку. Однако рекомендуется размещать среду службы "аналитика временных рядов Azure" и центр Интернета вещей или концентратор событий в одном регионе Azure.

Для создания, изменения и удаления источников событий среды можно использовать шаблоны [портал Azure](./tutorials-set-up-tsi-environment.md#create-an-azure-time-series-insights-gen2-environment), [Azure CLI](https://docs.microsoft.com/cli/azure/ext/timeseriesinsights/tsi/event-source), [Azure Resource Manager](time-series-insights-manage-resources-using-azure-resource-manager-template.md)и [REST API](/rest/api/time-series-insights/management(gen1/gen2)/eventsources) .

## <a name="start-options"></a>Параметры запуска

При создании источника событий можно указать, какие данные должны быть собраны ранее. Это необязательный параметр. Доступны следующие варианты:

| Имя   |  Описание  |  Пример шаблона Azure Resource Manager |
|----------|-------------|------|
| еарлиеставаилабле | Прием всех существующих данных, хранящихся в IoT или концентраторе событий | `"ingressStartAt": {"type": "EarliestAvailable"}` |
| евентсаурцекреатионтиме |  Начало приема данных, поступающих после создания источника события. Все существующие данные, которые были переработаны перед созданием источника события, будут игнорироваться. Это значение по умолчанию в портал Azure   |   `"ingressStartAt": {"type": "EventSourceCreationTime"}` |
| кустоменкуеуедтиме | Ваша среда будет принимать данные из настраиваемого времени в очереди (UTC) вперед. Все события, которые были поставлены в очередь Интернета вещей или концентратора событий в или после истечения настраиваемого времени в очереди, будут приняты и сохранены. Все события, которые были получены до наступления настраиваемого времени в очереди, будут проигнорированы. Обратите внимание, что "время в очереди" относится к времени (в формате UTC), которое событие поступило в центр Интернета вещей или в концентратор событий. Это отличается от пользовательского [Свойства timestamp](./concepts-streaming-ingestion-event-sources.md#event-source-timestamp) , которое находится в теле события. |     `"ingressStartAt": {"type": "CustomEnqueuedTime", "time": "2021-03-01T17:00:00.20Z"}` |

> [!IMPORTANT]
>
> - При выборе Еарлиеставаилабле и наличии большого количества предварительно существующих данных может возникнуть высокая первоначальная задержка, так как среда Gen2 "аналитика временных рядов Azure" обрабатывает все данные.
> - Эта высокая задержка должна быть в конечном итоге в случае индексирования данных. Если уровень задержки не снижается, отправьте запрос в службу поддержки на портале Azure.

* еарлиеставаилабле

![Схема Еарлиеставаилабле](media/concepts-streaming-event-sources/event-source-earliest-available.png)

* евентсаурцекреатионтиме

![Схема Евентсаурцекреатионтиме](media/concepts-streaming-event-sources/event-source-creation-time.png)

* кустоменкуеуедтиме

![Схема Кустоменкуеуедтиме](media/concepts-streaming-event-sources/event-source-custom-enqueued-time.png)
...
> [!CAUTION] 
> При удалении центра Интернета вещей или концентратора событий и повторном создании нового ресурса с тем же именем необходимо создать новый источник событий и подключить новый центр Интернета вещей или концентратор событий. Данные не будут приняты до завершения этого шага.
...
- Увеличьте срок хранения данных центра Интернета вещей или концентратора событий до семи дней.
