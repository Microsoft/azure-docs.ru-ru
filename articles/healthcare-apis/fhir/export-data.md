...
- Пользователь не может воспользоваться преимуществами [иерархических пространств имен](../../storage/blobs/data-lake-storage-namespace.md), но нет способа нацелить экспорт на конкретный подкаталог в контейнере. Мы предоставляем только возможность ориентироваться на конкретный контейнер (где мы создаем новую папку для каждого экспорта).
