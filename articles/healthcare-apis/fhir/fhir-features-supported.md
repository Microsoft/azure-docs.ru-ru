...
| search                         | Partial   | Partial   | Partial   | См. раздел Поиск ниже.                           |
| цепочка поиска                 | Partial       | Да       | Частично   | См. Примечание 2 ниже.                                   |
| обратный поиск в цепочке         | Partial       | Да       | Частично   | См. Примечание 2 ниже.                                   |
 ...
 **Примечание 2**
* Добавляет поддержку MVP для сцепленных и обратных цепочек поиска FHIR в CosmosDB. 

  В API Azure для FHIR и на сервере FHIR с открытым кодом, поддерживаемом Cosmos, сцепленный Поиск и обратная цепочка поиска являются реализацией MVP. Чтобы выполнить цепочку поиска по Cosmos DB, реализация просматривает выражение поиска и выдает вложенные запросы для разрешения соответствующих ресурсов. Это делается для каждого уровня выражения. Если любой запрос возвращает более 100 результатов, возникнет ошибка. По умолчанию включенный в цепочку Поиск находится за функциональным флагом. Чтобы использовать поиск в цепочке в Cosmos DB, используйте заголовок `x-ms-enable-chained-search: true` . Дополнительные сведения см. в разделе [PR 1695](https://github.com/microsoft/fhir-server/pull/1695).
...
| `_security`             | Да       | Да       | Да       |         |
| `_profile`              | Partial   | Partial   | Partial   | Поддерживается в STU3. Если база данных была создана **после** 20 февраля 2021 г., также будет добавлена поддержка в R4. Мы работаем над включением _profile для баз данных, созданных до 20 февраля 2021. |
| `_text`                 | Нет        | Нет        | Нет        |         |
...
* [**Единицы запросов (RUs)**](../../cosmos-db/concepts-limits.md) — вы можете настроить до 10 000 на портале для Azure API для FHIR. Потребуется как минимум 400 или 40 RUs/ГБ, в зависимости от того, что больше. Если вам требуется более 10 000 RUs, вы можете отправить запрос в службу поддержки, чтобы увеличить это. Максимальный доступный размер — 1 000 000.
...
| 1000    | 100-150       |      25  |
| 10 000   | 225-400       |      250  |
| 100 000  | 2500 – 4000   |      2500  |

Примечание. для каждого Cosmos DB требования требуется минимальная пропускная способность 40 единиц запросов в ГБ. 
