...
* Файл конфигурации по умолчанию имеет новое имя и расположение. Ранее `/etc/iotedge/config.yaml` сведения о конфигурации устройства теперь должны быть в по `/etc/aziot/config.toml` умолчанию. `iotedge config import`Команду можно использовать для переноса сведений о конфигурации в старое расположение и синтаксис в новую.
