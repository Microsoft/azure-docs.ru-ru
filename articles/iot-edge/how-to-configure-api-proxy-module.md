...
1. Отправьте запрос в журнал или пакет поддержки в соответствии с инструкциями в статье [Извлечение журналов из IOT Edge развертываний](how-to-retrieve-iot-edge-logs.md), но используйте имя домена `$upstream` и порт открытого прокси вместо адреса модуля хранилища BLOB-объектов. Например:
