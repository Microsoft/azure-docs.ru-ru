...
5. На IoT Edge устройстве откройте файл конфигурации.

   ```bash
   sudo nano /etc/aziot/config.toml
   ```

6. Замените значение на `hostname` DNS-имя.
...
IoT Edge устройства, расположенные за шлюзом, получают свои образы модулей с родительского IoT Edge устройства, указанного в `parent_hostname` поле файла конфигурации. Эта `Could not perform HTTP request` ошибка означает, что дочернее устройство не может связаться с родительским устройством через HTTP.
