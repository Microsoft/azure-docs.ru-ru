...
В этой статье показано, как создать индивидуальную или групповую регистрацию службы подготовки устройств с помощью аттестации симметричных ключей на IoT Edge устройстве, выполнив следующие действия.

* Создание экземпляра Службы подготовки устройств к добавлению в Центр Интернета вещей.
* Создайте регистрацию для устройства.
...
> [!TIP]
> Для групповых регистраций требуется [производный ключ](#derive-a-device-key) каждого устройства, а не первичный ключ регистрации DPS.
...
1. Найдите в файле раздел **подготовки**. Раскомментируйте строки для подготовки политики диагностики с симметричным ключом и убедитесь, что все остальные строки подготовки снабжены комментариями.
...
1. Обновите значения параметров `id_scope` , `registration_id` и `symmetric_key` с помощью сведений об DP и устройстве.

   Параметр симметричного ключа может принимать значение встроенного ключа, URI файла или URI PKCS # 11. Раскомментируйте только одну строку симметричного ключа в зависимости от формата, который вы используете.
...
1. Сохраните и закройте файл config. томл.
