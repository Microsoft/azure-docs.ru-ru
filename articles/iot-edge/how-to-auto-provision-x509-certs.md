...
### <a name="use-test-certificates-optional"></a>Использовать тестовые сертификаты (необязательно)
...
1. Найдите раздел конфигурации подготовки файла. Раскомментируйте строки для подготовки сертификата DPS X. 509 и убедитесь, что все остальные строки подготовки снабжены комментариями.
...
1. При необходимости укажите `registration_id` для устройства. В противном случае оставьте строку с комментарием, чтобы зарегистрировать устройство с именем CN сертификата удостоверения.

1. При необходимости используйте `always_reprovision_on_startup` `dynamic_reprovisioning` строки или для настройки поведения повторной подготовки устройства. Если устройство настроено для повторной подготовки при запуске, оно всегда будет пытаться подготовиться к службе DPS сначала, а затем вернуться к резервной копии при сбое. Если для устройства настроена динамическая повторная инициализация, IoT Edge будут перезапущены и повторно подготавливаются, если будет обнаружено событие повторной инициализации. Дополнительные сведения см. в разделе [Основные понятия повторной инициализации устройств центра Интернета вещей](../iot-dps/concepts-device-reprovision.md).

1. Сохраните и закройте файл config. YAML.
...
1. Создайте файл конфигурации для устройства на основе файла шаблона, который предоставляется в составе установки IoT Edge.
...
1. Найдите в файле раздел **подготовки**. Раскомментируйте строки для подготовки политики диагностики с помощью сертификата X. 509 и убедитесь, что все остальные строки подготовки снабжены комментариями.
...
1. Обновите значения параметров `id_scope` , `identity_cert` и `identity_pk` с помощью сведений об DP и устройстве.

   Значение сертификата удостоверения может быть предоставлено в виде универсального кода ресурса (URI) файла или может быть динамически выдано с помощью средства EST или локального центра сертификации. Раскомментируйте только одну строку в зависимости от выбранного формата.

   Значение закрытого ключа удостоверения можно указать в виде универсального кода ресурса (URI) файла или URI PKCS # 11. Раскомментируйте только одну строку в зависимости от выбранного формата.

   Если вы используете любые URI PKCS # 11, найдите раздел **PKCS # 11** в файле конфигурации и укажите сведения о своей конфигурации PKCS # 11.

1. При необходимости укажите `registration_id` для устройства. В противном случае оставьте строку с комментарием, чтобы зарегистрировать устройство с общим именем сертификата удостоверения.

1. Сохраните файл и закройте его.

1. Примените изменения конфигурации, внесенные в IoT Edge.
