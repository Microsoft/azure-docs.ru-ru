...
* Просмотрите более подробные журналы IoT Edge диспетчера безопасности:
  ...
  3. Перезапустите управляющую программу IoT Edgeного управления:
...
* Просмотр состояния системных служб IoT Edge:

   ```bash
   sudo iotedge system status
   ```

* Просмотр журналов IoT Edge системных служб:

   ```bash
   sudo iotedge system logs -- -f
   ```

* Включите журналы уровня отладки, чтобы просмотреть более подробные журналы IoT Edge системных служб:
  ...
  1. Вернитесь к журналам уровня сведений по умолчанию после отладки.
...
Вы можете просмотреть сообщения, проходящие через центр IoT Edge, и собрать аналитические сведения из подробных журналов из контейнеров среды выполнения. Чтобы включить подробные журналы в этих контейнерах, задайте `RuntimeLogLevel` переменную среды в манифесте развертывания.

Чтобы просмотреть сообщения, проходящие через центр IoT Edge, задайте `RuntimeLogLevel` для переменной среды значение `debug` для модуля edgeHub.
...
Также можно проверить сообщения, отправляемые между центром Интернета вещей и устройствами IoT. Просмотрите эти сообщения с помощью [расширения центра Интернета вещей Azure для Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=vsciot-vscode.azure-iot-toolkit). Дополнительные сведения см. в записи блога об [удобном средстве при разработке с помощью Центра Интернета вещей Azure](https://blogs.msdn.microsoft.com/iotdev/2017/09/01/handy-tool-when-you-develop-with-azure-iot/).
...
|Протокол|Порт|Входящий|Исходящий|Руководство|
|--|--|--|--|--|
