...
1. На вкладке **параметры 2,2** проверьте параметры конфигурации развертывания. Когда параметры будут удовлетворены, нажмите кнопку **Далее**.

   ![Проверка параметров конфигурации развертывания](./media/how-to-install-iot-edge-on-windows/default-deployment-configuration-settings.png)

   >[!NOTE]
   >При использовании виртуальной машины Windows рекомендуется использовать параметр по умолчанию, а не внешний коммутатор, чтобы убедиться, что виртуальная машина Linux, созданная в развертывании, может получить IP-адрес.
   >
   >При использовании коммутатора по умолчанию виртуальная машина Linux назначается внутреннему IP-адресу. Этот внутренний IP-адрес не может быть достигнут за пределами виртуальной машины Windows, но при входе в виртуальную машину Windows она может быть подключена локально.
   >
   >Если вы используете Windows Server, обратите внимание, что Azure IoT Edge для Linux в Windows не поддерживает параметр по умолчанию автоматически. Для локальной виртуальной машины Windows Server убедитесь, что виртуальная машина Linux может получить IP-адрес с помощью внешнего коммутатора. Для виртуальной машины Windows Server в Azure настройте внутренний коммутатор перед развертыванием IoT Edge для Linux в Windows.
...
1. Создание IoT Edge для развертывания Linux в Windows.
   ...
   > [!NOTE]
   > Эту команду можно выполнить без параметров или при необходимости настроить развертывание с параметрами. Сведения о параметрах и значениях по умолчанию см. в [справочнике по скриптам для Linux в Windows PowerShell. IOT Edge](reference-iot-edge-for-linux-on-windows-scripts.md#deploy-eflow)
