...
1. Чтобы разрешить сообщения, отправляемые с устройства в облако, с подчиненных устройств в центр Интернета вещей, включите маршрут, передающий все сообщения в центр Интернета вещей. Например:
    1. **Имя**: `Route`
...
1. Нажмите кнопку **сохранить** , чтобы сохранить изменения в параметрах среды выполнения.
1. Нажмите кнопку **Далее: Маршруты** , чтобы перейти к следующему шагу.
1. Чтобы разрешить сообщения, отправляемые с устройства в облако, с подчиненных устройств в центр Интернета вещей, включите маршрут, передающий все сообщения `$upstream` . Вышестоящий параметр указывает на родительское устройство в случае более низких устройств. Например:
    1. **Имя**: `Route`
    1. **Значение**: `FROM /messages/* INTO $upstream`
1. Выберите **Проверка + создать** , чтобы перейти к последнему шагу.
1. Выберите **создать** для развертывания на устройстве.
