---
title: Соглашения самонастраивающийся IoT | Документация Майкрософт
description: Описание соглашений самонастраивающийся IoT, которое использует устройства при отправке телеметрии и свойствах, а также на обработку команд и обновлений свойств.
author: rido-min
ms.author: rmpablos
ms.date: 07/10/2020
ms.topic: conceptual
...
ms.locfileid: "104582686"
---
# <a name="iot-plug-and-play-conventions"></a>Соглашения IoT Plug and Play

Устройства IoT самонастраивающийся должны следовать набору соглашений при обмене сообщениями с центром Интернета вещей. Устройства IoT самонастраивающийся используют протокол MQTT для обмена данными с центром Интернета вещей.
...
Вы описываете телеметрию, свойства и команды, которые устройство IoT самонастраивающийся реализует с помощью _модели_ [Digital двойников Definition Language v2 (дтдл)](https://github.com/Azure/opendigitaltwins-dtdl) . В этой статье есть два типа модели, на которые ссылается:
...
Дополнительные сведения см. в статье [Самонастраивающийся по моделированию IOT](concepts-modeling-guide.md).

## <a name="identify-the-model"></a>Указание модели

Чтобы объявить реализуемую модель, устройство или модуль IoT самонастраивающийся включает идентификатор модели в пакет подключения MQTT, добавляя `model-id` к `USERNAME` полю.
...
Теперь, когда вы узнали о соглашениях IoT самонастраивающийся, ниже приведены некоторые дополнительные ресурсы.
