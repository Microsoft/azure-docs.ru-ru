---
title: Как сертифицировать устройства IoT самонастраивающийся | Документация Майкрософт
...
ms.locfileid: "102199651"
---
# <a name="how-to-certify-iot-plug-and-play-devices"></a>Сертификация устройств Plug and Play IoT

Программа сертификации устройств IoT Plug and Play содержит средства для проверки того, соответствует ли устройство требованиям сертификации IoT Plug and Play. Средства также помогают организациям обеспечить доступность своих устройств IoT самонастраивающийся. Эти сертифицированные устройства адаптированы для решений IoT и помогают сократить время на рынке.

В этой статье показано, как:

- Установите расширение средства командной строки Azure IoT для Azure CLI
- Запуск тестов IoT самонастраивающийся для проверки приложения устройства на этапе разработки  
...
Код приложения, который выполняется в самонастраивающийся IoT, должен:
...
Процесс сертификации проверяет, является ли устройство IoT самонастраивающийся совместимым, подтверждая, что реализация устройства соответствует телеметрии, свойствам и командам, определенным в модели устройства [дтдл (Digital двойников Definition Language)](https://github.com/Azure/opendigitaltwins-dtdl) , и что модель доступна в [репозитории общедоступной модели Azure IOT](concepts-model-repository.md).
...
- Реализация телеметрии, свойств или команд, следующих за соглашением самонастраивающийся IoT.
...
[Расширение интерфейса командной строки Azure IOT](/cli/azure/ext/azure-iot/iot/product) позволяет проверить соответствие реализации устройства модели перед отправкой устройства для сертификации через портал сертифицированного устройства Azure.
...
Чтобы настроить [Azure CLI](/cli/azure) в вашей среде, ознакомьтесь с инструкциями по установке.
...
Дополнительные сведения см. в статье [Azure CLI для Azure IOT](/cli/azure/azure-cli-reference-for-iot).
...
> [!NOTE]
> При использовании интерфейса командной строки необходимо [войти](/cli/azure/authenticate-azure-cli) в подписку.
...
Шаг подключения и тестирования проверяет соответствие устройства требованиям сертификации IoT самонастраивающийся.
