...
* **Сохраняемость RDB** — когда настроено сохранение данных RDB (Redis Database), кэш Azure для Redis сохраняет моментальный снимок кэша Azure для Redis в двоичном формате Redis на диск (в учетной записи хранения Azure) на основе настраиваемой частоты резервного копирования. В случае аварии, при которой становятся недоступными как основной экземпляр кэша, так и реплика кэша, кэш воссоздается на основе последнего моментального снимка. Узнайте больше о [преимуществах](https://redis.io/topics/persistence#rdb-advantages) и [недостатках](https://redis.io/topics/persistence#rdb-disadvantages) сохраняемости RDB.
