...
## <a name="security-technical-capabilities-to-fulfill-your-responsibility"></a>Технические возможности по обеспечению безопасности для удовлетворения ваших обязанностей
...
[Защита идентификации Azure AD](../../active-directory/identity-protection/overview-identity-protection.md) — это служба безопасности, которая предоставляет объединенное представление об обнаружении рисков и потенциальных уязвимостях, влияющих на удостоверения вашей организации. Защита идентификации использует существующие возможности обнаружения аномалий Azure Active Directory (доступны в отчетах об аномальных действиях Azure AD) и вводит новые типы обнаружения рисков, которые могут обнаруживать аномалии в режиме реального времени.
...
Если вы взаимодействуете с хранилищем Azure через портал Azure, все транзакции выполняются через HTTPS. Также для взаимодействия со [службой хранилища Azure](https://azure.microsoft.com/services/storage/) и [Базой данных SQL Azure](https://azure.microsoft.com/services/sql-database/) можно использовать [REST API хранилища](/rest/api/storageservices/) через HTTPS.
...
[Azure RMS](/azure/information-protection/what-is-azure-rms) использует для защиты файлов и электронной почты шифрование, удостоверения и политики авторизации. Azure RMS работает на нескольких устройствах — телефонах, планшетах и ПК, защищая как внутри организации, так и за ее пределами. Такие возможности Azure RMS обеспечивает благодаря дополнительному уровню защиты, которая распространяется даже на данные, выходящие за пределы вашей организации.

При использовании Azure RMS для защиты файлов применяется отраслевой стандарт шифрования с полной поддержкой [FIPS 140-2](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.140-2.pdf). При использовании Azure RMS для защиты данных гарантируется, что защита остается в файле, даже если она копируется в хранилище, которое не находится под контролем ИТ-службы, например в службе облачного хранилища. Это же применимо и в отношении файлов, передаваемых по электронной почте. Такой файл защищается как вложение, а к самому письму прилагаются инструкции по его открытию.
...
Azure также поддерживает закрытое и безопасное подключение к ресурсам PaaS (например, службе хранилища Azure и базе данных SQL) из виртуальной сети Azure с помощью [частной ссылки Azure](https://docs.microsoft.com/azure/private-link/private-link-overview). Ресурс PaaS сопоставляется с [частной конечной точкой](https://docs.microsoft.com/azure/private-link/private-endpoint-overview) в виртуальной сети. Связь между частной конечной точкой в виртуальной сети и ресурсом PaaS использует магистральную сеть Майкрософт и не переключается через общедоступный Интернет. Предоставление доступа к службе через общедоступный Интернет больше не требуется. Вы также можете использовать частную ссылку Azure для доступа к службам, принадлежащим заказчику, и партнерской службе Azure в виртуальной сети.  Кроме того, Частная ссылка Azure позволяет создать собственную [службу частной связи](https://docs.microsoft.com/azure/private-link/private-link-service-overview) в виртуальной сети и предоставить ее своим клиентам в частном порядке в своих виртуальных сетях. Настройка и потребление с использованием Приватного канала Azure согласованы между службами Azure PaaS, клиентскими и общими партнерскими службами.
...
- Поддержка подробных деловых обсуждений нормативных требований и собственных целей Организации по обеспечению конфиденциальности, личной информации и безопасности данных.
...
Операционная безопасность Azure построена на платформе, которая включает в себя набор знаний, который можно получить с помощью различных возможностей, уникальных для корпорации Майкрософт, включая жизненный цикл разработки защищенных приложений (Майкрософт) (SDL), программу центра Microsoft Security Response Center и глубокую осведомленность о кибербезопасности угрозах.
