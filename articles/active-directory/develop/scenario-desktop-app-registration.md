...
Укажите универсальный код ресурса (URI) перенаправления для приложения, [настроив параметры платформы](quickstart-register-app.md#add-a-redirect-uri) для приложения в **Регистрация приложений** в портал Azure.

- Для приложений, использующих интерактивную проверку подлинности:
  - Приложения, использующие внедренные браузеры: `https://login.microsoftonline.com/common/oauth2/nativeclient`
  - Приложения, использующие системные браузеры: `http://localhost`

  > [!IMPORTANT]
  > По соображениям безопасности рекомендуется явно задавать `https://login.microsoftonline.com/common/oauth2/nativeclient` или в `http://localhost` качестве URI перенаправления. Некоторые библиотеки проверки подлинности, например MSAL.NET, используют значение по умолчанию `urn:ietf:wg:oauth:2.0:oob` , если не указан другой URI перенаправления, что не рекомендуется. Это значение по умолчанию будет Обновлено в виде критического изменения в следующем основном выпуске.
...
- Если вы создаете Node.js приложение электронного управления, используйте специальный файловый протокол вместо стандартного URI перенаправления веб-узла (https://) для обработки этапа перенаправления потока авторизации, например `msal://redirect` . Имя протокола пользовательского файла не должно быть очевидным для предположения и должно следовать рекомендациям в [спецификации OAuth 2.0 для собственных приложений](https://tools.ietf.org/html/rfc8252#section-7.1).
