...
Наконец, установите расширение виртуальной машины для входа Azure AD, чтобы включить имя входа Azure AD для виртуальной машины Windows. Расширения виртуальных машин — это небольшие приложения, которые выполняют задачи настройки и автоматизации после развертывания виртуальных машин Azure. Используйте команду [AZ VM Extension](/cli/azure/vm/extension#az-vm-extension-set) Set, чтобы установить расширение аадлогинфорвиндовс на виртуальной машине `myVM` в `myResourceGroup` группе ресурсов:
...
1. Подключитесь к виртуальной машине по протоколу RDP, используя учетную запись локального администратора, и изучите `CommandExecution.log` файл в разделе:
...
1. Откройте командную строку PowerShell на виртуальной машине и убедитесь, что эти запросы к конечной точке службы метаданных экземпляра (IMDS), работающей на узле Azure, возвращают:
   ...
   > [!NOTE]
   > Маркер доступа можно декодировать с помощью такого средства, как [calebb.NET](http://calebb.net/). Убедитесь, что `appid` в маркере доступа совпадает управляемое удостоверение, назначенное виртуальной машине.
...
1. Состояние устройства можно просмотреть, выполнив `dsregcmd /status` . Цель — показывать состояние устройства как `AzureAdJoined : YES` .
   ...
   > [!NOTE]
   > Действие присоединение к Azure AD регистрируется в средстве просмотра событий в `User Device Registration\Admin` журнале.
...
Кроме того, вход по протоколу RDP с использованием учетных записей Azure AD регистрируется в средстве просмотра событий в `AAD\Operational` журналах событий.
...
Дополнительные сведения о Azure Active Directory см. в разделе [что такое Azure Active Directory](../fundamentals/active-directory-whatis.md).
