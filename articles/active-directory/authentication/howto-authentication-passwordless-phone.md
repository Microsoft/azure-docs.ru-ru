...
description: Включение входа без пароля в Azure AD с помощью приложения Microsoft Authenticator
...
ms.locfileid: "101653479"
---
# <a name="enable-passwordless-sign-in-with-the-microsoft-authenticator-app"></a>Включение входа без пароля с помощью приложения Microsoft Authenticator 
...
> [!NOTE]
> Если вы включили Microsoft Authenticator вход без пароля с помощью Azure AD PowerShell, он был включен для всего каталога. Если вы включили использование этого нового метода, он заменяет политику PowerShell. Рекомендуется включить для всех пользователей в клиенте через меню " *методы проверки подлинности* ". в противном случае пользователи, не воявляющиеся в новую политику, больше не смогут выполнять вход без пароля.
...
1. В разделе **Microsoft Authenticator** выберите следующие параметры.
