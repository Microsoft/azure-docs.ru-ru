...
Серверы Azure MFA, связанные с поставщиками, потребуется активировать повторно с помощью учетных данных, созданных в разделе **портал Azure**  >  **Azure Active Directory**  >    >    >  **Параметры безопасности сервера** mfa. Перед повторной активацией необходимо удалить следующие файлы из `\Program Files\Multi-Factor Authentication Server\Data\` каталога на серверах Azure MFA в вашей среде:
...
Убедившись в том, что все параметры были перенесены, можно перейти к **портал Azure**  >  **Azure Active Directory**  >    >    >  **поставщики** MFA и выбрать многоточие **...** и нажать кнопку **Удалить**.
