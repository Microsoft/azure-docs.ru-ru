...
Приложения от корпорации Майкрософт, которые поддерживают как учетные записи пользователей, так и учетные записи организации, такие как [OneDrive](https://onedrive.live.com/) или [Microsoft Learn](/learn/), иногда могут размещаться на одном URL-адресе.  Это означает, что пользователи, которым требуется доступ к этому URL-адресу для рабочих целей, также имеют доступ к нему для личного использования, что может быть запрещено при соблюдении нормативных требований.
...
1. [Для `login.live.com` использования автопилота требуется](/mem/autopilot/networking-requirements) для развертывания. Сценарии Intune и автопилотного развертывания могут завершаться сбоем, если `login.live.com` заблокировано.
1. Данные телеметрии Организации и обновления Windows, которые используют службу login.live.com для идентификаторов устройств, [перестанут работать](/windows/deployment/update/windows-update-troubleshooting#feature-updates-are-not-being-offered-while-other-updates-are).
