...
Для каждого исходящего запроса к login.microsoftonline.com, login.microsoft.com и login.windows.net вставьте два заголовка HTTP: *restrict-Access-to-клиента* и *restrict-Access-context*.

> [!NOTE]
> Не включайте поддомены в `*.login.microsoftonline.com` в конфигурации прокси-сервера. Это приведет к включению device.login.microsoftonline.com и повлияет на проверку подлинности на основе сертификата клиента, которая используется в регистрации устройств и сценариях условного доступа на базе устройств. Настройте прокси-сервер, чтобы исключить device.login.microsoftonline.com из TLS-проверки и внедрения заголовка.
...
- **Пользователь** . в этом поле могут быть удалены персональные данные, где она будет иметь значение `00000000-0000-0000-0000-000000000000` . 
- **Приложение**
- **Состояние**
- **Дата**
- **Дата (UTC)** — где время в формате UTC равно скоординированному времени
- **IP-адрес**;
- **Клиент**
- **Username** — в этом поле могут быть удалены персональные данные, где будет установлено значение `{PII Removed}@domain.com`
- **Расположение**
- **ИД целевого клиента**.
...
3. Настройте Fiddler для вставки заголовков *Restrict-Access-To-Tenants* и *Restrict-Access-Context* с помощью настраиваемых правил.
   ...
   2. Добавьте в начало функции `OnBeforeRequest` следующие строки кода. Замените на \<List of tenant identifiers\> домен, зарегистрированный в клиенте (например, `contoso.onmicrosoft.com` ). Замените \<directory ID\> идентификатором GUID Azure AD клиента.  Чтобы журналы отображались в клиенте, **необходимо** указать правильный идентификатор GUID. 
...
## <a name="blocking-consumer-applications-public-preview"></a>Блокирующие приложения-потребители (общедоступная Предварительная версия)

Приложения от корпорации Майкрософт, которые поддерживают как учетные записи пользователей, так и учетные записи организации, такие как [OneDrive](https://onedrive.live.com/) или [Microsoft Learn](https://docs.microsoft.com/learn/), иногда могут размещаться на одном URL-адресе.  Это означает, что пользователи, которым требуется доступ к этому URL-адресу для рабочих целей, также имеют доступ к нему для личного использования, что может быть запрещено при соблюдении нормативных требований.

Некоторые организации пытаются устранить эту проблему, блокируя, `login.live.com` чтобы блокировать проверку подлинности личных учетных записей.  Это имеет несколько недостатков:

1. Блокировка `login.live.com` блокирует использование личных учетных записей в гостевых сценариях B2B, которые могут поработать с посетителями и совместной работой.
1. [Для `login.live.com` использования автопилота требуется](https://docs.microsoft.com/mem/autopilot/networking-requirements) для развертывания. Сценарии Intune и автопилотного развертывания могут завершаться сбоем, если `login.live.com` заблокировано.
1. Данные телеметрии Организации и обновления Windows, которые используют службу login.live.com для идентификаторов устройств, [перестанут работать](https://docs.microsoft.com/windows/deployment/update/windows-update-troubleshooting#feature-updates-are-not-being-offered-while-other-updates-are).

### <a name="configuration-for-consumer-apps"></a>Конфигурация для потребительских приложений

Несмотря на то, `Restrict-Access-To-Tenants` что заголовок функционирует как Allow-List, блок учетная запись Майкрософт (MSA) работает как сигнал Deny, указывая учетная запись Майкрософтную платформу, чтобы запретить пользователям входить в приложения-потребители. Чтобы отправить этот сигнал, `sec-Restrict-Tenant-Access-Policy` заголовок будет внедрен в трафик, посещаемый `login.live.com` с помощью того же корпоративного прокси-сервера или брандмауэра, как [описано выше](#proxy-configuration-and-requirements). Значение заголовка должно быть `restrict-msa` . Если заголовок имеется и пользовательское приложение пытается войти в систему напрямую, то вход будет заблокирован.

В настоящее время проверка подлинности в приложениях-получателях не отображается в [журналах администратора](#admin-experience), так как login.Live.com размещается отдельно от Azure AD.

### <a name="what-the-header-does-and-does-not-block"></a>Что заданный заголовок не блокируется

`restrict-msa`Политика блокирует использование потребительских приложений, но позволяет использовать несколько других типов трафика и проверки подлинности:

1. Трафик, меньший пользователем, для устройств.  Сюда входит трафик для автопилота, Центр обновления Windows и телеметрии Организации.
1. Проверка подлинности B2B для учетных записей потребителей. Пользователи с учетными записями Майкрософт, которые [приглашены на совместную работу с клиентом](https://docs.microsoft.com/azure/active-directory/external-identities/redemption-experience#invitation-redemption-flow) , проходят проверку подлинности в login.Live.com, чтобы получить доступ к клиенту ресурса.
    1. Этот доступ контролируется с помощью `Restrict-Access-To-Tenants` заголовка, чтобы разрешить или запретить доступ к этому клиенту ресурса.
1. "Транзитная" Проверка подлинности, используемая многими приложениями Azure и Office.com, где приложения используют Azure AD для входа пользователей-потребителей в контексте потребителя.
    1. Этот доступ также контролируется с помощью `Restrict-Access-To-Tenants` заголовка, чтобы разрешить или запретить доступ к Специальному "транзитному" клиенту ( `f8cdef31-a31e-4b4a-93e4-5f571e91255a` ).  Если этот клиент не отображается в `Restrict-Access-To-Tenants` списке разрешенных доменов, учетные записи потребителей будут заблокированы Azure AD для входа в эти приложения.

## <a name="next-steps"></a>Дальнейшие действия

- Ознакомьтесь со статьей [Updated Office 365 modern authentication](https://www.microsoft.com/microsoft-365/blog/2015/03/23/office-2013-modern-authentication-public-preview-announced/) (Обновление поддержи современной аутентификации в Office 365).
