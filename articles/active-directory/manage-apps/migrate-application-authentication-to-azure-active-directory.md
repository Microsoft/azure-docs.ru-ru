...
[Azure Active Directory (Azure AD)](../fundamentals/active-directory-whatis.md) предлагает универсальную платформу идентификации, которая предоставляет своим сотрудникам, партнерам и клиентам единое удостоверение для доступа к нужным приложениям и их совместной работы с любой платформой и устройством.

![Схема подключения Azure Active Directory](media/migrating-application-authentication-to-azure-active-directory-1.jpg)

В Azure AD есть [полный набор возможностей управления удостоверениями](../fundamentals/active-directory-whatis.md#which-features-work-in-azure-ad). Стандартизация проверки подлинности и авторизации приложения в Azure AD позволяет получить преимущества, предоставляемые этими возможностями.

Дополнительные материалы по миграции см. по адресу [https://aka.ms/migrateapps](./migration-resources.md)
...
- Улучшение безопасного доступа пользователей к приложениям и связанным корпоративным данным с помощью [политик условного доступа](../conditional-access/overview.md), [многофакторной проверки подлинности](../authentication/concept-mfa-howitworks.md)и технологий [защиты идентификации](../identity-protection/overview-identity-protection.md) на основе рисков в реальном времени.

- Защита доступа привилегированного пользователя к вашей среде с помощью [JIT-](../../azure-resource-manager/managed-applications/request-just-in-time-access.md) доступа администратора.
...
- Обеспечение безопасного удаленного доступа к локальным приложениям с помощью [Azure AD application proxy](./application-proxy.md).

- Отделение приложений от локального подхода к учетным данным в клиенте путем [настройки Azure AD в качестве доверенного универсального поставщика удостоверений](../hybrid/plan-connect-user-signin.md#choosing-the-user-sign-in-method-for-your-organization).
...
- Повысьте удобство работы с [единым пользовательским Sign-Onом (SSO)](./what-is-single-sign-on.md) с помощью простого и безопасного доступа к любому приложению с любого устройства и любого расположения.

- Используйте возможности самообслуживания IAM, такие как [самостоятельный сброс пароля](../authentication/concept-sspr-howitworks.md) и [Управление группами сайтов самообслуживания](../enterprise-users/groups-self-service-management.md).
...
- Сократите административные издержки, управляя только одним удостоверением для каждого пользователя в облачной и локальной средах.
  ...
  - [Автоматизация подготовки](../app-provisioning/user-provisioning.md) учетных записей пользователей (в [коллекции Azure AD](https://azuremarketplace.microsoft.com/marketplace/apps/category/azure-active-directory-apps)) на основе удостоверений Azure AD
  - Доступ ко всем приложениям на панели "MyApps" в [портал Azure ](https://portal.azure.com/)

- Позволяет разработчикам защитить доступ к своим приложениям и улучшить взаимодействие с конечными пользователями с помощью [платформы идентификации Майкрософт](../develop/v2-overview.md) и библиотеки проверки подлинности Майкрософт (MSAL).

- Предоставьте своим партнерам доступ к облачным ресурсам с помощью [службы совместной работы Azure AD B2B](../external-identities/what-is-b2b.md). Это устраняет затраты на настройку Федерации "точка — точка" с партнерами.

### <a name="address-compliance-and-governance"></a>Соответствие и управление адресами

Обеспечение соответствия нормативным требованиям путем применения политик корпоративного доступа и мониторинга доступа пользователей к приложениям и связанным данным с помощью интегрированных средств аудита и интерфейсов API. С помощью Azure AD можно отслеживать входы в приложения с помощью отчетов, использующих [средства отслеживания инцидентов и событий безопасности (SIEM)](../reports-monitoring/plan-monitoring-and-reporting.md). Вы можете получить доступ к отчетам с помощью портала или API-интерфейсов, а также программными средствами аудита, которые имеют доступ к приложениям, и для удаления доступа неактивных пользователей с помощью проверок доступа.
...
- **Использование Azure AD Connect Health.** Если у вас есть лицензия Azure AD Premium, мы рекомендуем развернуть [Azure AD Connect Health](../hybrid/how-to-connect-health-adfs.md) , чтобы проанализировать использование приложения в локальной среде. Вы можете использовать [отчет приложения ADFS](./migrate-adfs-application-activity.md) (Предварительная версия) для обнаружения приложений ADFS, которые можно перенести, и оценить готовность переносимого приложения. После завершения миграции разверните [Cloud Discovery](/cloud-app-security/set-up-cloud-discovery) , которая позволяет постоянно отслеживать теневые ИТ в вашей организации после того, как вы находитесь в облаке.

- **AD FS синтаксический анализ журнала**. Если у вас нет лицензий Azure AD Premium, мы рекомендуем использовать ADFS для средств миграции приложений Azure AD на основе [PowerShell.](https://github.com/AzureAD/Deployment-Plans/tree/master/ADFS%20to%20AzureAD%20App%20Migration) См. [руководство по решению](./migrate-adfs-apps-to-azure.md):

[Перенос приложений из службы федерации Active Directory (AD FS) (AD FS) в Azure AD.](./migrate-adfs-apps-to-azure.md)
...
Помимо выбора в [коллекции приложений Azure AD,](https://azuremarketplace.microsoft.com/marketplace/apps/category/azure-active-directory-apps) это могут быть приложения, которые уже существуют в вашей организации или какие-либо сторонние приложения от поставщика, который не входит в коллекцию Azure AD (приложения, не относящиеся к[коллекции)](./add-application-portal.md).
...
Azure AD может предоставить преимущества этим старым приложениям, так как вы можете включить современные функции безопасности и управления Azure AD, такие как [многофакторная идентификация](../authentication/concept-mfa-howitworks.md), [Условный доступ](../conditional-access/overview.md), [Защита идентификации](../identity-protection/index.yml), [делегированный доступ к приложениям](./access-panel-manage-self-service-access.md)и проверки [доступа](../governance/manage-user-access-with-access-reviews.md#create-and-perform-an-access-review) к этим приложениям, не затрагивая приложение.

Начните с **расширения этих приложений в облако** с помощью [прокси приложения](./application-proxy-configure-single-sign-on-password-vaulting.md) Azure AD, используя простые средства проверки подлинности (например, хранилище паролей) для быстрой миграции пользователей или через [интеграцию партнеров](https://azure.microsoft.com/services/active-directory/sso/secure-hybrid-access/) с контроллерами доставки приложений, которые могли быть развернуты.

### <a name="new-line-of-business-lob-apps"></a>Новые бизнес-приложения (LoB)

Обычно бизнес-приложения разрабатываются для внутреннего использования в вашей организации. Если в конвейере есть новые приложения, мы рекомендуем использовать [платформу Microsoft Identity Platform](../develop/v2-overview.md) для реализации OpenID Connect Connect.
...
Приложения, выбранные для пилотной программы, должны представлять требования к удостоверениям и безопасности для вашей организации, а также быть в наличии четких приобретенных владельцев приложений. Пилотные проекты обычно выполняются в отдельной тестовой среде. См. рекомендации [по пилотным испытаниям](../fundamentals/active-directory-deployment-plans.md#best-practices-for-a-pilot) на странице планы развертывания.
...
Кроме того, в Azure AD предусмотрены ограничения [службы и ограничения](../enterprise-users/directory-service-limits-restrictions.md) , которые следует учитывать.
...
Это также поможет вам реализовать [пять шагов для обеспечения безопасности инфраструктуры идентификации](../../security/fundamentals/steps-secure-identity.md). Используйте рекомендации в качестве отправной точки для Организации и настройте политики в соответствии с конкретными требованиями Организации.
...
- **Внешний:** Поставщики, поставщики, дистрибьюторы или другие бизнес-партнеры, взаимодействующие с вашей организацией в рамках обычного бизнеса с помощью [службы совместной работы Azure AD B2B.](../external-identities/what-is-b2b.md)

Вы можете определить группы для этих пользователей и заполнять эти группы различными способами. Вы можете выбрать, что администратор должен вручную добавить членов в группу или включить членство в группе сайтов самообслуживания. Можно установить правила, которые автоматически добавляют элементы в группы на основе заданных критериев с помощью [динамических групп](../enterprise-users/groups-dynamic-membership.md).

Внешние пользователи также могут обращаться к клиентам, которые требуют особого внимания. [Azure AD B2C](../../active-directory-b2c/overview.md), отдельный продукт поддерживает проверку подлинности клиента. Однако он выходит за рамки данного документа.
...
- **Общие рекомендации по миграции** — используйте технический документ, инструменты, шаблоны электронной почты и приложения анкеты в [средстве миграции приложений Azure AD](./migration-resources.md) для обнаружения, классификации и миграции приложений.

- **Приложения SaaS** — см. наш список [сотен учебных руководств по приложениям SaaS](../saas-apps/tutorial-list.md) и полный [план развертывания единого входа Azure AD](https://aka.ms/ssodeploymentplan) для пошагового выполнения полного процесса.

- **Приложения, работающие в локальной среде** — Узнайте все [о AD application proxy Azure](./application-proxy.md) и используйте полный [план развертывания Azure AD application proxy](https://aka.ms/AppProxyDPDownload) , чтобы быстро приступить к работе.

- **Разрабатываемые приложения** — ознакомьтесь с нашим пошаговым руководством по [интеграции](../develop/quickstart-register-app.md) и [регистрации](../develop/quickstart-register-app.md) .
...
Чтобы протестировать каждое приложение, войдите в систему с помощью тестового пользователя и убедитесь, что все функциональные возможности одинаковы до миграции. Если во время тестирования вы определили, что пользователям потребуется обновить свои параметры [MFA](/active-directory/authentication/howto-mfa-userstates) или [SSPR](../authentication/tutorial-enable-sspr.md)или вы добавляете эти функции во время миграции, обязательно добавьте его в план взаимодействия с конечным пользователем. См. раздел [MFA](https://aka.ms/mfatemplates) и шаблоны связи для конечных пользователей [SSPR](https://aka.ms/ssprtemplates) .
...
- Выберите **Управление &gt; условным доступом**. Проверьте список политик и убедитесь, что вы не блокируете доступ к приложению с помощью [политики условного доступа](../conditional-access/overview.md).
...
| **Единый вход на основе пароля** | Скачайте и установите [расширение для безопасного входа в MyApps](../user-help/my-apps-portal-end-user-access.md#download-and-install-the-my-apps-secure-sign-in-extension). Это расширение позволяет запускать любые облачные приложения вашей организации, требующие использования единого входа. |
...
При возникновении проблем ознакомьтесь с [руководством по устранению неполадок приложений](../app-provisioning/isv-automatic-provisioning-multi-tenant-apps.md) , чтобы получить помощь. См. также [проблемы при входе в приложение, разработанное специально для](./application-sign-in-problem-federated-sso-gallery.md).
...
Наведите на себя **пользователя** на портале [MyApps](../user-help/my-apps-portal-end-user-access.md#download-and-install-the-my-apps-secure-sign-in-extension). Здесь они могут получить доступ ко всем облачным приложениям, приложениям, предоставляемым с помощью [Azure AD Connect](../hybrid/whatis-azure-ad-connect.md), а приложения, использующие [прокси приложения](./application-proxy.md) , имеют разрешения на доступ к этим приложениям.


Вы можете помочь пользователям узнать, как обнаруживать свои приложения:

- Использование [существующей функции единого входа](./view-applications-portal.md) для **связывания пользователей с любым приложением**


- Включить [самостоятельный доступ](./manage-self-service-access.md)к приложению для приложения и **позволить пользователям добавлять приложения, которые вы** намерены

- [Скрыть приложения от конечных пользователей](./hide-application-from-user-portal.md) (приложения Майкрософт по умолчанию или другие приложения), чтобы **сделать приложения более доступными для обнаружения**

### <a name="make-apps-accessible"></a>Сделать приложения доступными

**Разрешить пользователям доступ к приложениям с мобильных устройств**. Пользователи могут получить доступ к порталу MyApps с помощью браузера, управляемого Intune, на устройствах [iOS](./hide-application-from-user-portal.md) 7,0 или более поздней версии или [Android](./hide-application-from-user-portal.md) .
...
- **Автоматически преобразуйте внутренние URL-адреса** , настроенные в [прокси приложения](./application-proxy.md) , на соответствующие внешние URL-адреса. Теперь пользователи могут работать со ссылками, которыми они знакомы, независимо от того, где они находятся.
...
- **Обеспечьте безопасный доступ пользователей к приложениям.** Включение [политик условного доступа](../conditional-access/overview.md)или [защиты идентификации](../identity-protection/overview-identity-protection.md)для безопасного доступа пользователей к приложениям в зависимости от состояния устройства, расположения и других возможностей.

- **Автоматическая подготовка.** Настройте [автоматическую подготовку пользователей](../app-provisioning/user-provisioning.md) с помощью различных сторонних приложений SaaS, доступ к которым необходим пользователям. Помимо создания удостоверений пользователей, оно включает в себя обслуживание и удаление удостоверений пользователей по мере изменения состояния или ролей.

- **Делегирование управления доступом пользователей** . При необходимости включите самостоятельный доступ к приложениям для приложений и *назначьте утверждающего для утверждения доступа к этим приложениям*. Использование [самостоятельного управления](../enterprise-users/groups-self-service-management.md)группами для групп, назначенных коллекциям приложений.
...
- **Аудит приложения** с помощью **корпоративных приложений, аудит** или доступ к тем же данным из [API отчетов Azure AD](../reports-monitoring/concept-reporting-api.md) для интеграции в избранные инструменты.
...
- **Получение подробных сведений о входе** с помощью **корпоративных приложений, входов в систему**. Получите доступ к тем же сведениям из [API отчетов Azure AD.](../reports-monitoring/concept-reporting-api.md)

- **Визуализация использования приложения** из [пакета содержимого Azure AD PowerBI](../reports-monitoring/howto-use-azure-monitor-workbooks.md)
...
Многие [планы развертывания](../fundamentals/active-directory-deployment-plans.md) доступны для использования, и мы всегда делаем больше!
