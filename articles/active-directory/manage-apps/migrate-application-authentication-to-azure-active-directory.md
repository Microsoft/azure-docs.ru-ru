...
Дополнительные материалы по миграции можно найти по адресу [https://aka.ms/migrateapps](./migration-resources.md)
...
- Предоставьте своим партнерам доступ к облачным ресурсам с помощью [службы совместной работы Azure AD B2B](../external-identities/what-is-b2b.md). Облачные ресурсы изменяют затраты на настройку Федерации "точка — точка" с вашими партнерами.
...
Эти сведения можно использовать для защиты доступа ко всем службам, интегрированным с Azure AD. Эти рекомендации согласованы с рейтингом Microsoft Secure и [рейтингом удостоверений в Azure AD](../fundamentals/identity-secure-score.md). Оценка поможет вам:
...
Внешние пользователи также могут ссылаться на клиентов. [Azure AD B2C](../../active-directory-b2c/overview.md), отдельный продукт поддерживает проверку подлинности клиента. Однако он выходит за рамки данного документа.
...
- **Разрабатываемые приложения** — ознакомьтесь с пошаговыми инструкциями по [интеграции](../develop/quickstart-register-app.md) и [регистрации](../develop/quickstart-register-app.md) .
...
Чтобы протестировать каждое приложение, войдите в систему с помощью тестового пользователя и убедитесь, что все функциональные возможности одинаковы до миграции. Если во время тестирования вы определили, что пользователям потребуется обновить свои параметры [MFA](/azure/active-directory/authentication/howto-mfa-userstates) или [SSPR](../authentication/tutorial-enable-sspr.md)или вы добавляете эти функции во время миграции, обязательно добавьте его в план взаимодействия конечных пользователей. См. раздел [MFA](https://aka.ms/mfatemplates) и шаблоны связи для конечных пользователей [SSPR](https://aka.ms/ssprtemplates) .

После переноса приложений перейдите в [портал Azure](https://aad.portal.azure.com/) , чтобы проверить успешность миграции. Выполните приведенные далее инструкции.
...
- Проведите **Аудит приложения** с помощью * * корпоративных приложений, аудита или доступа к той же информации из [API отчетов Azure AD](../reports-monitoring/concept-reporting-api.md) , чтобы интегрировать их в избранные инструменты.
...
- **Визуализация использования приложения** из [пакета содержимого Azure AD Power BI](../reports-monitoring/howto-use-azure-monitor-workbooks.md)
