...
- Агент Azure AD Connect Health для AD FS (версия 3.1.95.0)

  - Исправление для разрешения имени пользователя в формате NT4 в качестве имени участника-пользователя во время событий входа.
  - Исправление для идентификации неверных сценариев идентификаторов приложений с выделенным кодом ошибки.
  - Изменения для добавления нового свойства для идентификатора клиента OAuth.
  - Исправление для вывода правильных значений в полях **протокол** и **тип проверки подлинности** в Sign-In отчете Azure AD для некоторых сценариев входа в систему.
  - Исправление для показа IP-адресов в поле IP-цепочки в отчете Azure AD Sign-In в порядке запроса.
  - Изменения, внесенные в новое поле, чтобы различаться при запросе дополнительной проверки подлинности во время входа.
  - Исправление для AD FS свойства идентификатора приложения, отображаемого в отчете Azure AD Sign-In.
...
- Агент Azure AD Connect Health для AD FS (версия 3.1.77.0)
   ...
   - Исправление ошибки для предупреждения "Недопустимое имя субъекта-службы (SPN) для службы AD FS", для которого оповещение сообщается неправильно.
...
*  Агент Azure AD Connect Health для синхронизации (версия 3.1.7.0) выпущен вместе с Azure AD Connect версии 1.1.880.0.    
   1. Исправление [высокой нагрузки на ЦП агента мониторинга с выпусками платформа .NET Framework KB](https://support.microsoft.com/help/4346822/high-cpu-issue-in-azure-active-directory-connect-health-for-sync)
