...
Конечные точки находятся в `{host}/scim/` каталоге, и для взаимодействия с ними можно использовать стандартные HTTP-запросы. Сведения об изменении `/scim/` маршрута см. в разделе *контроллерконстант. CS* в   >    >  **контроллерах** азуреадпровисионингсЦимреференце сЦимреференцеапи.
