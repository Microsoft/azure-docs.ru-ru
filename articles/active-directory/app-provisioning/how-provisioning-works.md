...
- Новый начальный цикл активируется с помощью параметра " **перезапустить подготовку** " в портал Azure или с помощью соответствующей команды API Microsoft Graph. При выполнении этого действия также удаляются все сохраненные водяные знаки, а все исходные объекты должны быть оценены повторно.
