...
- **Строка примеров**. Мы включили в шаблон строку с примерами значений для каждого столбца. Строку примеров необходимо удалить, заменив реальными значениями.
...
3. В разделе **Управление** выберите элемент **Все пользователи**.
4. Выберите элементы **Массовые операции** > **Массовое приглашение**.

    ![Кнопка массового приглашения](media/tutorial-bulk-invite/bulk-invite-button.png)
...
4. На странице **Массовое приглашение пользователей** выберите действие **Скачать**, чтобы получить готовый CSV-файл со свойствами для приглашения.
     ...
     ![Загрузка CSV-файла](media/tutorial-bulk-invite/download-button.png)

1. Откройте CSV-файл шаблона и добавьте в него строку для каждого гостевого пользователя. Ниже перечислены обязательные параметры.

   * **Адрес электронной почты для приглашения** обозначает пользователя, который получит приглашение.

   * **URL-адрес перенаправления** — это URL-адрес, на который перенаправляется приглашенный пользователь после принятия приглашения. Если вы хотите перенаправить пользователя на страницу "Мои приложения", нужно изменить это значение на https://myapps.microsoft.com или https://myapplications.microsoft.com.
