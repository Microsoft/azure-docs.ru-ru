...
| **Обработка масштабируемых метаданных** | В больших данных даже метаданные могут быть "большими данными". Delta Lake обрабатывает метаданные так же, как данные, используя распределенную вычислительную мощность Spark. В результате Delta Lake может легко работать с таблицами, вмещающими несколько петабайтов данных с миллиардами разделов и файлов. |
...
| **Единый пакет и источник потоковой передачи и приемник** | Таблица в Delta Lake — это как пакетная таблица, так и источник потоковой передачи и приемник. Прием потоковых данных, выполнение пакетов задним числом и интерактивные запросы работают без дополнительной настройки. |
