...
2. Задайте параметр расположения для своей среды, а затем выполните следующую команду, чтобы установить службы IIS на виртуальной машине: 
...
1. Повторите эти действия для **myVM2**.

   :::image type="content" source="../media/application-gateway-web-application-firewall-portal/application-gateway-backend.png" alt-text="Добавление внутренних серверов":::
...
Все настройки и параметры WAF находятся в отдельном объекте, который называется политикой WAF. Политика должна быть связана с вашим шлюзом приложений. 

Создайте базовую политику WAF с управляемым набором правил по умолчанию (DRS).

1. Вверху слева на портале выберите **Создать ресурс**. Введите **WAF** для поиска, выберите элемент **Брандмауэр веб-приложения** и щелкните **Создать**.
2. На странице **Создание политики WAFН** на вкладке **Основные сведения** введите или выберите следующие сведения, примите значения по умолчанию для остальных параметров и нажмите кнопку **Просмотр и создание**:

   |Параметр  |Значение  |
   |---------|---------|
   |Объект политики     |Региональный WAF (Шлюз приложений)|
...
1. Щелкните **Далее: параметры политики**.
1. Примите остальные значения по умолчанию и щелкните **Далее: управляемые правила**.
1. Примите остальные значения по умолчанию и щелкните **Далее: настраиваемые правила**.
1. Щелкните **Далее: ассоциация**.
1. Щелкните **Добавить ассоциацию**, а затем выберите **Шлюз приложений**.
1. Установите флажок **Apply the Web Application Firewall policy configuration even if it is different from the current configuration** (Применить конфигурацию политики Брандмауэра веб-приложений, даже если она отличается от текущей).
1. Выберите **Добавить**.
1. На вкладке **Ассоциация** щелкните **Добавить ассоциацию** и выберите **Шлюз приложений**.
