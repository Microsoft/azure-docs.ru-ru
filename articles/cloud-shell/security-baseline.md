...
**Руководство**. Azure Cloud Shell — это интерфейс командной строки на основе браузера, использующий ту же авторизацию, которая используется для доступа к портал Azure. в этом случае для Cloud Shell также потребуется любая многофакторная проверка подлинности, необходимая для подключения к портал Azure.
