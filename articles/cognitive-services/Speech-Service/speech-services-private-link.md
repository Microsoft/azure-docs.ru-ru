...
В этой статье описано, как удалить частные конечные точки позже, но по-прежнему использовать речевой ресурс.
...
> [!WARNING]
> Речевой ресурс, использующий пользовательское доменное имя, взаимодействует с голосовыми службами по-другому.
> Может потребоваться настроить код приложения для использования речевого ресурса с частной конечной точкой, а также для использования речевого ресурса _без закрытой_ конечной точки.
> Могут потребоваться оба сценария, так как переключение на имя личного домена _необратимо_ .
>
...
```
## <a name="turn-on-a-custom-domain-name"></a>Включение пользовательского доменного имени

Чтобы использовать пользовательское доменное имя с выбранным ресурсом речи, используйте команду [AZ cognitiveservices Account Update](/cli/azure/cognitiveservices/account#az_cognitiveservices_account_update) .
...
## <a name="turn-on-private-endpoints"></a>Включить частные конечные точки

Мы рекомендуем использовать [закрытую зону DNS](../../dns/private-dns-overview.md) , подключенную к виртуальной сети, с необходимыми обновлениями для частных конечных точек. В процессе подготовки можно создать частную зону DNS. Если вы используете собственный DNS-сервер, вам также может потребоваться изменить конфигурацию DNS.
...
Для создания частных конечных точек используйте одну из следующих статей.
В этих статьях используется веб-приложение в качестве примера ресурса для предоставления доступа через частные конечные точки.
...
## <a name="adjust-an-application-to-use-a-speech-resource-with-a-private-endpoint"></a>Настройка приложения для использования речевого ресурса с частной конечной точкой

Речевой ресурс с пользовательским доменом взаимодействует с голосовыми службами другим способом. Это справедливо для ресурса речи с поддержкой пользовательского домена как с закрытыми конечными точками, так и без них. Сведения в этом разделе относятся к обоим сценариям.

Следуйте инструкциям в этом разделе, чтобы настроить существующие приложения и решения для использования речевого ресурса с пользовательским именем домена и включенной частной конечной точкой.

Речевой ресурс с пользовательским именем домена и частной конечной точкой использует другой способ взаимодействия со службами речи. В этом разделе объясняется, как использовать такой ресурс с интерфейсами API и [пакетом SDK](speech-sdk.md)для речевых служб.

> [!NOTE]
> Речевой ресурс без закрытых конечных точек, использующих пользовательское доменное имя, также имеет особый способ взаимодействия со службами речи.
> Этот способ отличается от сценария использования речевого ресурса, использующего закрытую конечную точку. Это важно учитывать, так как вы можете удалить частные конечные точки позже.
> См. раздел _Настройка приложения для использования речевого ресурса без закрытых конечных точек_ в этой статье.
...
>
> Использование маркера авторизации и его передача в специальную конечную точку через `Authorization` заголовок будет работать, *только* если вы включили параметр доступ **ко всем сетям** в разделе " **Сетевые подключения** " вашего ресурса речи. В других случаях при `Forbidden` `BadRequest` попытке получить маркер авторизации возникнет ошибка или.
...
## <a name="adjust-an-application-to-use-a-speech-resource-without-private-endpoints"></a>Настройка приложения для использования речевого ресурса без закрытых конечных точек
...
В этом разделе объясняется, как использовать речевой ресурс с пользовательским именем домена, но *без* частных конечных точек с интерфейсами API и [пакетом SDK](speech-sdk.md)для речевых служб. Это может быть ресурс, который был бы использован в сценарии частной конечной точки, но в нем были удалены частные конечные точки.
