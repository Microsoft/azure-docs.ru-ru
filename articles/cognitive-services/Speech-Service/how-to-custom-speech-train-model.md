...
> [!NOTE]
> Базовые модели и пользовательские модели можно использовать только до определенной даты (см. раздел [модель и жизненный цикл конечной точки](./how-to-custom-speech-model-and-endpoint-lifecycle.md)). В Speech Studio эта дата отображается в столбце **срок действия** для каждой модели и конечной точки. После этого запроса к конечной точке или записи пакетной службы может произойти сбой или вернуться к базовой модели.
>
