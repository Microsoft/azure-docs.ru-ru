...
Контейнер Распознавание речи (LUIS) загружает подготовленную или опубликованную модель Распознавание речи. Как [Luis приложение](https://www.luis.ai)контейнер DOCKER предоставляет доступ к прогнозам запросов из конечных точек API контейнера. Вы можете получать журналы запросов из контейнера и отправлять их обратно в Распознавание речиное приложение, чтобы улучшить точность прогнозов приложения.
