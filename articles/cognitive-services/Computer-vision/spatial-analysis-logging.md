...
|--|--|
|допост| Либо *true* , либо *false*. Указывает, были ли отправлены журналы. Если вы решили не отправлять журналы, API возвращает информацию ***синхронно** _. При выборе отправки журналов API возвращает значение 200, если запрос является допустимым, и начинает отправку журналов в _асинхронном режиме_* *.|
...
## <a name="how-to-file-a-support-ticket-for-spatial-analysis"></a>Как заменять запрос в службу поддержки для пространственного анализа 

Если требуется дополнительная поддержка для поиска решения проблемы с контейнером пространственного анализа, выполните следующие действия, чтобы заполнить и отправить запрос в службу поддержки. Наша группа получит дополнительные рекомендации. 

### <a name="fill-out-the-basics"></a>Заполните основы 
На [новой странице запроса в службу](https://ms.portal.azure.com/#blade/Microsoft_Azure_Support/HelpAndSupportBlade/newsupportrequest) поддержки создайте новый запрос в службу поддержки. Следуйте инструкциям на экране, чтобы ввести следующие параметры:

![Основные сведения о поддержке](./media/support-ticket-page-1-final.png)

1. Задайте **тип проблемы** `Technical` .
2. Выберите подписку, которую вы используете для развертывания контейнера пространственного анализа.
3. Выберите `My services` и выберите в `Cognitive Services` качестве службы.
4. Выберите ресурс, который вы используете для развертывания контейнера пространственного анализа.
5. Напишите краткое описание проблемы, о которой вы сталкиваетесь. 
6. Выберите `Spatial Analysis` в качестве типа проблемы.
7. Выберите соответствующий подтип в раскрывающемся списке.
8. Нажмите кнопку **Далее: решения** , чтобы перейти на следующую страницу.

### <a name="recommended-solutions"></a>Рекомендуемые решения
На следующем этапе будут предложены Рекомендуемые решения для выбранного типа проблемы. Эти решения решают наиболее распространенные проблемы, но если это не полезно для вашего решения, нажмите кнопку **Далее: подробности** , чтобы перейти к следующему шагу.

### <a name="details"></a>Сведения
На этой странице добавьте дополнительные сведения о проблеме, с которой вы столкнулись. Не забудьте включить как можно больше сведений, так как это поможет нашим специалистам лучше сократить проблемы. Включите предпочтительный способ связи и серьезность проблемы, чтобы мы могли правильно связаться с вами, и выберите **Далее: Проверка + создать** , чтобы перейти к следующему шагу. 

### <a name="review-and-create"></a>Проверка и создание 
Ознакомьтесь с подробными сведениями о запросе в службу поддержки, чтобы убедиться, что все правильно и представляет проблему эффективно. Когда вы будете готовы, щелкните **создать** , чтобы отправить билет нашей команде. После получения билета Вы получите подтверждение по электронной почте, и наша группа будет работать как можно быстрее. Вы можете просмотреть состояние билета в портал Azure.
