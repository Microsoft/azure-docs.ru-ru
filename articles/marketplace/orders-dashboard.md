---
title: Панель мониторинга заказов центра партнеров в службе "аналитика коммерческих рынков" | Microsoft AppSource и Azure Marketplace
...
Дополнительные сведения об использовании рабочих мест, сайтов и лимитных счетов см. в статье [планирование предложения SaaS для коммерческого рынка](plan-saas-offer.md).
...
***Таблица 1. Словарь терминов данных***

| Имя столбца в<br>пользовательский интерфейс | Имя атрибута | Определение | Имя столбца в программном<br>доступ к отчетам |
| ------------ | ------------- | ------------- | ------------- |
| Идентификатор подписки на платформу | Идентификатор подписки на платформу | Уникальный идентификатор, связанный с подпиской Azure, которую клиент использовал для приобретения предложения коммерческого рынка. Для предложений инфраструктуры это GUID подписки Azure клиента. Для предложений SaaS это отображается как нули, так как для покупок SaaS не требуется подписка Azure. | Идентификатор подписки на платформу |
| монсстартдате | Дата начала месяца | Дата начала месяца представляет месяц покупки. Формат: гггг-мм-дд. | монсстартдате |
| Тип предложения | Тип предложения | Тип предложения коммерческого рынка. | OfferType |
| Тип лицензии Azure | Тип лицензии Azure | Тип лицензионного соглашения, используемого клиентами для покупок в Azure. Другое название — канал. Допустимые значения:<ul><li>Поставщик облачных решений</li><li>Enterprise</li><li>Enterprise через торгового посредника</li><li>Оплата по мере использования</li></ul> | азурелиценсетипе |
| Тип лицензии Marketplace | Тип лицензии Marketplace | Метод выставления счетов коммерческого предложения. Различные значения:<ul><li>Счета за использование Azure</li><li>Использование собственной лицензии</li><li>Бесплатный</li><li>Корпорация Майкрософт в качестве торгового посредника</li></ul> | маркетплацелиценсетипе |
...
| Идентификатор заказа | Идентификатор заказа | Уникальный идентификатор заказа клиента для службы коммерческого рынка. Предложения на основе использования виртуальных машин не связаны с заказом. | OrderId |
| Заказанное количество | Заказанное количество | Число активов, связанных с ИДЕНТИФИКАТОРом заказа для активных заказов | OrderQuantity |
...
| Новый клиент | Новый клиент | Значение определяет, приобрел ли новый клиент одно или несколько предложений в первый раз. Значением будет "да", если дата попадает на тот же календарный месяц, что и дата приобретения. Значением будет "нет", если клиент приобретал какие-либо ваши предложения до отчетного календарного месяца. | исневкустомер |
| Состояние заказа | Состояние заказа | Состояние коммерческого заказа Marketplace на момент последнего обновления данных. | OrderStatus |
| Дата отмены заказа | Дата отмены заказа | Дата отмены заказа на коммерческой рынок. | ордерканцелдате |
| Название компании клиента | Название компании клиента | Название компании, которое указал клиент. Имя может отличаться от названия города в подписке Azure клиента. | кустомеркомпанинаме |
| Дата покупки по заказу | Дата покупки по заказу | Дата создания заказа на коммерческой рынок. Формат: гггг-мм-дд. | ордерпурчаседате |
...
| AssetCount | Число ресурсов | Число активов, связанных с ИДЕНТИФИКАТОРом заказа. | Не рекомендуется |
|||||
