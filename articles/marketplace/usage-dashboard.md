...
- Нормализованная гистограмма с накоплением **по предложениям** отображает распределение нормализованных часов использования для пяти первых предложений в соответствии с выбранным периодом вычислений. Пять ведущих предложений отображаются на диаграмме, а остальные группируются в категории **остальные** .
...
| Имя столбца в<br>пользовательский интерфейс | Имя атрибута | Определение | Имя столбца в программном<br>доступ к отчетам |
| ------------ | ------------- | ------------- | ------------- |
| Идентификатор подписки на платформу | Идентификатор подписки на платформу | Уникальный идентификатор, связанный с подпиской Azure, которую клиент использовал для приобретения предложения коммерческого рынка. Ранее назывался GUID подписки Azure. | маркетплацесубскриптионид |
| монсстартдате | Дата начала месяца | Дата начала месяца представляет месяц покупки. | монсстартдате |
| Тип предложения | Тип предложения | Тип предложения коммерческого рынка. | OfferType |
| Тип лицензии Azure | Тип лицензии Azure | Тип лицензионного соглашения, используемого клиентами для покупок в Azure. Также называется каналом. Допустимые значения:<ui><li>Поставщик облачных решений</li><li>Enterprise</li><li>Enterprise через торгового посредника</li><li>Оплата по мере использования</li></ul> | азурелиценсетипе |
| Тип лицензии Marketplace | Тип лицензии Marketplace | Метод выставления счетов коммерческого предложения. Допустимые значения:<ul><li>Счета за использование Azure</li><li>Использование собственной лицензии</li><li>Бесплатный</li><li>Корпорация Майкрософт в качестве торгового посредника</li></ul> | маркетплацелиценсетипе |
| номер SKU | номер SKU | План, связанный с предложением. | номер SKU |
| Страна клиента | Страна или регион клиента | Название страны или региона, которое указал клиент. Страна или регион могут отличаться от страны или региона в подписке Azure клиента. | CustomerCountry |
| Предварительная версия SKU | Предварительная версия SKU | Значение показывает, помечен ли SKU как "предварительный просмотр". Значение "да" указывает, что номер SKU был помечен таким образом и развертывать и использовать этот образ могут только пользователи подписок Azure, которым вы дали разрешение. Если номер SKU не был помечен как "предварительная версия", значением будет "нет". | испревиевску |
| Тип начисления платы за SKU | Тип начисления платы за SKU | Тип выставления счетов, связанный с каждым SKU в предложении. Допустимые значения:<ul><li>Free</li><li>Платный</li></ul> | скубиллингтипе |
| Внутренний | Не рекомендуется | Не рекомендуется | Не рекомендуется |
| Размер виртуальной машины | Размер виртуальной машины | Для типов предложений на основе виртуальных машин эта сущность обозначает размер виртуальной машины, связанной с номером SKU предложения. | VMSize |
| Имя облачного экземпляра | Имя облачного экземпляра | Экземпляр Microsoft Cloud, в котором была развернута виртуальная машина | CloudInstanceName |
| сервицепланнаме | Не рекомендуется | Не рекомендуется (то же определение, что и SKU) | сервицепланнаме |
| Название предложения | Название предложения | Имя предложения коммерческого рынка. | OfferName |
| DeploymentMethod | Не рекомендуется | Не рекомендуется (то же определение, что и тип предложения) | DeploymentMethod |
| Название компании клиента | Название компании клиента | Название компании, которое указал клиент. Имя может отличаться от имени в подписке Azure клиента. | кустомеркомпанинаме |
| Дата использования | Дата использования | Дата создания события использования для активов на основе использования. | UsageDate |
| исмултисолутион | Является многорешению | Указывает, является ли предложение типом предложения для решения. | исмултисолутион |
| Новый клиент | Не рекомендуется | Не рекомендуется | исневкустомер |
| Размер ядра | Размер ядра | Число ядер, связанных с предложением на основе виртуальной машины. | коресизе |
| Тип потребления | Тип потребления | Указывает, является ли событие использования, связанное с предложением, одним из следующих:<ul><li>Нормализованное использование</li><li>Необработанное использование</li><li>Измерение эффективности использования</li></ul> | UsageType |
| Дата окончания срока действия пробной версии | Дата окончания срока действия пробной версии | Дата, когда пробный период для определенного заказа закончится или закончился. | триаленддате |
| Валюта клиента | Валюта клиента | Валюта, используемая клиентом для транзакции коммерческого магазина. | кустомеркурренцикк |
| Цена (в валюте клиента) | Цена | Цена за единицу SKU, показанную в валюте клиента. | прицекк |
| Валюта выплаты | Валюта выплаты | Издатель оплачивается за события использования, связанные с ресурсом в валюте, настроенной издателем. | пайауткурренципк |
| Ориентировочная цена (в валюте издателя) | Предполагаемая цена | Цена за единицу SKU в валюте, настроенной издателем. | естиматедприцепк |
| Ссылка на данные о потреблении | Ссылка на данные о потреблении | Объединенный идентификатор GUID, используемый для подключения отчета об использовании (в коммерческом анализе Marketplace) к отчету о транзакциях выплаты. Ссылка на использование связана с OrderId и Линеитемид в отчете о транзакциях выплат. | усажереференце |
| Единица использования | Единица использования | Единица потребления, связанная с номером SKU. | усажеунит |
| ИД клиента | ИД клиента | Уникальный идентификатор, назначенный клиенту. У клиента может быть не более одной подписки Azure Marketplace. | CustomerId |
| Идентификатор счета выставления счетов | Идентификатор счета выставления счетов | Идентификатор учетной записи, для которой формируется выставление счетов. Сопоставьте **идентификатор счета для выставления счетов с кодом** **CustomerID** , чтобы подключить отчет о транзакциях выплат с отчетами по клиентам, заказам и использованию. | биллингаккаунтид |
| Количество использований | Количество использований | Совокупные единицы использования, потребляемые ресурсом, который развертывается клиентом.<br>Это зависит от элемента типа использования. Например, если типом использования является нормализованное использование, то для нормализованного использования используется количество для использования. | усажекуантити |
| нормализедусаже | Нормализованное потребление | Общая нормализованная единица использования, потребляемая ресурсом, который развертывается клиентом.<br>Часы нормализованного использования определяются как часы использования, нормализованные с учетом количества ядер виртуальных машин: [количество ядер виртуальных машин] x [количество часов фактического использования]. Для виртуальных машин, используемых в режиме общего ядра (SHAREDCORE), в качестве множителя [количество ядер виртуальных машин] используется значение 1/6 (или 0,1666). | нормализедусаже |
| метередусаже | Лимитное использование | Совокупные единицы использования, потребляемые счетчиками, настроенными с предложением, которое развертывается клиентом. | метередусаже |
| равусаже | Фактическое потребление | Общее количество необработанных единиц использования, потребляемых ресурсом, который развертывается клиентом.<br>Часы использования необработанных данных определяются как время, в течение которого виртуальные машины выполняются в условиях единиц использования. | равусаже |
| Расширенная смета затрат (в валюте клиента) | Оценка расширенной оплаты в валюте клиента | Обозначает расходы, связанные с использованием. Столбец представляет собой произведение цены (CC) и объема использования. | естиматедекстендедчаржекк |
| Расширенная смета затрат (в валюте издателя) | Оценка расширенной оплаты в валюте выплаты | Обозначает расходы, связанные с использованием. Столбец — это продукт оценочной цены (PC) и объема использования. | Естиматедекстендед Чаржепк |
|||||
