...
4. Создание приложения Azure Active Directory (AD) в Azure. AppSource будет использовать это приложение для подготовки и отмены подготовки пользователя тестового диска в клиенте.
    ...
    10. Выберите категорию разрешения **приложения** , а затем — **User. ReadWrite. ALL**, **Directory. Read. ALL** и **Directory. ReadWrite. ALL** .
    ...
    12. В предупреждении сообщения выберите **Да**.
    ...
    13. Чтобы создать секрет для Azure AD App:
...
5. Добавьте роль субъекта-службы в приложение, чтобы разрешить приложению Azure AD удалять пользователей из клиента Azure.
...
6. Добавьте созданное ранее приложение Azure в качестве пользователя приложения в экземпляр CRM тестового диска.
    ...
    10. Кроме того, включите **действие от имени другого пользователя** .
    11. Назначьте пользователю приложения настраиваемую роль безопасности, созданную для тестового диска.
