...
### <a name="inbound-availability-alerting"></a>Входящее оповещение о доступности
Чтобы оповещать о доступности входящего трафика, можно создать два отдельных предупреждения, используя метрики доступности пути к данным и состояния проверки работоспособности. У клиентов могут быть различные сценарии, требующие определенной логики предупреждений, но приведенные ниже примеры будут полезны для большинства конфигураций.

С помощью доступности пути к данным можно запускать предупреждения при недоступности определенного правила балансировки нагрузки. Это предупреждение можно настроить, задав условие предупреждения для доступности пути к данным и разбиения по всем текущим значениям и будущим значениям для внешнего порта и внешнего IP-адреса. Если задать для логики предупреждения значение меньше или равное 0, это предупреждение будет запускаться каждый раз, когда любое правило балансировки нагрузки перестанет отвечать. Задайте гранулярность статистической обработки и частоту оценки в соответствии с требуемой оценкой. 

С состоянием проверки работоспособности можно оповещать, когда данный экземпляр внутреннего сервера не может ответить на пробу работоспособности в течение значительного промежутка времени. Настройте условие оповещения для использования метрики состояния проверки работоспособности и разбиения по внутреннему IP-адресу и порту внутреннего сервера. Это обеспечит отправку предупреждений отдельно для каждого отдельного экземпляра сервера, обслуживающего трафик по определенному порту. Используйте тип статистической обработки " **Среднее** " и установите пороговое значение в соответствии с частотой проверки экземпляра серверной части и тем, что вы считаете пороговым значением работоспособности. 

Можно также оповещать на уровне серверного пула, не прерывая ни одного измерения и не используя **средний** тип статистической обработки. Это позволит настроить правила оповещений, такие как предупреждение, если 50% членов серверного пула находятся в неработоспособном состоянии.

### <a name="outbound-availability-alerting"></a>Исходящее оповещение о доступности
Чтобы настроить для исходящей доступности, можно настроить два отдельных предупреждения с помощью счетчика подключений SNAT и используемых метрик портов SNAT.

Для обнаружения сбоев исходящих подключений настройте оповещение с помощью счетчика подключений SNAT и фильтрацию до состояния соединения = Failed. Используйте **Общий** агрегат. Затем его можно разделить на внутренние IP-адреса, настроенные на все текущие и будущие значения, чтобы получать предупреждения отдельно для каждого экземпляра сервера, на котором возникли неудачные подключения. Установите пороговое значение больше нуля или большее число, если вы хотите увидеть некоторые сбои исходящих подключений.

С помощью портов SNAT можно оповещать о повышении риска нехватки ресурсов SNAT и исходящих сбоев подключения. При использовании этого предупреждения убедитесь, что разбиение выполняется по внутреннему IP-адресу и протоколу, и используется **Среднее** агрегирование. Установите пороговое значение, превышающее процентное соотношение количества выделенных портов на экземпляр, который вы считали ненадежными. Например, можно настроить оповещение низкого уровня серьезности, когда экземпляр внутреннего сервера использует 75% выделенных портов и высокий уровень серьезности при использовании 90% или 100% выделенных портов.  
...
- Сведения об использовании [аналитики](./load-balancer-insights.md) для просмотра этих метрик, предварительно настроенных для Load Balancer
