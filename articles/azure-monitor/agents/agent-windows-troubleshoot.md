...
- Другим способом для обнаружения проблемы с подключением является запуск средства **тестклаудконнективити** . Средство устанавливается по умолчанию с агентом в папке *%Системрут%\програм Files\Microsoft Monitoring agent\agent.*. Перейдите в эту папку в командной строке с повышенными привилегиями и запустите инструмент. Средство возвращает результаты и особенности, на которых произошел сбой теста (например, если он был связан с определенным портом или URL-адресом, который был заблокирован). 
