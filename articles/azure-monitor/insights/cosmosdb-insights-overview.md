...
## <a name="view-from-an-azure-cosmos-db-resource"></a>Просмотр из Azure Cosmos DB ресурса

1. Найдите или выберите любую из существующих учетных записей Azure Cosmos DB.

:::image type="content" source="./media/cosmosdb-insights-overview/cosmosdb-search.png" alt-text="Выполните поиск по запросу Azure Cosmos DB." border="true":::

2. Перейдя к учетной записи Azure Cosmos DB, в разделе Мониторинг выберите **аналитика (Предварительная версия)** или **книги** , чтобы выполнить дальнейший анализ пропускной способности, запросов, хранилища, доступности, задержки, системы и управления учетными записями.

:::image type="content" source="./media/cosmosdb-insights-overview/cosmosdb-overview.png" alt-text="Обзор Cosmos DB Insights." border="true":::

### <a name="time-range"></a>Диапазон времени

По умолчанию в поле **диапазон времени** отображаются данные за **последние 24 часа**. Можно изменить диапазон времени, чтобы данные отображались в любом месте за последние 5 минут до последних семи дней. Селектор диапазона времени также включает в себя **Пользовательский** режим, позволяющий вводить даты начала и окончания для просмотра настраиваемого временного интервала на основе доступных данных для выбранной учетной записи.

:::image type="content" source="./media/cosmosdb-insights-overview/cosmosdb-time-range.png" alt-text="Cosmos DB диапазон времени." border="true":::

### <a name="insights-overview"></a>Обзор аналитических сведений

На вкладке **Обзор** представлены наиболее распространенные метрики для выбранной учетной записи Azure Cosmos DB, включая:

* Общее количество запросов
* Неудачные запросы (429s)
* Нормализованное потребление единиц запросов (max)
* Использование индекса & данных
* Метрики учетных записей Cosmos DB по коллекциям

**Всего запросов:** На этом графике представлено общее количество запросов для учетной записи с разбивкой по коду состояния. Единицы в нижней части графа являются суммой общего количества запросов за период.

:::image type="content" source="./media/cosmosdb-insights-overview/cosmosdb-total-requests.png" alt-text="Cosmos DB общего графа запросов." border="true":::

**Неудачные запросы (429s)**: на этом графике представлено представление запросов со сбоями с кодом состояния 429. Единицы в нижней части графа являются суммой общего количества неудачных запросов за период.

:::image type="content" source="./media/cosmosdb-insights-overview/cosmosdb-429.png" alt-text="Cosmos DB графа неудачных запросов." border="true":::

**Нормализованное потребление единиц запросов (max)**. Этот граф предоставляет максимальный процент между 0-100% нормализованных единиц потребления единицы измерения для указанного периода.

:::image type="content" source="./media/cosmosdb-insights-overview/cosmosdb-normalized-ru.png" alt-text="Cosmos DB нормализованное потребление единиц запросов." border="true":::
...
Книги сохраняются внутри группы ресурсов, либо в разделе **Мои отчеты**, который является доступным только для вас, либо в разделе **Общие отчеты**, доступном всем пользователям внутри группы ресурсов. После сохранения пользовательской книги перейдите в коллекцию книг и запустите эту книгу.
