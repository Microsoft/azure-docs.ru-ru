...
При этом обновлении данные производительности VM Insights хранятся в той же таблице *инсигхтсметрикс* , что и [аналитика контейнеров](../containers/container-insights-overview.md), что упрощает запрос двух наборов данных. Кроме того, вы можете хранить более разнообразные наборы данных, которые не могли бы храниться в ранее использованной таблице. 
...
Мы выпустили новое решение с именем Вминсигхтс, которое включает в себя больше возможностей для сбора данных, а также новое расположение для хранения этих данных в рабочей области Log Analytics. 

В прошлом мы включили решение ServiceMap в рабочую область и настроили счетчики производительности в рабочей области Log Analytics для отправки данных в таблицу *производительности* . Это новое решение отправляет данные в таблицу с именем *инсигхтсметрикс* , которая также используется контейнерной аналитикой. Эта схема таблицы позволяет сохранять больше метрик и наборов данных служб, несовместимых с форматом таблицы *производительности* .
...
## <a name="how-will-this-change-affect-my-bill"></a>Как это изменение повлияет на счет?
...
Мы получили огромное мнение от клиентов о наборе функций работоспособности виртуальной машины. Эта функция асигнификант интересна и повышена ее потенциальная возможность поддержки рабочих процессов мониторинга. Мы планируем внести ряд изменений, чтобы добавить функции и устранить полученные отзывы. 
...
Если вы являетесь клиентом, который использует функцию работоспособности и хотите использовать его для нового развертывания, свяжитесь с нами по vminsights@microsoft.com запросу.
