---
title: Настройка мониторинга в VM insights guest health с помощью правил сбора данных (предварительная версия)
description: Описывает, как изменить мониторинг по умолчанию в работоспособности гостевых систем VM Insights с помощью шаблонов диспетчер ресурсов.
ms.topic: conceptual
author: bwren
ms.author: bwren
ms.date: 10/15/2020
...
ms.locfileid: "102052149"
---
# <a name="configure-monitoring-in-vm-insights-guest-health-using-data-collection-rules-preview"></a>Настройка мониторинга в VM insights guest health с помощью правил сбора данных (предварительная версия)
[Работоспособность гостевых систем VM Insights](vminsights-health-overview.md) позволяет просматривать работоспособность виртуальной машины в соответствии с набором измерений производительности, которые вычисляются с регулярным интервалом. В этой статье описывается, как можно изменить мониторинг по умолчанию на нескольких виртуальных машинах с помощью правил сбора данных.


## <a name="monitors"></a>Мониторы
Состояние работоспособности виртуальной машины определяется [сводкой состояния работоспособности](vminsights-health-overview.md#health-rollup-policy) каждого из ее мониторов. Существует два типа мониторов для работоспособности гостевых систем VM Insights, как показано в следующей таблице.
...
Набор мониторов, используемых гостевой службой "аналитика виртуальной машины" и их конфигурация, нельзя изменить напрямую. Вы можете создавать [переопределения](#overrides) , когда изменяете поведение конфигурации по умолчанию. Переопределения определяются в правилах сбора данных. Можно создать несколько правил сбора данных, каждый из которых содержит несколько переопределений для достижения требуемой конфигурации мониторинга.
...
|:---|:---|:---|:---|:---|:---|:---|:---|:---|:---|
| загрузка ЦП;  | Верно | Неверно | Нет | \> 90%    | 60 с | 240 с | Min | 2 | 3 |
| Объем доступной памяти | Верно | Неверно | Нет | \< 100 МБ | 60 с | 240 с | Макс. | 2 | 3 |
| Файловая система      | Верно | Неверно | Нет | \< 100 МБ | 60 с | 120 с | Макс. | 1 | 1 |
