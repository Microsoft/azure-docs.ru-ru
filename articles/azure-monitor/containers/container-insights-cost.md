---
title: Мониторинг затрат для аналитики контейнеров | Документация Майкрософт
description: В этой статье описываются затраты на мониторинг метрик & данных инвентаризации, собираемых контейнерной аналитикой, чтобы помочь клиентам в управлении их использованием и связанными затратами.
ms.topic: conceptual
ms.date: 05/29/2020
...
ms.locfileid: "101728915"
---
# <a name="understand-monitoring-costs-for-container-insights"></a>Общие сведения о затратах на мониторинг для аналитики контейнеров

В этой статье содержатся рекомендации по использованию аналитики для контейнеров, которые помогут вам понять следующее:

* Оценка затрат перед включением этого анализа

* Как измерять затраты после включения аналитики контейнера для одного или нескольких контейнеров
...
Ниже приведена сводка по типам данных, собираемых из кластера Kubernetes с помощью аналитики контейнеров, которые влияют на затраты и могут быть настроены в зависимости от использования.
...
- [Сбор журналов диагностики](../../aks/view-control-plane-logs.md) журналов главного узла Kubernetes в кластере AKS для анализа данных журнала, созданных основными компонентами, такими как *KUBE-аписервер* и *KUBE-Controller-Manager*.

## <a name="what-is-collected-from-kubernetes-clusters"></a>Что собирается из кластеров Kubernetes

Контейнерная аналитика включает предопределенный набор метрик и собираемых элементов инвентаризации, которые записываются в качестве данных журнала в рабочую область Log Analytics. Все перечисленные ниже метрики собираются по умолчанию каждые одну минуту.
...
Дополнительные сведения о том, как с помощью данных, собранных в контейнере, см. в статье [Управление использованием и оценка затрат](../logs/manage-cost-storage.md).