---
title: Оповещения метрик из аналитики контейнера
description: В этой статье рассматриваются рекомендуемые оповещения метрик, доступные в общедоступной предварительной версии контейнера аналитика.
ms.topic: conceptual
ms.date: 10/28/2020
...
ms.locfileid: "101728898"
---
# <a name="recommended-metric-alerts-preview-from-container-insights"></a>Рекомендуемые оповещения метрик (Предварительная версия) из аналитики контейнера

Чтобы предупредить о проблемах с системными ресурсами, возникающих в пиковом спросе и выполнении практических мощностей, при работе с контейнерной аналитикой необходимо создать оповещение журнала на основе данных о производительности, хранящихся в журналах Azure Monitor. Служба "аналитика контейнеров" теперь включает в себя предварительно настроенные правила оповещений метрик для AKS и кластера Kubernetes с поддержкой дуги Azure, который находится в общедоступной предварительной версии.
...
Если вы не знакомы с Azure Monitor оповещениями, ознакомьтесь со статьей [Обзор оповещений в Microsoft Azure](../alerts/alerts-overview.md) перед началом работы. Дополнительные сведения об оповещениях метрик см. [в разделе оповещения метрик в Azure Monitor](../alerts/alerts-metric-overview.md).
...
Чтобы предупредить о важности, в контейнере аналитики включены следующие оповещения метрик для AKS и кластеров Kubernetes с поддержкой ARC.
...
В этом разделе описано, как включить оповещение метрик контейнера (Предварительная версия) из портал Azure.

1. Войдите на [портал Azure](https://portal.azure.com/).

2. Доступ к функции оповещения о метриках контейнеров (Предварительная версия) можно получить непосредственно из кластера AKS, выбрав **аналитику** в левой области в портал Azure.

3. На панели команд выберите **Рекомендуемые оповещения**.

    ![Параметр "Рекомендуемые оповещения" в контейнере "аналитика"](./media/container-insights-metric-alerts/command-bar-recommended-alerts.png)
...
Вы можете просматривать правила генерации оповещений в контейнере и управлять ими, изменять их пороговое значение или настраивать [группу действий](../alerts/action-groups.md) для кластера AKS. Хотя эти действия можно выполнять из портал Azure и Azure CLI, их также можно выполнять непосредственно из кластера AKS в контейнере аналитики.
