---
title: Мониторинг Kubernetes с помощью Container Insights | Документация Майкрософт
description: В этой статье описывается, как просмотреть и проанализировать производительность кластера Kubernetes с помощью аналитики контейнера.
ms.topic: conceptual
ms.date: 03/26/2020
...
ms.locfileid: "102124318"
---
# <a name="monitor-your-kubernetes-cluster-performance-with-container-insights"></a>Мониторинг производительности кластера Kubernetes с помощью аналитики контейнеров

С помощью аналитических данных можно использовать диаграммы производительности и состояние работоспособности для наблюдения за рабочей нагрузкой кластеров Kubernetes, размещенных в службе Kubernetes Azure (AKS), Azure Stack или другой среде с двух точек зрения. Вы можете отслеживать непосредственно из кластера или просматривать все кластеры в подписке с Azure Monitor. Просматривать экземпляры контейнеров Azure также можно при наблюдении за конкретным кластером AKS.
...
Дополнительные сведения о том, как включить аналитику контейнера, см. в статье [Встроенная аналитика контейнера](container-insights-onboard.md).
...
Основные отличия в наблюдении за кластером Windows Server с контейнерной аналитикой по сравнению с кластером Linux описаны [здесь](container-insights-overview.md#what-does-container-insights-provide) в обзорной статье.
...
* **Все** — Просмотр всех кластеров Kubernetes, размещенных в Azure, Azure Stack и локальных средах, подключенных к контейнеру аналитики.
...
* Неправильно **настроено:** аналитика контейнера не была правильно задана в указанной рабочей области.
...
Доступ к контейнеру аналитики доступен непосредственно из кластера AKS. для этого выберите кластер **Insights**  >   в левой области или при выборе кластера в представлении с несколькими кластерами. Сведения о кластере организованы в четыре перспективы:
...
Используйте клавиши со стрелками влево и вправо для перемещения по каждой точке данных на диаграмме. Используйте клавиши со стрелками вверх и вниз для прохода по линиям процентиля. Щелкните значок закрепления в правом верхнем углу любой диаграммы, чтобы закрепить выбранную диаграмму на последней просмотренной панели мониторинга Azure. На панели мониторинга вы можете изменить размер и положение диаграммы. Выбор диаграммы на панели мониторинга перенаправляет вас в контейнеры аналитики и загружает правильную область и представление.

Контейнерная аналитика также поддерживает Azure Monitor [Обозреватель метрик](../essentials/metrics-getting-started.md), позволяющий создавать собственные диаграммы графиков, сопоставлять и исследовать тенденции, а также закреплять их на панелях мониторинга. В обозревателе метрик можно также использовать критерии, заданные для визуализации метрик в качестве основания для [правила генерации оповещений на основе метрик](../alerts/alerts-metric.md).

## <a name="view-container-metrics-in-metrics-explorer"></a>Просмотр метрик контейнера в обозревателе метрик

В обозревателе метрик можно просматривать агрегированные метрики использования узла и Pod из аналитики контейнера. В следующей таблице представлены сведения, которые помогут понять, как использовать диаграммы метрик для визуализации метрик контейнера.
...
Книги объединяют текст, заносить в журнал запросы, метрики и параметры в расширенные Интерактивные отчеты, позволяющие анализировать производительность кластера. Описание книг, доступных для аналитики контейнеров, см. [в книге в контейнере аналитики](../insights/container-insights-reports.md) .


## <a name="next-steps"></a>Дальнейшие действия

- Ознакомьтесь с разработкой [оповещений о производительности с помощью Container Insights](./container-insights-log-alerts.md) , чтобы узнать, как создавать оповещения для высокой загрузки ЦП и памяти для поддержки DevOps или рабочих процессов и процедур.
