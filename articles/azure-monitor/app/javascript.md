...
|------|-------------|---------|
| instrumentationKey | **Обязательно**<br>Ключ инструментирования, полученный из портал Azure. | строка<br/>null |
| accountId | Необязательный идентификатор учетной записи, если приложение группирует пользователей в учетные записи. Без пробелов, запятых, точек с запятой, знаков равенства или вертикальных линий | строка<br/>null |
| сессионреневалмс | Сеанс регистрируется, если пользователь неактивен в течение этого периода времени в миллисекундах. | NUMERIC<br/>1800000<br/>(30 минут) |
| сессионекспиратионмс | Сеанс заносится в журнал, если он продолжался в течение этого времени (в миллисекундах). | NUMERIC<br/>86400000<br/>(24 часа) |
| максбатчсизеинбитес | Максимальный размер пакета телеметрии. Если пакет превышает это ограничение, он немедленно отправляется и запускается новый пакет | NUMERIC<br/>10000 |
| максбатчинтервал | Время пакетной передачи данных перед отправкой (в миллисекундах) | NUMERIC<br/>15 000 |
| отключить&#8203;Ексцептионтраккинг | Если значение — true, исключения не собираются. | Логическое<br/> false |
| дисаблетелеметри | Если значение — true, данные телеметрии не собираются и не отправляются. | Логическое<br/>false |
| енабледебуг | Если значение — true, **внутренние** данные отладки создаются как исключение, **а не** заносятся в журнал, независимо от параметров ведения журнала пакета SDK. Значение по умолчанию — false. <br>**_Примечание._** Включение этого параметра приведет к потере телеметрии при возникновении внутренней ошибки. Это может быть полезно для быстрого выявления проблем с конфигурацией или использованием пакета SDK. Если вы не хотите терять данные телеметрии во время отладки, попробуйте использовать `consoleLoggingLevel` или `telemetryLoggingLevel` вместо `enableDebug` . | Логическое<br/>false |
| логгинглевелконсоле | Записывает в консоль **внутренние** ошибки Application Insights. <br>0: выкл., <br>1: только критические ошибки, <br>2: все (ошибки & предупреждения) | NUMERIC<br/> 0 |
| логгинглевелтелеметри | Отправляет **внутренние** ошибки Application Insights как данные телеметрии. <br>0: выкл., <br>1: только критические ошибки, <br>2: все (ошибки & предупреждения) | NUMERIC<br/> 1 |
| диагностиклогинтервал | внутрикластерных Интервал опроса (в мс) для внутренней очереди ведения журнала | NUMERIC<br/> 10000 |
| самплингперцентаже | Процент событий, которые будут отправлены. Значение по умолчанию — 100, то есть отправляются все события. Установите этот параметр, если хотите сохранить ограничения на данные для крупномасштабных приложений. | NUMERIC<br/>100 |
| аутотраккпажевиситтиме | Если значение равно true, то время просмотра предыдущей инструментированной страницы отсылается и отправляется в виде данных телеметрии, а для текущего pageview запускается новый таймер. | Логическое<br/>false |
| дисаблеажакстраккинг | Если значение равно true, вызовы AJAX не собираются. | Логическое<br/> false |
| дисаблефетчтраккинг | Если значение — true, запросы на выборку не собираются.|Логическое<br/>Да |
| оверридепажевиевдуратион | Если значение — true, поведение по умолчанию trackPageView изменяется для записи окончания интервала длительности просмотра страницы при вызове trackPageView. Если для trackPageView не задано значение false и пользовательская длительность не предоставлена, то производительность просмотра страницы вычисляется с помощью API времени навигации. |Логическое<br/>
| максажакскаллспервиев | По умолчанию 500 — определяет, сколько вызовов AJAX будет отслеживаться каждым представлением страницы. Задайте значение-1, чтобы отслеживать все неограниченные вызовы AJAX на странице. | NUMERIC<br/> 500 |
| дисабледаталоссаналисис | Если значение равно false, внутренние буферы отправителя телеметрии будут проверяться при запуске для элементов, которые еще не были отправлены. | Логическое<br/> Да |
| отключить&#8203;Коррелатионхеадерс | Если значение равно false, пакет SDK добавит два заголовка ("Request-ID" и "Request-context") во все запросы зависимости, чтобы сопоставить их с соответствующими запросами на стороне сервера. | Логическое<br/> false |
| Коррелатионхеадер&#8203;Ексклудеддомаинс | Отключение заголовков корреляции для конкретных доменов | string[]<br/>неопределенный |
| Коррелатионхеадер&#8203;Ексклудепаттернс | Отключение заголовков корреляции с помощью регулярных выражений | Regex []<br/>неопределенный |
| Домены&#8203;Коррелатионхеадер | Включение заголовков корреляции для конкретных доменов | string[]<br/>неопределенный |
| Дисаблефлуш&#8203;Онбефореунлоад | Если значение равно true, метод Flush не будет вызываться при триггерах событий Онбефореунлоад | Логическое<br/> false |
| енаблесессионсторажебуффер | Если значение — true, буфер со всеми неотправленными данными телеметрии хранится в хранилище сеанса. Буфер восстанавливается при загрузке страницы | Логическое<br />Да |
| кукиекфг | По умолчанию используется параметр "использование cookie". см. параметры [икукиекфгконфиг](#icookiemgrconfig) для полных значений по умолчанию. | [икукиекфгконфиг](#icookiemgrconfig)<br>(С 2.6.0)<br/>неопределенный |
| ~~искукиеуседисаблед~~<br>дисаблекукиесусаже | Если значение — true, пакет SDK не будет хранить или считывать данные из файлов cookie. Обратите внимание, что это отключает файлы cookie пользователя и сеанса, а также подготавливает к просмотру лопасти использования и возможности. Искукиеуседисабле является нерекомендуемым в пользу Дисаблекукиесусаже, если оба предоставлены для Дисаблекукиесусаже приоритета.<br>(Начиная с v 2.6.0) И если `cookieCfg.enabled` также определено, он имеет приоритет над этими значениями, использование файлов cookie можно снова включить после инициализации через Core. жеткукиемгр (). сетенаблед (true). | псевдоним для [`cookieCfg.enabled`](#icookiemgrconfig)<br>false |
| кукиедомаин | Пользовательский домен cookie. Это полезно, если требуется предоставить общий доступ к Application Insights файлам cookie между поддоменами.<br>(Начиная с v 2.6.0) Если `cookieCfg.domain` определено, это значение будет иметь приоритет над этим значением. | псевдоним для [`cookieCfg.domain`](#icookiemgrconfig)<br>null |
| кукиепас | Путь к пользовательскому файлу cookie. Это полезно, если вы хотите предоставить общий доступ к Application Insights файлам cookie за шлюзом приложений.<br>Если `cookieCfg.path` определено, это значение будет иметь приоритет над этим значением. | псевдоним для [`cookieCfg.path`](#icookiemgrconfig)<br>(С 2.6.0)<br/>null |
| исретридисаблед | Если задано значение false, повторите попытку в 206 (частичное успешное завершение), 408 (timeout), 429 (слишком много запросов), 500 (внутренняя ошибка сервера), 503 (служба недоступна) и 0 (вне сети, только если обнаружено) | Логическое<br/>false |
| иссторажеуседисаблед | Если задано значение true, пакет SDK не будет хранить или считывать данные из локального хранилища и в хранилище сеанса. | Логическое<br/> false |
| исбеаконапидисаблед | Если значение равно false, пакет SDK будет отсылать все данные телеметрии с помощью [API маяка](https://www.w3.org/TR/beacon) . | Логическое<br/>Да |
| онунлоаддисаблебеакон | Когда вкладка закрывается, пакет SDK будет передавать все оставшиеся данные телеметрии с помощью [API маяка](https://www.w3.org/TR/beacon) . | Логическое<br/> false |
| сдкекстенсион | Задает имя расширения пакета SDK. Допускаются только буквы. Имя расширения добавляется в качестве префикса к тегу "AI. internal. Сдкверсион" (например, "ext_javascript: 2.0.0"). | строка<br/> null |
| Исбровсерлинк&#8203;Траккинженаблед | Если значение — true, пакет SDK будет отслеживанием всех запросов на [компоновку браузера](/aspnet/core/client-side/using-browserlink) . | Логическое<br/>false |
| appId | AppId используется для корреляции между зависимостями AJAX, происходящими на стороне клиента, с запросами на стороне сервера. Если API маяка включен, он не может использоваться автоматически, но может быть задан вручную в конфигурации. |строка<br/> null |
| включить&#8203;Корскоррелатион | Если значение — true, пакет SDK добавит два заголовка ("Request-ID" и "Request-context") во все запросы CORS для корреляции исходящих зависимостей AJAX с соответствующими запросами на стороне сервера. | Логическое<br/>false |
| namePrefix | Необязательное значение, которое будет использоваться в качестве постфикса имени для localStorage и имени файла cookie. | строка<br/>неопределенный |
| Включение отслеживания&#8203;для&#8203;автоматического маршрута | Автоматическая регистрация изменений маршрута в одностраничных приложениях (SPA). Если значение — true, каждое изменение маршрута будет отсылать новый pageview Application Insights. Изменения хэш-маршрута ( `example.com/foo#bar` ) также записываются в виде новых просмотров страниц.| Логическое<br/>false |
| Енаблерекуест&#8203;Хеадертраккинг | Если значение равно true, то отслеживание заголовков запроса AJAX & FETCH. | Логическое<br/> false |
| Енаблереспонсе&#8203;Хеадертраккинг | Если значение равно true, то отслеживание заголовков ответа на запрос AJAX & FETCH. | Логическое<br/> false |
| дистрибутедтраЦингмоде | Задает режим распределенной трассировки. Если задан режим AI_AND_W3C или W3C, заголовки контекста трассировки W3C (трацепарент/трацестате) будут формироваться и включаться во все исходящие запросы. AI_AND_W3C предоставляется для обеспечения обратной совместимости с любыми устаревшими службами, Application Insights инструментированные. См. пример [здесь](./correlation.md#enable-w3c-distributed-tracing-support-for-web-apps).| `DistributedTracingModes`или<br/>NUMERIC<br/>(Начиная с v 2.6.0) `DistributedTracingModes.AI_AND_W3C`<br />(v 2.5.11 или более ранняя версия) `DistributedTracingModes.AI` |
| включить&#8203;Ажаксеррорстатустекст | Значение true показывает, что текст данных об ошибке ответа содержится в событии зависимостей при неудачных запросах AJAX. | Логическое<br/> false |
| включить&#8203;Ажаксперфтраккинг |Флаг, позволяющий выполнять поиск и включение дополнительного окна браузера. время производительности в сообщаемых `ajax` (XHR и выборке) метриках. | Логическое<br/> false |
| Максажаксперф&#8203;Лукупаттемптс | Максимальное количество попыток поиска в окне. время производительности (если доступно). это необходимо, так как не все браузеры заполняют окно. производительность перед созданием отчета о конце запроса XHR и о запросах на выборку добавляется после завершения.| NUMERIC<br/> 3 |
| ажаксперфлукупделай | Время ожидания перед повторной попыткой поиска окон. `ajax` время, необходимое для запроса, равно времени в миллисекундах и передается непосредственно в setTimeout (). | NUMERIC<br/> 25 мс |
| Енаблеунхандлед&#8203;Промисережектион отслеживание&#8203; | Если задано значение true, необработанные отклонения в обещании будут собираются и сообщаются как ошибка JavaScript. Если Дисабликсцептионтраккинг имеет значение true (не записывать исключения), значение конфигурации будет проигнорировано, а необработанные отклонения обещаний не будут выводиться. | Логическое<br/> false |
| отключить проверку&#8203;InstrumentationKey&#8203; | Если значение — true, проверка ключа инструментирования пропускается. | Логическое<br/>false |
| енаблеперфмгр | Если параметр включен (true), то будет создан локальный Перфевентс для кода, инструментированного для создания Перфевентс (через вспомогательную функцию Доперф ()). Это можно использовать для обнаружения проблем с производительностью в пакете SDK на основе использования или при необходимости в собственном инструментированном коде. [Дополнительные сведения можно найти в базовой документации](https://github.com/microsoft/ApplicationInsights-JS/blob/master/docs/PerformanceMonitoring.md). С 2.5.7 v | Логическое<br/>false |
| перфевтссендалл | Если _енаблеперфмгр_ включен и [иперфманажер](https://github.com/microsoft/ApplicationInsights-JS/blob/master/shared/AppInsightsCore/src/JavaScriptSDK.Interfaces/IPerfManager.ts) запускает [инотификатионманажер](https://github.com/microsoft/ApplicationInsights-JS/blob/master/shared/AppInsightsCore/src/JavaScriptSDK.Interfaces/INotificationManager.ts). перфевент () этот флаг определяет, инициируется ли событие (и отправляется всем прослушивателям) для всех событий (true) или только для событий "Parent" (значение &lt; по умолчанию — false &gt; ).<br />Родительский [иперфевент](https://github.com/microsoft/ApplicationInsights-JS/blob/master/shared/AppInsightsCore/src/JavaScriptSDK.Interfaces/IPerfEvent.ts) — это событие, при котором другие иперфевент по-прежнему выполняются в момент создания этого события, а свойство _Parent_ не равно null или не определено. С 2.5.7 v |  Логическое<br />false |
| идленгс | Определяет длину по умолчанию, используемую для создания новых случайных значений сеанса и идентификаторов пользователей. По умолчанию установлено значение 22, а предыдущее значение по умолчанию — 5 (v 2.5.8 или меньше). Если необходимо, чтобы Предыдущая максимальная длина была равна 5. |  NUMERIC<br />22 |

## <a name="cookie-handling"></a>Обработка файлов cookie

В версии 2.6.0 Управление файлами cookie теперь доступно непосредственно из экземпляра и может быть отключено и снова включено после инициализации.

Если эта функция отключена во время инициализации с помощью `disableCookiesUsage` `cookieCfg.enabled` конфигураций или, теперь можно повторно включить с помощью функции [икукиемгр](https://github.com/microsoft/ApplicationInsights-JS/blob/master/shared/AppInsightsCore/src/JavaScriptSDK.Interfaces/ICookieMgr.ts) `setEnabled` .

Управление файлами cookie на основе экземпляра также заменяет предыдущие глобальные функции кореутилс `disableCookies()` , `setCookie(...)` `getCookie(...)` и `deleteCookie(...)` . Кроме того, чтобы воспользоваться преимуществами расширений встряхиваний в виде дерева, которые также появились в составе версии 2.6.0, больше не следует использовать глобальные функции.

### <a name="icookiemgrconfig"></a>икукиемгрконфиг

Конфигурация файлов cookie для управления файлами cookie на основе экземпляра, добавленная в версии 2.6.0.

| Имя | Описание | Тип и значение по умолчанию |
|------|-------------|------------------|
| Включено | Логическое значение, указывающее, включено ли использование файлов cookie с помощью пакета SDK текущим экземпляром. Если значение равно false, экземпляр пакета SDK, инициализированный этой конфигурацией, не будет хранить или считывать данные из файлов cookie. | Логическое<br/> Да |
| домен | Пользовательский домен cookie. Это полезно, если требуется предоставить общий доступ к Application Insights файлам cookie между поддоменами. Если значение не указано, использует значение из корневого `cookieDomain` значения. | строка<br/>null |
| path | Указывает путь, используемый для файла cookie, если он не указан, будет использовать любое значение из корневого `cookiePath` значения. | строка <br/> / |
| getCookie | Функция для выборки именованного значения cookie, если она не указана, будет использовать внутренний синтаксический анализ или кэширование файлов cookie. | `(name: string) => string` <br/> null |
| setCookie | Функция для задания именованного файла cookie с указанным значением, который вызывается только при добавлении или обновлении файла cookie. | `(name: string, value: string) => void` <br/> null |
| делкукие | Функция для удаления именованного файла cookie с указанным значением, отделенного от setCookie, чтобы избежать необходимости анализировать значение, чтобы определить, добавлен ли файл cookie или удален. Если он не указан, будет использоваться внутренний синтаксический анализ или кэширование файлов cookie. | `(name: string, value: string) => void` <br/> null |

### <a name="simplified-usage-of-new-instance-cookie-manager"></a>Упрощенное использование диспетчера файлов cookie нового экземпляра

- appInsights. [жеткукиемгр ()](https://github.com/microsoft/ApplicationInsights-JS/blob/master/shared/AppInsightsCore/src/JavaScriptSDK.Interfaces/ICookieMgr.ts). сетенаблед (true/false);
- appInsights. [жеткукиемгр ()](https://github.com/microsoft/ApplicationInsights-JS/blob/master/shared/AppInsightsCore/src/JavaScriptSDK.Interfaces/ICookieMgr.ts). Set ("микукие", "The% 20encoded% 20value");
- appInsights. [жеткукиемгр ()](https://github.com/microsoft/ApplicationInsights-JS/blob/master/shared/AppInsightsCore/src/JavaScriptSDK.Interfaces/ICookieMgr.ts). Get ("микукие");
- appInsights. [жеткукиемгр ()](https://github.com/microsoft/ApplicationInsights-JS/blob/master/shared/AppInsightsCore/src/JavaScriptSDK.Interfaces/ICookieMgr.ts). Del ("микукие");
