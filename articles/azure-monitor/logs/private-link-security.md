...
Так как некоторые конечные точки Azure Monitor являются глобальными, невозможно создать подключение к частной ссылке для определенного компонента или рабочей области. Вместо этого, когда вы настраиваете частную ссылку на один Application Insights компонент или Log Analytics рабочую область, записи DNS обновляются для **всех** Application Insights компонентов. Любая попытка приема или запроса компонента будет проходить через частную ссылку и, возможно, завершится ошибкой. В отношении Log Analytics, конечные точки приема и конфигурации относятся к конкретной рабочей области, а это означает, что Настройка частного канала будет применяться только к указанным рабочим областям. Прием и настройка других рабочих областей будут направлены к общедоступным Log Analytics конечным точкам по умолчанию.
...
> [!NOTE]
> Для завершения: после настройки подключения частного канала к одному ресурсу оно применяется к ресурсам Azure Monitor по сети. Для Application Insightsных ресурсов это ALL или Nothing. Это означает, что следует добавить все ресурсы Application Insights в сети в АМПЛС или ни один из них.
> 
> Чтобы справиться с рисками утечка данных, мы рекомендуем добавить все Application Insights и Log Analytics ресурсы в АМПЛС и блокировать исходящий трафик в сети как можно больше.
...
> [!NOTE]
> Каждая из этих зон сопоставляет конкретные конечные точки Azure Monitor с частными IP-адресами из пула IP-адресов виртуальной сети. Ниже приведены примеры использования IP-адресов, показанных на следующих рисунках. Вместо этого в вашей конфигурации должны отображаться частные IP-адреса из вашей сети.
...
Вам потребуется добавить ресурсы, содержащие отслеживаемые рабочие нагрузки, в Приватный канал. Например, см. раздел [использование частных конечных точек для веб-приложения Azure](../../app-service/networking/private-endpoint.md).
...
>
> Диагностика на уровне кода (профилировщик или отладчик) требуют [предоставления собственной учетной записи хранения](../app/profiler-bring-your-own-storage.md) для поддержки частной ссылки.

### <a name="handling-the-all-or-nothing-nature-of-private-links"></a>Обработка частных ссылок "все-или ничего"
Как описано в разделах [Планирование настройки частной связи](#planning-your-private-link-setup), Настройка частной ссылки, даже для одного ресурса, влияет на все ресурсы Azure Monitor в этих сетях, а также в других сетях, использующих одну и ту же службу DNS. Такое поведение может усложнить процесс адаптации. Следуйте приведенным ниже рекомендациям.
