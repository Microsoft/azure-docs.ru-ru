...
В Log Analytics выделенных кластерах используется [модель ценообразования](./logs-dedicated-clusters.md#cluster-pricing-model) резервирования емкости, начиная с 1000 ГБ в день.
...
Выполните процедуру, показанную в [статье выделенные кластеры](./logs-dedicated-clusters.md#creating-a-cluster). 
...
Выполните процедуру, показанную в [статье выделенные кластеры](./logs-dedicated-clusters.md#link-a-workspace-to-cluster).
...
Customer-Managed ключ предоставляется в выделенном кластере, и эти операции упоминаются в [статье выделенный кластер](./logs-dedicated-clusters.md#change-cluster-properties) .
...
- Сведения о [правильном проектировании рабочих областей log Analytics](./design-logs-deployment.md)