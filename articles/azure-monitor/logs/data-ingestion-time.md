...
Задержка — это время, в течение которого данные создаются в отслеживаемой системе, и время, в течение которого они становятся доступными для анализа в Azure Monitor. Типичная задержка приема данных журнала — от 20 секунд до 3 минут. Однако конкретная задержка для конкретных данных зависит от ряда факторов, описанных ниже.
...
- Время агента — время обнаружения события, его получение, а затем Отправка в журнал Azure Monitor точка приема журналов как запись журнала. В большинстве случаев этот процесс обрабатывается агентом. В сети может быть добавлена дополнительная задержка.
...
Условия сети могут негативно повлиять на задержку данных, чтобы достичь Azure Monitor точки приема журналов.

### <a name="azure-activity-logs-resource-logs-and-metrics"></a>Журналы действий Azure, журналы ресурсов и метрики
Данные Azure добавляют дополнительное время, чтобы стать доступным на Azure Monitor точки приема журналов для обработки:

- Журналы ресурсов обычно добавляются в течение 30-90 секунд, в зависимости от службы Azure. Некоторые службы Azure (в частности, база данных SQL Azure и виртуальная сеть Azure) в настоящее время сообщают о журналах каждые 5 минут. Работа выполняется для дальнейшего улучшения. Ознакомьтесь с [запросом ниже](#checking-ingestion-time), чтобы изучить задержку в своей среде.
- Для сбора метрик платформы Azure в Azure Monitor в журнал можно экспортировать дополнительные 3 минуты.
- Если используется устаревшая интеграция, данные журнала действий могут занять дополнительные 10-15 минут. Рекомендуется использовать параметры диагностики уровня подписки для приема журналов действий в журналы Azure Monitor, что приводит к дополнительной задержке около 30 секунд.
...
После получения доступа к точке приема данные занимают еще 30-60 секунд для выполнения запросов.
