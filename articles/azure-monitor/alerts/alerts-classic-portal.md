...
description: Узнайте, как использовать портал Azure, CLI или PowerShell для создания, просмотра и управления правилами оповещений для классических метрик.
author: yanivlavi
ms.author: yalavi
ms.topic: conceptual
ms.date: 02/14/2021
...
Классические оповещения метрик в Azure Monitor позволяют получать уведомления, когда одна из метрик достигает порогового значения. Классические оповещения метрик — это более старая функциональность, которая позволяет оповещать только метрики без измерений. Существует более новая функциональность, называемая "оповещения метрик", которая имеет улучшенные функциональные возможности по сравнению с классическими оповещениями метрики. Подробнее о новой функциональности оповещений метрик можно узнать в [обзоре оповещений метрик](./alerts-metric-overview.md). В этой статье описывается создание, просмотр и управление правилами оповещений для классических метрик с помощью портал Azure, Azure CLI и PowerShell.
