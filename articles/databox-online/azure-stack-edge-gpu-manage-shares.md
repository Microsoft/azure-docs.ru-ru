...
1. В портал Azure перейдите к ресурсу Azure Stackного периметра и перейдите в раздел **облачного хранилища > общие папки**. На панели команд щелкните **+ Добавление общего файлового ресурса**.
    ...
    ![Выбор команды "Добавление общего файлового ресурса"](media/azure-stack-edge-gpu-manage-shares/add-share-1.png)
...
6. Этот шаг зависит от того, создается ли общая папка типа SMB или NFS.
    - **При создании общей папки SMB** в поле **Все привилегированные локальные пользователи** выберите **Создать** или **Использовать существующий**. При создании локального пользователя укажите **имя пользователя** и **пароль**, а затем подтвердите пароль. Таким образом вы предоставите разрешения локальному пользователю. Назначив разрешения здесь, вы сможете изменять их в проводнике.
        ...
        ![Добавление общей папки SMB](media/azure-stack-edge-gpu-manage-shares/add-smb-share.png)
    ...
    - **При создании общей папки типа NFS** необходимо предоставить **IP-адреса разрешенных клиентов**, которые могут получать доступ к ресурсу.
        ...
        ![Добавление общей папки типа NFS](media/azure-stack-edge-gpu-manage-shares/add-nfs-share.png)
...
1. В портал Azure перейдите к ресурсу Azure Stackного периметра и перейдите в раздел **облачного хранилища > общие папки**. На панели команд щелкните **+ Добавление общего файлового ресурса**.
    ...
    ![Выберите Добавить общий ресурс 2](media/azure-stack-edge-gpu-manage-shares/add-local-share-1.png)
...
7. Нажмите кнопку **создания**. 
    ...
    ![Создание локальной общей папки](media/azure-stack-edge-gpu-manage-shares/add-local-share-2.png)
    ...
    ![Просмотр обновлений колонки "Общие папки"](media/azure-stack-edge-gpu-manage-shares/add-local-share-3.png)
    ...
    ![Просмотр общих сведений локальной общей папки](media/azure-stack-edge-gpu-manage-shares/add-local-share-4.png)
...
1. В портал Azure перейдите к ресурсу Azure Stackного периметра и перейдите в раздел **облачного хранилища > общие папки**. В списке общих папок выберите ту, которую необходимо подключить. Для выбранной общей папки в столбце **Используется для вычислений** отобразится состояние **Отключено**.
    ...
    ![Выбор общей папки](media/azure-stack-edge-gpu-manage-shares/mount-share-1.png)

2. Выберите **Подключить**.

    ![Выбор параметра "Подключить"](media/azure-stack-edge-gpu-manage-shares/mount-share-2.png)

3. При появлении запроса на подтверждение нажмите кнопку **Да**. В этом случае общая папка будет подключена.

    ![Подтверждение подключения](media/azure-stack-edge-gpu-manage-shares/mount-share-3.png)
...
4. После подключения перейдите к списку общих папок. Вы увидите, что в столбце **Используется для вычислений** в качестве состояния общей папки отображается **Включено**.
    ...
    ![Общая папка подключена](media/azure-stack-edge-gpu-manage-shares/mount-share-4.png)
...
5. Снова выберите общую папку, чтобы просмотреть ее локальную точку подключения. Модуль пограничных вычислений использует эту локальную точку подключения для общей папки.
    ...
    ![Локальная точка подключения для общей папки](media/azure-stack-edge-gpu-manage-shares/mount-share-5.png)
...
1. В портал Azure перейдите к ресурсу Azure Stackного периметра и перейдите в раздел **облачного хранилища > общие папки**. В списке общих папок выберите папку, которую требуется отключить. Убедитесь, что она не используется модулем. Если же общая папка используется модулем, это приведет к возникновению проблем с модулем.
    ...
    ![Выберите общий ресурс 2](media/azure-stack-edge-gpu-manage-shares/unmount-share-1.png)

2.  Выберите **Отключить**.

    ![Выбор параметра "Отключить"](media/azure-stack-edge-gpu-manage-shares/unmount-share-2.png)

3. При появлении запроса на подтверждение нажмите кнопку **Да**. В этом случае общая папка будет отключена.

    ![Подтверждение отключения](media/azure-stack-edge-gpu-manage-shares/unmount-share-3.png)
...
4. После отключения перейдите к списку общих папок. Вы увидите, что в столбце **Используется для вычислений** в качестве состояния общей папки отображается **Отключено**.
    ...
    ![Общая папка отключена](media/azure-stack-edge-gpu-manage-shares/unmount-share-4.png)
...
1. В списке общих папок выберите и щелкните папку, которую требуется удалить.
    ...
    ![Выберите общий ресурс 3](media/azure-stack-edge-gpu-manage-shares/delete-share-1.png)

2. Щелкните **Удалить**.

    ![Нажмите кнопку "Удалить"](media/azure-stack-edge-gpu-manage-shares/delete-share-2.png)

3. При запросе подтверждения нажмите кнопку **Да**.

    ![Подтверждение удаления](media/azure-stack-edge-gpu-manage-shares/delete-share-3.png)
...
1.  На портале Azure перейдите к разделу **Общие папки**. Выберите и щелкните общую папку, которую требуется обновить.
    ...
    ![Выберите общий доступ 4](media/azure-stack-edge-gpu-manage-shares/refresh-share-1.png)

2.  Нажмите кнопку **Обновить**. 

    ![Нажатие кнопки обновления](media/azure-stack-edge-gpu-manage-shares/refresh-share-2.png)
 
3.  При запросе подтверждения нажмите кнопку **Да**. Запустится задание обновления содержимого локальной общей папки.

    ![Подтверждение обновления](media/azure-stack-edge-gpu-manage-shares/refresh-share-3.png)
...
![Обновленная метка времени](media/azure-stack-edge-gpu-manage-shares/refresh-share-4.png)
...
2. Перейдите в **контейнеры** и выберите **+ контейнер** , чтобы создать контейнер. Присвойте этому контейнеру имя *невконтаинер*. Установите для **общего уровня доступа** значение контейнер.
    ...
    ![Автоматическая синхронизация закрепленных файлов 1](media/azure-stack-edge-gpu-manage-shares/image-1.png)
...
3. Выберите имя контейнера и задайте следующие метаданные:  
    ...
    ![Автоматическая синхронизация для закрепленных файлов 2](media/azure-stack-edge-gpu-manage-shares/image-2.png)
...
4. Создайте на устройстве новую общую папку. Сопоставьте его с закрепленным контейнером, выбрав параметр существующий контейнер. Пометка общего ресурса как доступного только для чтения. Создайте нового пользователя и укажите имя пользователя и соответствующий пароль для этой общей папки.  
    ...
    ![Автоматическая синхронизация закрепленных файлов 3](media/azure-stack-edge-gpu-manage-shares/image-3.png)
...
6. Выберите **Обновить данные** в портал Azure, чтобы устройство скачало политику закрепления для этого контейнера хранилища Azure.  
    ...
    ![Автоматическая синхронизация закрепленных файлов 4](media/azure-stack-edge-gpu-manage-shares/image-4.png)
...
1. В своем ресурсе перейдите к разделу **Обзор**. Из списка общих папок выберите и щелкните ту, которая связана с учетной записью хранения и которую необходимо синхронизировать.
    ...
    ![Выбор общей папки с соответствующей учетной записью хранения](media/azure-stack-edge-gpu-manage-shares/sync-storage-key-1.png)

2. Щелкните **Sync storage key** (Синхронизация ключа хранилища). Нажмите кнопку **ОК** в запросе на подтверждение.

     ![Выбор синхронизации ключа хранилища](media/azure-stack-edge-gpu-manage-shares/sync-storage-key-2.png)
