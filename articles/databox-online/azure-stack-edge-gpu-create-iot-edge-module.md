...
![Как работает модуль Azure IoT Edge на Azure Stack пограничных Pro](./media/azure-stack-edge-gpu-create-iot-edge-module/how-module-works-1.png)
...
- Устройство Azure Stack пограничной Pro, на котором работает.
    ...
    Дополнительные сведения см. в подразделе [Настройка вычислений](azure-stack-edge-j-series-deploy-configure-compute.md#configure-compute) для Azure Stack пограничных Pro.<!--Update link?-->
...
3. Сделайте следующее:
   ...
   6. Для номера SKU выберите вариант **Базовый**.
      ...
      ![Создание реестра контейнеров](./media/azure-stack-edge-gpu-create-iot-edge-module/create-container-registry-1.png)
 
4. Нажмите кнопку **создания**.
5. После создания реестра контейнеров просмотрите его и выберите **Ключи доступа**.

    ![Получение ключей доступа](./media/azure-stack-edge-gpu-create-iot-edge-module/get-access-keys-1.png)
...
3. В палитре команд введите и выполните команду **Azure IoT Edge: New IoT Edge Solution** (Azure IoT Edge: создать решение IoT Edge). В палитре команд укажите следующие сведения для создания решения:
    ...
    2. Введите имя своего решения или примите имя по умолчанию **EdgeSolution**.
        ...
        ![Создание решения 1](./media/azure-stack-edge-gpu-create-iot-edge-module/create-new-solution-1.png)
    ...
    4. Замените имя модуля, предложенное по умолчанию, любым именем на ваш выбор. В нашем примере это **FileCopyModule**.
        ...
        ![Создание решения 2](./media/azure-stack-edge-gpu-create-iot-edge-module/create-new-solution-2.png)
    ...
    5. В качестве репозитория образов первого модуля укажите Реестр контейнеров Azure, который вы создали в предыдущем разделе. Замените **localhost:5000** скопированным значением имени входа на сервер.
        ...
        ![Создание решения 3](./media/azure-stack-edge-gpu-create-iot-edge-module/create-new-solution-3.png)

4. Выберите **Файл > Открыть папку**.

    ![Создание решения 4](./media/azure-stack-edge-gpu-create-iot-edge-module/create-new-solution-4.png)

5. Найдите и выберите папку **EdgeSolution**, которую вы создали ранее. Окно VS Code загружает рабочую область решения IoT Edge, в которой размещены пять элементов верхнего уровня. Вы не измените `.vscode` папку, файл **gitignore** , **env** и deployment.template.jsв * * в этой статье.
    ...
    ![Создание решения 5](./media/azure-stack-edge-gpu-create-iot-edge-module/create-new-solution-5.png)
...
2. Войдите в DOCKER, введя следующую команду в интегрированном терминале.
    ...
    ![Сборка и отправка решения IoT Edge](./media/azure-stack-edge-gpu-create-iot-edge-module/build-iot-edge-solution-1.png)
...
3. Предоставив учетные данные, вы сможете отправить образ модуля в Реестр контейнеров Azure. В обозревателе VS Code щелкните правой кнопкой мыши файл **module.json** и выберите действие **Build and Push IoT Edge solution** (Собрать и отправить решение IoT Edge).
    ...
    ![Сборка и отправка IoT Edge решение 2](./media/azure-stack-edge-gpu-create-iot-edge-module/build-iot-edge-solution-2.png)
    ...
    ![Выбор платформы](./media/azure-stack-edge-gpu-create-iot-edge-module/select-platform.png)
