...
- Служба архивации дисков Azure поддерживает PowerShell. В настоящее время Azure CLI не поддерживается.
...
- Вы можете удалить экземпляр резервной копии, который будет прекращать резервное копирование, а также удалит все данные резервных копий. В настоящее время невозможно отключить резервное копирование, так как параметр " **прерывать резервное копирование" и "хранить данные резервной копии** " не поддерживается.
