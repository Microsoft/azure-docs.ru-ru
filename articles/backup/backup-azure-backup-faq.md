...
### <a name="can-i-move-a-subscription-that-contains-a-recovery-services-vault-to-a-different-tenant"></a>Можно ли переместить подписку, содержащую хранилище служб восстановления, в другой клиент?

Да. Убедитесь, что вы выполняете следующие действия: 

>[!IMPORTANT]
>После перемещения подписки убедитесь, что выполнены следующие действия.<ul><li>Если хранилище использует CMK (управляемые пользователем ключи), необходимо обновить хранилище. Это позволяет хранилищу повторно создавать и перенастраивать удостоверение управляемого хранилища и CMK (которое будет размещаться в новом клиенте), в противном случае операция резервного копирования или восстановления завершится ошибкой.</li><li>Необходимо перенастроить разрешения RBAC в подписке, так как невозможно переместить существующие разрешения.</li></ul>
