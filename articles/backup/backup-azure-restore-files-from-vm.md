...
3. В меню Панель мониторинга архивации выберите пункт **Восстановление файлов**.
    ...
    ![Выберите восстановление файлов](./media/backup-azure-restore-files-from-vm/vm-backup-menu-file-recovery-button.png)32
...
Невозможно запустить исполняемый сценарий на виртуальной машине с любой из следующих характеристик: Выбор альтернативного компьютера
...
Вы не можете запустить скачанный исполняемый файл на той же резервной виртуальной машине, если на резервной виртуальной машине есть дисковые пространства Windows. Выберите альтернативный компьютер.
...
См. статью требования для восстановления файлов из резервных копий виртуальных машин с большим диском.<br>
[ОС Windows](#for-backed-up-vms-with-large-disks-windows)<br>
[ОС Linux](#for-backed-up-vms-with-large-disks-linux)

После выбора правильного компьютера для выполнения сценария ILR убедитесь, что он соответствует [требованиям к операционной системе](#step-3-os-requirements-to-successfully-run-the-script) и [требованиям к доступу](#step-4-access-requirements-to-successfully-run-the-script). 
...
Кроме того, убедитесь, что у вас есть [правильный компьютер для выполнения сценария ILR](#step-2-ensure-the-machine-meets-the-requirements-before-executing-the-script) и соответствует [требованиям к доступу](#step-4-access-requirements-to-successfully-run-the-script).
...
Кроме того, убедитесь, что у вас есть [правильный компьютер для выполнения сценария ILR](#step-2-ensure-the-machine-meets-the-requirements-before-executing-the-script) и соответствует [требованиям операционной системы](#step-3-os-requirements-to-successfully-run-the-script).
...
После соблюдения всех требований, перечисленных в [шаге 2](#step-2-ensure-the-machine-meets-the-requirements-before-executing-the-script), [Шаг 3](#step-3-os-requirements-to-successfully-run-the-script) и [4](#step-4-access-requirements-to-successfully-run-the-script), скопируйте сценарий из скачанного расположения (обычно это папка downloads), [чтобы узнать, как создать и скачать скрипт](#step-1-generate-and-download-script-to-browse-and-recover-files). Щелкните исполняемый файл правой кнопкой мыши и запустите его с учетными данными администратора. При появлении запроса введите пароль или вставьте его из памяти и нажмите клавишу ВВОД. После ввода пароля сценарии подключаются к точке восстановления.
...
После выполнения всех требований, перечисленных в [шаге 2](#step-2-ensure-the-machine-meets-the-requirements-before-executing-the-script), [шага 3](#step-3-os-requirements-to-successfully-run-the-script) и [4](#step-4-access-requirements-to-successfully-run-the-script), создайте скрипт Python для компьютеров Linux. См. [Шаг 1, чтобы узнать, как создать и скачать скрипт](#step-1-generate-and-download-script-to-browse-and-recover-files). Скачайте сценарий и скопируйте его на соответствующий или совместимый сервер Linux. Вам может потребоваться изменить разрешения, чтобы выполнить его с помощью ```chmod +x <python file name>```. Затем запустите файл Python с помощью ```./<python file name>```.
