...
При сбое предупреждения не возникает несколько исключений. Их можно сформулировать следующим образом.
...
## <a name="azure-monitor-alerts-for-azure-backup-preview"></a>Azure Monitor оповещений для Azure Backup (Предварительная версия)

Azure Backup также предоставляет оповещения с помощью Azure Monitor, чтобы обеспечить единообразное взаимодействие пользователей с управлением оповещениями в разных службах Azure, включая резервное копирование. С помощью Azure Monitor предупреждений можно перенаправлять оповещения в любой канал уведомлений, поддерживаемый Azure Backup например электронной почтой, ITSM, веб-перехватчиком, приложением логики и т. д.

Сейчас эта функция доступна для баз данных Azure для PostgreSQL Server, больших двоичных объектов Azure и управляемых дисков Azure. Оповещения создаются для следующих сценариев, и к ним можно получить доступ, перейдя в хранилище службы архивации и щелкнув пункт меню " **предупреждения** ":

- Удаление данных резервного копирования
- Сбой резервного копирования (для получения оповещений о сбое резервного копирования необходимо зарегистрировать флаг АФЕК с именем **енаблеазуребаккупжобфаилуреалертстоазуремонитор** на портале предварительной версии).
- Сбой восстановления (чтобы получить оповещения о сбое восстановления, необходимо зарегистрировать флаг АФЕК с именем **енаблеазуребаккупжобфаилуреалертстоазуремонитор** на портале предварительной версии).

Дополнительные сведения о Azure Monitor оповещениях см. [в разделе Обзор оповещений в Azure](https://docs.microsoft.com/azure/azure-monitor/alerts/alerts-overview).
