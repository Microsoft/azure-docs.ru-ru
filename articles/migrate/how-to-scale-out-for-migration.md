...
1. Щелкните **Обнаружение**  >  **виртуальные машины, которые виртуализованы?** 
1. Выберите **Да с помощью VMware vSphere гипервизора.**
...
2. Нажмите кнопку **создать ключ** , чтобы начать создание необходимых ресурсов Azure. Не закрывайте страницу Discover во время создания ресурсов.
...
> 2. Выполните следующую команду, чтобы создать хэш ZIP-файла.
    ...
    - Пример использования для общедоступного облака: ```C:\>Get-FileHash -Path .\AzureMigrateInstaller-VMware-Public-Scaleout.zip -Algorithm SHA256 ```.
> 3. Скачайте последнюю версию установщика масштабируемых устройств с портала, если вычисленное значение хэша не соответствует этой строке: 1E6B6E3EE8B2A800818B925F5DA67EF7874DAD87E32847120B32F3E21F5960F9
...
- В диспетчере конфигурации выберите **Set up prerequisites** (Настройка необходимых компонентов) и выполните следующее.
   - **Возможность подключения**. Устройство проверит наличие доступа к Интернету у сервера. Если на сервере используется прокси-сервер:
     1. Щелкните **Setup Proxy (настроить прокси-сервер** ), чтобы указать адрес прокси-сервера (в форме http://ProxyIPAddress или http://ProxyFQDN) прослушиваемом порте).
