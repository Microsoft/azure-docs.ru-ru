---
title: Оценка готовности серверов VMware к миграции на виртуальные машины Azure с помощью службы "Миграция Azure"
description: Узнайте, как оценить готовность серверов VMware к миграции на виртуальные машины Azure с помощью службы "Миграция Azure".
author: rashi-ms
ms.author: rajosh
ms.manager: abhemraj
ms.topic: tutorial
...
В этой статье содержатся сведения о том, как оценивать обнаруженные серверы из среды VMware при подготовке к миграции в на виртуальные машины Azure с помощью средства обнаружения и оценки службы "Миграция Azure".
...
Прежде чем приступить к работе с этим учебником по оценке серверов для миграции на виртуальные машины Azure, убедитесь, что обнаружили серверы, которые хотите оценить:
...
2. В **средстве "Обнаружение и оценка" службы "Миграция Azure"** щелкните элемент **Оценка** и выберите пункт **Виртуальная машина Azure**.

    ![Расположение кнопки оценки](./media/tutorial-assess-vmware-azure-vm/assess-servers.png)

3. На вкладке **Оценка серверов** > **Тип оценки**
4. В **источнике обнаружения**:

    - Если вы обнаружили серверы с помощью устройства, выберите элемент **Servers discovered from Azure Migrate appliance** (Серверы, обнаруженные с устройства Миграции Azure).
    - Если вы обнаружили серверы с помощью импортированного CSV-файла, выберите элемент **Imported servers** (Импортированные серверы). 
...
1. В разделе **Windows, Linux and SQL Server (Windows, Linux и SQL Server)**  > **Azure Migrate: Discovery and assessment (Миграция Azure: обнаружение и оценка)** щелкните номер рядом с элементом **Azure VM assessment** (Оценка виртуальных машин Azure).
