...
- **Предыдущая версия**. Если вы используете предыдущую версию службы "Миграция Azure" (поддерживалась только оценка локальных виртуальных машин VMware), теперь следует использовать текущую версию. Проекты предыдущих версий в этой статье называются классическими проектами. В феврале 2024 г. будет прекращена поддержка классической версии Миграции Azure. После февраля 2024 г. классическая версия Миграции Azure больше не будет поддерживаться, а метаданные инвентаризации в классических проектах будут удалены. Если вам по-прежнему нужно использовать классические проекты службы "Миграция Azure", которые созданы в предыдущей версии, далее указано то, что можно сделать, а что — нет:
...
2. Вы можете создать рабочую область или присоединить имеющуюся.
  ...
  - При подключении имеющейся рабочей области вы можете выбрать нужную из всех доступных рабочих областей в той же подписке, что и проект миграции. В списке указаны только рабочие области, созданные в регионе, в котором [поддерживается решение "Сопоставление служб"](../azure-monitor/vm/vminsights-configure-workspace.md#supported-regions). Чтобы подключить рабочую область, необходим доступ к ней с ролью "Читатель".
...
Настроив рабочую область, скачайте и установите агенты на каждом локальном компьютере, который нужно оценить. Кроме того, если у вас есть компьютеры без подключения к Интернету, на них необходимо скачать и установить [шлюз Log Analytics](../azure-monitor/agents/gateway.md).
...
Вы можете установить агент из командной строки или автоматически, например с помощью Configuration Manager. Дополнительные сведения об использовании этих методов для установки агента MMA см. в разделе [Установка и настройка агента](../azure-monitor/agents/log-analytics-agent.md#installation-options).
...
- Узнайте больше о [поддержке агента зависимостей](../azure-monitor/vm/vminsights-enable-overview.md#supported-operating-systems) для ОС Windows и Linux.
