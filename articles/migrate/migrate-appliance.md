...
Сведений о папке на сервер | ((Folder)container).ChildEntity.Type
Сведений центра обработки данных на сервер | ((Datacenter)container).VmFolder
...
Сведения об узле на сервер | ((HostSystem)container).VM

### <a name="performance-data"></a>Данные о производительности

Ниже приведены данные о производительности, собираемые устройством для сервера, работающего в VMware, и отправляемые в Azure.
...
Пропускная способность чтения с дисков (МБ/с) | virtualDisk.read.average | Вычисление размера диска, стоимости хранилища, размера сервера
Пропускная способность записи на диски (МБ/с) | virtualDisk.write.average | Вычисление размера диска, стоимости хранилища, размера сервера
Число операций чтения с диска в секунду | virtualDisk.numberReadAveraged.average | Вычисление размера диска, стоимости хранилища, размера сервера
Число операций записи на диск в секунду | virtualDisk.numberWriteAveraged.average  | Вычисление размера диска, стоимости хранилища, размера сервера
...
### <a name="installed-software-inventory"></a>Инвентаризация установленного программного обеспечения

Устройство собирает данные инвентаризации установленного программного обеспечения на серверах.

#### <a name="windows-server-software-inventory-data"></a>Данные инвентаризации программного обеспечения Windows Server

Ниже приведены данные инвентаризации программного обеспечения, собираемые устройством из каждого Windows Server, обнаруженного в среде VMware.
...
#### <a name="linux-server-software-inventory-data"></a>Данные инвентаризации серверного программного обеспечения Linux

Ниже приведены данные инвентаризации программного обеспечения, собираемые устройством с каждого сервера Linux, обнаруженного в среде VMware. В зависимости от операционной системы сервера выполняется одна или несколько команд.
...
Тип сервера (GEN 1 или 2) | Msvm_VirtualSystemSettingData | VirtualSystemSubType
...
Максимальный объем памяти, который может использоваться сервером | Msvm_MemorySettingData | Ограничение
...
Сервер Hyper-V динамическая память | Текущая нагрузка (%)<br/> Физическая память, доступная для гостя (МБ) | Рекомендуемый размер и стоимость сервера
Виртуальное устройство хранения Hyper-V | Чтение (байт в секунду) | Вычисление размера диска, стоимости хранилища, размера сервера
Виртуальное устройство хранения Hyper-V | Запись (байт в секунду) | Вычисление размера диска, стоимости хранилища, размера сервера
Виртуальный сетевой адаптер Hyper-V | Получение (байт в секунду) | Вычисление размера сервера
Виртуальный сетевой адаптер Hyper-V | Отправка (байт в секунду) | Вычисление размера сервера

- Загрузка ЦП — это сумма всех использований для всех виртуальных процессоров, подключенных к серверу.
...
- На устройстве в окне   >  **программы и компоненты** панели управления.
