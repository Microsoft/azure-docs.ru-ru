---
title: Настройка анализа зависимостей без агента в службе "миграция Azure"
description: Настройка анализа зависимостей без агента в службе "миграция Azure".
author: vikram1988
ms.author: vibansa
ms.manager: abhemraj
ms.topic: how-to
...
ms.locfileid: "104786589"
---
# <a name="analyze-server-dependencies-agentless"></a>Анализ зависимостей сервера (без агента)

В этой статье описывается, как настроить анализ зависимостей без агента с помощью службы "миграция Azure": средство обнаружения и оценки. [Анализ зависимостей](concepts-dependency-visualization.md) помогает определить и понять зависимости между серверами для оценки и миграции в Azure.

> [!IMPORTANT]
> Анализ зависимостей без агента сейчас находится на этапе предварительной версии для серверов, работающих в среде VMware, обнаруженных с помощью средства "миграция Azure: обнаружение и оценка".
...
- Вам потребуется учетная запись пользователя, чтобы служба "миграция Azure" могла получить доступ к серверу для получения данных о зависимостях. [Сведения](migrate-support-matrix-vmware.md#dependency-analysis-requirements-agentless) о требованиях к учетной записи для серверов Windows и Linux.
...
2. Выберите подписку, в которой был создан проект. 
...
1. Получите список обнаруженных серверов в проекте с помощью следующих команд. В приведенном ниже примере имя проекта — Фабрикамдемопрожект, а группа ресурсов, к которой он принадлежит, — Фабрикамдеморг. Список серверов будет сохранен в FabrikamDemo_VMs.csv
...
2. Войдите в Azure, выполнив приведенные ниже инструкции. 
    ...
    - Выберите подписку, в которой был создан проект.
