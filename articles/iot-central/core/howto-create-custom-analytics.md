...
## <a name="configure-export-in-iot-central-and-create-a-new-destination"></a>Настройка экспорта в IoT Central и создание нового назначения
...
1. Перейдите на страницу **Экспорт данных** и выберите **+ Новый экспорт**.
1. Перед завершением первого окна выберите **создать назначение**.

Окно будет выглядеть следующим образом.  

:::image type="content" source="media/howto-create-custom-analytics/data-export-2.png" alt-text="изображение конфигурации назначения для экспорта данных.":::

3. Введите следующие значения.

| Параметр | Значение |
| ------- | ----- |
| Имя конечной папки | Имя назначения |
| Тип назначения | Центры событий Azure |
| Строка подключения| Строка подключения концентратора событий, которая была заметка ранее. | 
| Концентратор событий| Имя концентратора событий|

4. Нажмите кнопку **создать** для завершения.

5. Для настройки экспорта используйте следующие параметры:

    | Параметр | Значение |
    | ------- | ----- |
    | Введите имя экспорта | евенсубекспорт |
    | Активировано | Включено |
    | Данные| Выбор телеметрии | 
    | Места назначения| Создайте назначение, как показано ниже, для экспорта, а затем выберите его в раскрывающемся меню назначения. |

:::image type="content" source="media/howto-create-custom-analytics/data-export-1.png" alt-text="Снимок экрана с конфигурацией назначения для экспорта данных.":::
