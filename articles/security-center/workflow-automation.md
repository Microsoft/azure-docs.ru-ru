...
Каждая программа безопасности включает в себя несколько рабочих процессов для реагирования на инциденты. Это могут быть, например, уведомление релевантных участников, запуск процесса управления изменениями и выполнение конкретных шагов для исправления. Специалисты по безопасности рекомендуют автоматизировать как можно больше этапов таких процедур. Автоматизация сокращает затраты. Автоматизация помогает сократить издержки и повысить безопасность, так как обеспечивает быстрое и согласованное выполнение этапов процесса, причем в соответствии с заранее определенными требованиями.

В этой статье описывается функция автоматизации рабочих процессов в центре безопасности Azure. Эта функция может активировать Logic Apps оповещений системы безопасности, рекомендаций и изменений соответствия нормативным требованиям. Например, вам может потребоваться, чтобы Центр безопасности отправлял определенному пользователю сообщение электронной почты при неудачной оценке соответствия. Вы также узнаете, как создавать Logic Apps с помощью [Azure Logic Apps](../logic-apps/logic-apps-overview.md).
...
1. Чтобы определить новый рабочий процесс, щелкните **Добавить автоматизацию рабочего процесса**. 
    ...
    1. Название и описание для автоматизации.
...
1. Определив приложение логики, вернитесь на панель определения автоматизации рабочего процесса (Add workflow automation). Нажмите кнопку **Обновить** , чтобы убедиться, что новое приложение логики доступно для выбора.
