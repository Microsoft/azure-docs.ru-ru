...
1. Добавьте пул адресов VPN-клиента.
   ...
   ```
2. Отправьте [корневой сертификат](../vpn-gateway/vpn-gateway-howto-point-to-site-rm-ps.md#Certificates) VPN в Azure для VPN-шлюза. В этом примере предполагается, что корневой сертификат хранится на локальном компьютере, где выполняются следующие командлеты PowerShell и вы используете PowerShell локально. Также можно передать сертификат с помощью портал Azure.
