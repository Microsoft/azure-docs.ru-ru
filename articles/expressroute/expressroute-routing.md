...
* Протокол
    * Для интерфейсов маршрутизации необходимо зарезервировать подсеть/125 или две подсети/126.
    * В качестве используемых подсетей можно использовать либо частные, либо общедоступные IP-адреса.
    * Подсети не должны конфликтовать с диапазоном, зарезервированным клиентом для использования в облаке Майкрософт.
    * Если используется подсеть /125, ее можно разбить на две подсети /126. 
      * Первая подсеть/126 используется для первичной ссылки, а вторая — для вторичной.
...
Мы поддерживаем до 4000 префиксов IPv4 и 100 IPv6, объявленных для нас через частный пиринг Azure. Это можно увеличить до 10 000 префиксов IPv4, если надстройка ExpressRoute Premium включена. Мы принимаем до 200 префиксов на сеанс BGP для общедоступного пиринга Azure и пиринга Майкрософт. 
