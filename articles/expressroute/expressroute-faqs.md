...
### <a name="if-i-pay-for-an-expressroute-circuit-of-a-given-bandwidth-does-the-private-connection-i-purchase-from-my-network-service-provider-have-to-be-the-same-speed"></a>Если я платите за канал ExpressRoute заданной пропускной способности, делает частное подключение, которое я приобретает от поставщика сетевых услуг, иметь одинаковую скорость?

Нет. Вы можете приобрести частное подключение от поставщика услуг. Однако ваше подключение к Azure ограничено пропускной способностью приобретенного вами канала ExpressRoute.
...
Существует не более 1000 префиксов, объявленных для одного подключения ExpressRoute или через пиринг виртуальных сетей с помощью транзитного шлюза. Например, если имеется 999 адресных пространств в одной виртуальной сети, подключенной к каналу ExpressRoute, все 999 этих префиксов будут объявлены в локальной среде. Кроме того, если включена виртуальная сеть, разрешающая транзит шлюза с 1 адресным пространством и 500-лучевой виртуальных сетей с параметром "разрешить удаленный шлюз", то виртуальная сеть, развернутая с помощью шлюза, будет объявлять 501 в локальной среде.
...
### <a name="how-do-i-connect-my-virtual-networks-to-an-expressroute-circuit"></a>Разделы справки подключить виртуальные сети к каналу ExpressRoute?
...
### <a name="can-i-set-up-a-site-to-site-vpn-connection-to-my-virtual-network-in-conjunction-with-expressroute"></a>Можно ли настроить VPN-подключение типа "сеть — сеть" к виртуальной сети в сочетании с ExpressRoute?
...
### <a name="how-do-i-enable-routing-between-my-site-to-site-vpn-connection-and-my-expressroute"></a>Разделы справки включить маршрутизацию между VPN-подключением типа "сеть — сеть" и ExpressRoute?

Если вы хотите включить маршрутизацию между ветвью, подключенной к Експрессауте, и ветвь, подключенная к VPN-подключению типа "сеть — сеть", необходимо настроить [сервер маршрутизации Azure](../route-server/expressroute-vpn-support.md).
...
### <a name="i-received-a-notification-about-maintenance-on-my-expressroute-circuit-what-is-the-technical-impact-of-this-maintenance"></a>Я получил уведомление об обслуживании в канале ExpressRoute. Что такое техническое влияние на обслуживание?

При работе с каналом в [режиме "активный — активный](https://docs.microsoft.com/azure/expressroute/designing-for-high-availability-with-expressroute#active-active-connections)" вы должны столкнуться с минимальным влиянием на обслуживание. Мы выполняем обслуживание основного и дополнительного подключений канала отдельно. Запланированное обслуживание обычно выполняется за пределами рабочего времени в часовом поясе расположения пиринга, и вы не можете выбрать время обслуживания.

### <a name="i-received-a-notification-about-a-software-upgrade-or-maintenance-on-my-expressroute-gateway-what-is-the-technical-impact-of-this-maintenance"></a>Я получил уведомление об обновлении или обслуживании программного обеспечения на шлюзе ExpressRoute. Что такое техническое влияние на обслуживание?

Во время обновления или обслуживания шлюза вы должны столкнуться с минимальными последствиями. Шлюз ExpressRoute состоит из нескольких экземпляров, и во время обновления экземпляры переводятся в автономный режим по отдельности. Хотя это может привести к тому, что шлюз временно поддерживает меньшую пропускную способность сети для виртуальной сети, сам шлюз не будет испытывать простои.
