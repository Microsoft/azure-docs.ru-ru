...
Azure PowerShell [командлет Get-азсубскриптион](/powershell/module/Az.Accounts/Get-AzSubscription) `TenantId` по умолчанию отобразит для управляющего клиента. Можно использовать `HomeTenantId` `ManagedByTenantIds` атрибуты и для каждой подписки, позволяя определить, принадлежит ли возвращаемая Подписка к управляемому клиенту или к управляющему клиенту.
...
- Управляйте гибридными кластерами Kubernetes в масштабе. в [Azure Arc включена Kubernetes (Предварительная версия)](../../azure-arc/kubernetes/overview.md):
  - [Управление кластерами Kubernetes, подключенными](../../azure-arc/kubernetes/quickstart-connect-cluster.md) к делегированным подпискам и (или) группам ресурсов в Azure
  - [Использование гитопс](../../azure-arc/kubernetes/tutorial-use-gitops-connected-cluster.md) для подключенных кластеров
...
- Просмотр данных для всех делегированных ресурсов клиента в [центре архивации](../../backup/backup-center-overview.md)
...
- Развертывание определений политик и назначений политик в нескольких клиентах
...
- Обратите внимание, что Просмотр сведений о соответствии для несоответствующих ресурсов в клиентских клиентах в настоящее время не поддерживается
