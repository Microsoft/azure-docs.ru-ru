...
Эта статья о миграции является одним из нескольких ключевых слов NFS и Синхронизация файлов Azure. Проверьте, относится ли эта статья к вашему сценарию.

> [!div class="checklist"]
> * Источник данных: подключенное к сети хранилище (NAS)
> * Маршрут миграции: сервер Linux с SAMBA &rArr; Windows Server 2012 R2 или более поздней &rArr; синхронизации с файловыми ресурсами Azure
...
Если вы не используете Samba на сервере Linux и хотите перенести папки в гибридное развертывание на Windows Server, вместо Robocopy можно использовать средства копирования Linux. Помните о возможностях средства копирования. Ознакомьтесь с [разделом основные](storage-files-migration-overview.md#migration-basics) сведения о миграции в статье Обзор миграции, чтобы узнать, что искать в средстве копирования.
...
* Подготавливайте или добавляйте непосредственно подключенное хранилище (DAS). Запоминающее устройство, подключаемое к сети (NAS), не поддерживается.
  ...
  Объем предоставляемого хранилища может быть меньше, чем вы используете на сервере Linux Samba, если вы используете функцию распределения по [уровням облака](storage-sync-cloud-tiering-overview.md) синхронизация файлов Azure. 

Объем хранилища, который вы подготавливаете, может быть меньше, чем вы используете на сервере Linux Samba. При выборе этой конфигурации необходимо также использовать функцию "Синхронизация файлов Azure" для [облачного уровня](storage-sync-cloud-tiering-overview.md) . Однако при копировании файлов из более крупного серверного пространства Linux Samba на более короткий том Windows Server на более позднем этапе потребуется работать с пакетами:
  ...
  3. Если на томе создано больше свободного пространства, перейдите к следующему пакету файлов. Также можно ознакомиться с командой Robocopy в разделе "предстоящие средства [Robocopy](#phase-7-robocopy) " для использования нового `/LFSM` коммутатора. Использование `/LFSM` может значительно упростить задания Robocopy, но оно несовместимо с некоторыми другими коммутаторами Robocopy, которые могут зависеть от.
...
Возможно, средство Robocopy перемещает файлы быстрее, чем вы можете синхронизировать с облаком и уровнем локально, что приведет к нехватке места на локальном диске. После этого средство Robocopy завершится ошибкой. Рекомендуется работать с общими ресурсами в последовательности, которая не позволяет решить проблему. Например, рекомендуется не начинать задания Robocopy для всех общих ресурсов одновременно. Или рассмотрите возможность перемещения общих ресурсов, которые соответствуют текущему объему свободного пространства на экземпляре Windows Server. Если задание Robocopy завершается неудачно, вы всегда можете повторно выполнить команду, если вы используете следующий параметр Mirror/очистки:
