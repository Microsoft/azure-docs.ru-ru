...
Эта статья о миграции является одним из нескольких ключевых слов NAS и Синхронизация файлов Azure. Проверьте, относится ли эта статья к вашему сценарию.

> [!div class="checklist"]
> * Источник данных: подключенное к сети хранилище (NAS)
> * Маршрут миграции. &rArr; сервер NAS: &rArr; Отправка и синхронизация с файловыми ресурсами Azure
...
Целью является перемещение общих папок на устройстве NAS на сервер Windows. Затем используйте Синхронизация файлов Azure для гибридного облачного развертывания. Как правило, миграция должна быть выполнена таким образом, чтобы гуаранти целостность рабочих данных и их доступность во время миграции. Последний требует, чтобы время простоя не превышало минимального, так что оно может поместиться в или только немного больше, чем обычные периоды обслуживания.
...
* Подготавливаете или добавляйте непосредственно подключенное хранилище (DAS по сравнению с NAS, которое не поддерживается).
    ...
    Объем предоставляемого хранилища может быть меньше, чем вы используете в настоящее время на устройстве NAS. При выборе этой конфигурации необходимо также использовать функцию "Синхронизация файлов Azure" для [облачного уровня](storage-sync-cloud-tiering-overview.md) .
