...
> [!Important]  
> Вы можете внести изменения в пространство имен любой облачной конечной точки или конечной точки сервера в группе синхронизации и синхронизировать файлы с другими конечными точками в группе синхронизации. Если вы вносите изменения непосредственно в облачную конечную точку (общий файловый ресурс Azure), эти изменения сначала должны быть определены заданием обнаружения изменений службы "Синхронизация файлов Azure". Задание обнаружения изменений инициируется для облачной конечной точки каждые 24 часа. Дополнительные сведения см. в статье [Часто задаваемые вопросы о службе файлов Azure](storage-files-faq.md#afs-change-detection).

### <a name="consider-the-count-of-storage-sync-services-needed"></a>Рассмотрите число необходимых служб синхронизации хранилища.
В предыдущем разделе обсуждается основной ресурс для настройки Синхронизация файлов Azure: *Служба синхронизации хранилища*. Сервер Windows Server может быть зарегистрирован только в одной службе синхронизации хранилища. Поэтому часто рекомендуется развертывать только одну службу синхронизации хранилища и регистрировать все серверы, на которых она работает. 

Создавайте несколько служб синхронизации хранилища только в том случае, если у вас есть:
* отдельные наборы серверов, которые не должны обмениваться данными друг с другом. В этом случае необходимо разработать систему, чтобы исключить определенные наборы серверов для синхронизации с общей папкой Azure, которая уже используется как облачная конечная точка в группе синхронизации в другой службе синхронизации хранилища. Другой способ увидеть, что серверы Windows, зарегистрированные в другой службе синхронизации хранилища, не могут синхронизироваться с одной и той же общей папкой Azure.
* требуется больше зарегистрированных серверов или групп синхронизации, чем может поддерживать одна служба синхронизации хранилища. Дополнительные сведения см. в [Синхронизация файлов Azure целевые показатели масштабирования](storage-files-scale-targets.md#azure-file-sync-scale-targets) .

## <a name="plan-for-balanced-sync-topologies"></a>Планирование топологий с сбалансированной синхронизацией
Перед развертыванием ресурсов важно спланировать то, что будет синхронизироваться на локальном сервере, где находится файловый ресурс Azure. Создание плана поможет определить, сколько учетных записей хранения, файловых ресурсов Azure и ресурсов синхронизации потребуется. Эти рекомендации по-прежнему актуальны, даже если ваши данные в настоящее время не находятся на сервере Windows Server или на сервере, который требуется использовать в долгосрочной перспективе. [Раздел Migration (миграция](#migration) ) поможет определить соответствующие пути переноса для вашей ситуации.
...
- Для текущих заданий оптимизации дедупликации распределение по уровням в облаке с применением политики дат будет отложено параметром дедупликации [MinimumFileAgeDays](/powershell/module/deduplication/set-dedupvolume?view=win10-ps&preserve-view=true), если файл еще не распределен по уровням. 
...
Дополнительные сведения см. в разделе [Синхронизация файлов Azure метрики производительности](storage-files-scale-targets.md#azure-file-sync-performance-metrics) и [целевые показатели масштабирования синхронизация файлов Azure](storage-files-scale-targets.md#azure-file-sync-scale-targets) .
...
Если у вас уже есть файловый сервер Windows 2012 R2 или более поздней версии, Синхронизация файлов Azure можно установить непосредственно на месте без необходимости перемещения данных на новый сервер. Если вы планируете выполнить миграцию на новый файловый сервер Windows в рамках внедрения Синхронизация файлов Azure или если ваши данные в настоящее время находятся в подключенном к сети хранилище (NAS), существует несколько возможных подходов к переносу, чтобы использовать Синхронизация файлов Azure с этими данными. Выбор подхода перехода зависит от того, где находятся данные. 

Ознакомьтесь со статьей [Синхронизация файлов Azure и Обзор миграции в общую папку Azure](storage-files-migration-overview.md) , где можно найти подробные рекомендации по вашему сценарию.
...
> [!WARNING]
> Параметр Robocopy/B не поддерживается в Синхронизация файлов Azure. Использование ключа Robocopy/B с конечной точкой сервера Синхронизация файлов Azure, так как источник может привести к повреждению файла.
