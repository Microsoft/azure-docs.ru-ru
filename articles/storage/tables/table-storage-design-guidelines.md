...
* ***Укажите в запросах как PartitionKey, так и RowKey.** _ _Point запросы * такие, как, являются наиболее эффективными запросами службы таблиц.  
...
* ***Используйте значения составного ключа.** _ Допустимые ключи: _ *PartitionKey** и **RowKey**. Например, значения составного ключа можно использовать для включения альтернативных путей доступа с ключами к сущностям.  
