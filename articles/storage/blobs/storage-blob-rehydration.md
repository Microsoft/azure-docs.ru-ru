...
Восстановление большого двоичного объекта не влияет на его `Last-Modified` время. С помощью функции [управления жизненным циклом](storage-lifecycle-management-concepts.md) можно создать сценарий, в котором восстанавливается большой двоичный объект, а затем политика управления жизненным циклом перемещает большой двоичный объект обратно в архив, так как `Last-Modified` это время выходит за пределы порогового значения, установленного для политики. Чтобы избежать этого сценария, используйте метод *[Копировать архивный большой двоичный объект в веб-уровень](#copy-an-archived-blob-to-an-online-tier)* . Метод Copy создает новый экземпляр большого двоичного объекта с обновленным `Last-Modified` временем и не запускает политику управления жизненным циклом.
