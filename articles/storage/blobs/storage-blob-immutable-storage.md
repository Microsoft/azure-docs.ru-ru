...
- **Настройка уровня контейнера**: на уровне контейнера пользователи могут настроить временные политики периода удержания и теги юридического удержания. Пользователи могут создавать и блокировать временные политики удержания, расширять период хранения, устанавливать и очищать удержания по юридическим причинам и т. д. с помощью простых настроек уровня контейнера. Эти политики применяются для всех имеющихся и новых больших двоичных объектов в контейнере. Для учетной записи с поддержкой службы HNS эти политики также применяются ко всем каталогам в контейнере.
...
> [!IMPORTANT]
> Политика хранения на основе времени должна быть *заблокирована* , чтобы большой двоичный объект находился в совместимом неизменяемом состоянии (с записью и удалением) для 17A-4 (f) и других нормативных требований. Корпорация Майкрософт рекомендует заблокировать политику в разумное время, обычно менее 24 часов. Начальное состояние примененной политики хранения на основе времени *разблокируется*, что позволяет протестировать функцию и внести изменения в политику перед ее блокировкой. Хотя *незаблокированное* состояние обеспечивает защиту от неизменности, использование *незаблокированного* состояния для любых целей, отличных от краткосрочных, не рекомендуется.
>
> После блокировки политики хранения, основанной на времени, невозможно удалить политику и разрешить не более пяти увеличений срока действия. Невозможно уменьшить срок хранения.
...
Незаблокированная политика хранения, основанная на времени, рекомендуется только для тестирования компонентов, а политика должна быть заблокирована, чтобы быть совместимой с 17A-4 (f) и другими нормативными требованиями.
...
**Для учетной записи с поддержкой службы HNS можно переименовать или переместить большой двоичный объект, если он находится в неизменяемом состоянии?**
Нет, как имя, так и структура каталога считаются важными данными уровня контейнера, которые не могут быть изменены после установки неизменяемой политики. В общем случае переименование и перемещение доступны только для учетных записей с поддержкой HNS.
