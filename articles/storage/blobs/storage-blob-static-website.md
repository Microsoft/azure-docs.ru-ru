...
[Статические веб-приложения Службы приложений](https://azure.microsoft.com/services/app-service/static/) — это отличная альтернатива размещению статических веб-сайтов в Службе хранилища Azure. Она также подходит для сценариев, когда вам не нужен веб-сервер для отрисовки содержимого. Статические веб-приложения Службы приложений предоставляют полностью управляемый процесс непрерывной интеграции и непрерывной поставки (CI/CD) из источника GitHub в глобальное развертывание.
...
> [!NOTE]
> [Поддержка CORS для Службы хранилища Azure](/rest/api/storageservices/cross-origin-resource-sharing--cors--support-for-the-azure-storage-services) недоступна со статическим веб-сайтом.
...
Если вы планируете размещать веб-сайт в нескольких географических регионах, мы рекомендуем использовать [сеть доставки содержимого](../../cdn/index.yml) для регионального кэширования. Используйте [Azure Front Door](../../frontdoor/index.yml), если хотите предоставлять различное содержимое в каждом регионе. Также предоставляется возможность отработки отказа. Если вы планируете использовать личный домен, то не рекомендуется использовать [Диспетчер трафика Azure](../../traffic-manager/index.yml). Проблемы могут возникать из-за способа проверки имен личных доменов Службой хранилища Azure.
