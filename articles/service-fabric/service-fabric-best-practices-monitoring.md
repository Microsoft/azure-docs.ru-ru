...
Как правило, модуль наблюдения является отдельной службой, которая может отслеживать работоспособность и нагрузку в службах, проверять связь с конечными точками и сообщать о непредвиденных событиях работоспособности в кластере. Он позволяет предотвратить ошибки, которые невозможно обнаружить только на основе производительности отдельной службы. Устройства наблюдения также являются хорошим местом для размещения кода, который выполняет действия по исправлению, не требующие вмешательства пользователя, например очистка файлов журнала в хранилище с определенными интервалами времени. Если вам нужна полностью реализованная служба наблюдения за открытым исходным кодом, которая включает в себя простую в использовании модель расширяемости наблюдения и работает в кластерах Windows и Linux, см. проект [фабрикобсервер](https://aka.ms/sf/FabricObserver) . Фабрикобсервер — это готовое к производству программное обеспечение. Мы рекомендуем развертывать Фабрикобсервер в тестовых и рабочих кластерах и расширять их в соответствии с вашими потребностями либо через модель подключаемых модулей, либо развернув ее и создав собственные встроенные наблюдатели. Первым является рекомендуемый подход (подключаемые модули).
