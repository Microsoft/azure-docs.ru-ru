...
* [Нодетипехеалсполицимап](/dotnet/api/system.fabric.health.clusterhealthpolicy.nodetypehealthpolicymap). Карту политики работоспособности типа узла можно использовать во время оценки работоспособности кластера для описания специальных типов узлов. Типы узлов оцениваются относительно процентных долей, связанных с именем типа узла в сопоставлении. Установка этого значения не влияет на глобальный пул узлов, используемых для `MaxPercentUnhealthyNodes` . Например, кластер содержит сотни узлов различных типов и несколько типов узлов, которые размещают важную работу. Ни один из узлов этого типа не должен быть отключен. Можно указать значение Global `MaxPercentUnhealthyNodes` , равное 20%, чтобы допустить некоторые сбои для всех узлов, но для типа узла `SpecialNodeType` присвойте параметру `MaxPercentUnhealthyNodes` 0 значение. Таким образом, если некоторые из множества узлов находятся в неработоспособном состоянии, но ниже глобального неработоспособного процента, кластер будет считаться состоянием состояния "предупреждение". Состояние состояния "предупреждение" не влияет на обновление кластера или на другой мониторинг, вызванный состоянием работоспособности "ошибка". Но даже один узел типа `SpecialNodeType` в состоянии работоспособности "ошибка" сделает кластер неработоспособным и активирует откат или приостанавливает обновление кластера в зависимости от конфигурации обновления. И наоборот, если установить глобальное значение `MaxPercentUnhealthyNodes` 0 и задать `SpecialNodeType` для максимального процента неработоспособных узлов значение 100 с одним узлом типа `SpecialNodeType` в состоянии ошибки, кластер будет находиться в состоянии ошибки, поскольку глобальное ограничение в данном случае более строго. 

  Следующий пример содержит фрагмент манифеста кластера. Чтобы определить записи в карте типа узла, перед именем параметра укажите "Нодетипемаксперцентунхеалсинодес-", за которым следует имя типа узла.
