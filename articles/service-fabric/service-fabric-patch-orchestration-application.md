...
## <a name="automatic-os-image-upgrades"></a>Автоматическое обновление образа ОС

Получение [автоматических обновлений образов ОС в масштабируемых наборах виртуальных машин](../virtual-machine-scale-sets/virtual-machine-scale-sets-automatic-upgrade.md) — это рекомендуемый способ обновления операционной системы в Azure. Для автоматического обновления образов ОС на основе масштабируемого набора виртуальных машин потребуется Серебряная или более надежная устойчивость масштабируемого набора.
...
- Дополнительный зонд работоспособности или использование расширения работоспособности приложений для масштабируемых наборов виртуальных машин не требуется.
...
При использовании бронзовой устойчивости автоматическое обновление образа ОС недоступно. В то время как приложение для управления [исправлениями](#patch-orchestration-application ) (предназначенное только для кластеров, не размещенных в Azure) *не рекомендуется* для уровней устойчивости, а также для автоматизации обновлений Windows в отношении доменов обновления Service Fabric.

> [!IMPORTANT]
> Обновление в виртуальной машине, при котором "Центр обновления Windows" применяет исправления операционной системы без замены диска ОС, не поддерживается в Azure Service Fabric.

Для правильной работы функции с отключенными Центр обновления Windows в операционной системе необходимо выполнить два действия.

1. Включение автоматического обновления образа ОС, отключение ARM для обновлений Windows 
    ```json
...
1. Обновить модель масштабируемого набора после изменения этой конфигурации повторное создание образа всех компьютеров требуется для обновления модели масштабируемого набора, чтобы изменения вступили в действие.
...
Дополнительные инструкции см. в [разделе Автоматическое обновление образа ОС в масштабируемых наборах виртуальных машин](../virtual-machine-scale-sets/virtual-machine-scale-sets-automatic-upgrade.md) .

## <a name="patch-orchestration-application"></a>Приложение для управления исправлениями
...
оператионстарттиме | Дата/время | Указывает время запуска операции (загрузки или установки).
оператионтиме | Дата/время | Указывает время завершения операции (загрузки или установки).
