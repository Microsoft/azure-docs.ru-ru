...
Предварительный просмотр данных в конструкторе потока данных и отладка конвейеров потоков данных предназначен для лучшего использования с небольшими примерами данных. Однако если необходимо протестировать логику в конвейере или потоке данных на основе больших объемов данных, увеличьте размер Azure Integration Runtime, используемого в сеансе отладки, с большим количеством ядер и минимальным числом вычислений общего назначения.
...
> [!NOTE]
> При наличии конвейера с потоками данных, выполняемыми параллельно, или потоками данных, которые необходимо протестировать с помощью больших наборов данных, выберите пункт "использовать среду выполнения действий", чтобы фабрика могла использовать Integration Runtime, выбранные в действии потока данных. Это позволит потокам данных выполняться на нескольких кластерах и выполнять параллельные выполнения потоков данных.
