...
**Хранимая процедура**. Выберите этот параметр, если требуется создать проекцию и исходные данные из хранимой процедуры, которая выполняется из базы данных-источника. Можно ввести схему, имя процедуры и параметры или нажать кнопку "Обновить", чтобы запросить ADF для обнаружения схем и имен процедур. Затем можно щелкнуть импорт, чтобы импортировать все параметры процедуры с помощью формы ``@paraName`` .

![Хранимая процедура](media/data-flow/stored-procedure-2.png "Хранимая процедура")

- Пример SQL: ```Select * from MyTable where customerId > 1000 and customerId < 2000```
- Пример параметризованного SQL: ``"select * from {$tablename} where orderyear > {$year}"``
