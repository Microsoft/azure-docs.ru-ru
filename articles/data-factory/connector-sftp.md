...
- Копирование файлов из и на SFTP-сервер с помощью **обычной**, **открытого ключа SSH** или **многофакторной** проверки подлинности.
...
| authenticationType | Укажите тип проверки подлинности.<br/>Допустимые значения: " *базовый*", " *SshPublicKey* " и " *многофакторный*". Дополнительные сведения о свойствах см. в разделе [использование обычной проверки подлинности](#use-basic-authentication) . Примеры для JSON см. в разделе [использование проверки подлинности с открытым ключом SSH](#use-ssh-public-key-authentication) . |Да |
...
### <a name="use-multi-factor-authentication"></a>Использование многофакторной проверки подлинности

Чтобы использовать многофакторную проверку подлинности, которая представляет собой сочетание обычной проверки подлинности и открытого ключа SSH, укажите имя пользователя, пароль и сведения о закрытом ключе, описанные в разделах выше.

**Пример. многофакторная идентификация**
