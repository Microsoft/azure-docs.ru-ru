...
|:--- |:--- |:--- |
| type | `type`Для свойства должно быть задано значение `AzureBlobStorage` (предложено) или `AzureStorage` (см. следующие примечания). | Да |
| connectionString | Укажите сведения, необходимые для подключения к хранилищу для `connectionString` Свойства. <br/> Можно также поместить ключ учетной записи в Azure Key Vault и извлечь `accountKey` конфигурацию из строки подключения. Дополнительные сведения см. в следующих примерах и в статье [хранение учетных данных в Azure Key Vault](store-credentials-in-key-vault.md) . | Да |
...
>[!NOTE]
>Если вы используете `AzureStorage` тип связанной службы, он по-прежнему поддерживается как есть. Но мы рекомендуем использовать новый `AzureBlobStorage` тип связанной службы для пересылки.
...
| Свойство | Описание | Обязательно |
|:--- |:--- |:--- |
| type | `type`Свойству должно быть присвоено значение `AzureBlobStorage` (рекомендуется) или `AzureStorage` (см. следующую заметку). | Да |
| sasUri | Укажите URI подписанного URL-адрес для ресурсов хранилища, таких как BLOB-объект или контейнер. <br/>Пометьте это поле как `SecureString` безопасное, чтобы безопасно хранить его в фабрике данных. Также можно разместить маркер SAS в Azure Key Vault, чтобы использовать автоматическое вращение и удалить часть токена. Дополнительные сведения см. в следующих примерах и [хранение учетных данных в Azure Key Vault](store-credentials-in-key-vault.md). | Да |
| connectVia | [Среда выполнения интеграции](concepts-integration-runtime.md), используемая для подключения к хранилищу данных. Вы можете использовать среду выполнения интеграции Azure или локальную среду выполнения интеграции (если хранилище данных находится в частной сети). Если это свойство не указано, служба использует среду выполнения интеграции Azure по умолчанию. | Нет |
...
| аккаунткинд | Укажите тип учетной записи хранения. Допустимые значения: **Storage** (общего назначения v1), **StorageV2** (общее назначение версии 2), **блобстораже** или **блоккблобстораже**. <br/><br/>При использовании связанной службы больших двоичных объектов Azure в потоке данных проверка подлинности управляемого удостоверения или субъекта-службы не поддерживается, если тип учетной записи является пустым или хранилищем. Укажите правильный тип учетной записи, выберите другую проверку подлинности или обновите учетную запись хранения до общего назначения версии 2. | Нет |
...
>- Если доступ к хранилищу BLOB-объектов осуществляется через закрытую конечную точку с помощью потока данных, обратите внимание, что при использовании проверки подлинности субъекта службы данные подключаются к конечной точке ADLS 2-го поколения вместо конечной точки BLOB Убедитесь, что вы создали соответствующую закрытую конечную точку в ADF, чтобы обеспечить доступ.
...
> - Если доступ к хранилищу BLOB-объектов осуществляется через закрытую конечную точку с помощью потока данных, обратите внимание, что при использовании проверки подлинности управляемого удостоверения поток данных подключается к конечной точке ADLS 2-го поколения вместо конечной точки Убедитесь, что вы создали соответствующую закрытую конечную точку в ADF, чтобы обеспечить доступ.
...
| Свойство                 | Описание                                                  | Обязательно |
| ------------------------ | ------------------------------------------------------------ | -------- |
| type                     | `type`Свойство в разделе `storeSettings` должно иметь значение `AzureBlobStorageWriteSettings` . | Да      |
...
В этом случае все файлы, которые были источником в, `/data/sales` перемещаются в `/backup/priorSales` .
...
**Параметр имени файла.** Определяет способ присвоения имени целевым файлам в папке назначения. Доступные параметры имени файла:
   ...
   - **Шаблон**: введите шаблон, в котором перечислены выходные файлы для каждого раздела. Например, `loans[n].csv` будет создавать `loans1.csv` , `loans2.csv` и т. д.
...
| Свойство | Описание | Обязательно |
|:--- |:--- |:--- |
| type | `type`Свойство набора данных должно иметь значение `AzureBlob` . | Да |
...
| Свойство | Описание | Обязательно |
|:--- |:--- |:--- |
| type | `type`Свойство источника действия копирования должно иметь значение `BlobSource` . | Да |
| recursive | Указывает, следует ли читать данные рекурсивно из вложенных папок или только из указанной папки. Обратите внимание, что если `recursive` для задано значение `true` , а приемник является хранилищем на основе файлов, пустая папка или подпапка не копируется или не создается в приемнике.<br/>Допустимые значения: `true` (по умолчанию) и `false` . | Нет |
| maxConcurrentConnections | Количество одновременных подключений к хранилищу. Указывайте, только если требуется ограничить количество параллельных подключений к хранилищу данных. | Нет |
...
| Свойство | Описание | Обязательно |
|:--- |:--- |:--- |
| type | `type`Свойство приемника действия копирования должно иметь значение `BlobSink` . | Да |
| copyBehavior | Определяет поведение копирования, когда источником являются файлы из файлового хранилища данных.<br/><br/>Допустимые значения:<br/><b>— PreserveHierarchy (по умолчанию)</b>. Сохраняет иерархию файлов в целевой папке. Относительный путь исходного файла в исходной папке идентичен относительному пути целевого файла в целевой папке.<br/><b>— FlattenHierarchy</b>. Все файлы из исходной папки размещаются на первом уровне в целевой папке. Целевые файлы имеют автоматически сформированные имена. <br/><b>— MergeFiles</b>. Объединяет все файлы из исходной папки в один файл. Если указано имя Blob-объекта или имя файла, то оно присваивается объединенному файлу. В противном случае присваивается автоматически созданное имя файла. | Нет |
| maxConcurrentConnections | Количество одновременных подключений к хранилищу. Указывайте, только если требуется ограничить количество параллельных подключений к хранилищу данных. | Нет |
