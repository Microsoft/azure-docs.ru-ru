...
Щелкните [здесь](../virtual-network/service-tags-overview.md#use-the-service-tag-discovery-api-public-preview) , чтобы узнать, как получить список IP-адресов, из которых могут исходиться проверки работоспособности диспетчера трафика. Для получения последнего списка можно использовать REST API, Azure CLI или Azure PowerShell. Просмотрите список IP-адресов, чтобы убедиться в том, что входящие подключения с этих адресов IP разрешены на конечных точках для проверки состояния работоспособности.
...
> [!NOTE]
> Общедоступные IP-адреса могут измениться без предварительного уведомления. Обязательно извлеките последние сведения с помощью API обнаружения тегов служб или загружаемого JSON-файла.
