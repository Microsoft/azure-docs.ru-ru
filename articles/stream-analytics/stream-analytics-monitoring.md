...
| Загрузка ЦП% (Предварительная версия)       | Процент ЦП, используемый заданием. Даже если это значение очень высокое (90% или выше), не следует повышать число серверов на основе этой метрики. При увеличении числа невыполненных входных событий или задержки водяного знака вы можете использовать эту метрику загрузки ЦП%, чтобы определить, является ли ЦП узким местом. Возможно, у этой метрики есть пики. Рекомендуется выполнять масштабирование тестов, чтобы определить верхнюю границу задания, после которой входные данные получают невыполненную работу или задержка водяного знака из-за узких мест ЦП. |
