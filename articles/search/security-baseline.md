...
**Рекомендации**. прием журналов, связанных с когнитивный Поиск, с помощью Azure Monitor для агрегирования данных безопасности, создаваемых устройствами конечных точек, сетевыми ресурсами и другими системами безопасности. В Azure Monitor используйте рабочие области Log Analytics для запроса и выполнения анализа и используйте учетные записи хранения Azure для долгосрочного и архивного хранения. Кроме того, вы можете включить эти данные в Azure Sentinel или SIEM стороннего производителя.
...
**Встроенные определения политики Azure — Microsoft. Search**:
...
- [Настройка политики хранения для журналов учетных записей хранения Azure](https://docs.microsoft.com/azure/storage/common/manage-storage-analytics-logs#enable-logs)
...
- [Как получить роль каталога в Azure Active Directory (Azure AD) с помощью PowerShell](/powershell/module/azuread/get-azureaddirectoryrole?view=azureadps-2.0&amp;preserve-view=true)
...
**Руководство**. Включение многофакторной проверки подлинности в Azure Active Directory (Azure AD) и соблюдение рекомендаций по удостоверениям и доступу для центра безопасности.
...
**Руководство**. Использование рабочей станции привилегированного доступа (привилегированным доступом) с многофакторной проверкой подлинности, настроенной для входа в ресурсы Azure и получения доступа к ним.
...
**Руководство**: неприменимо к когнитивный Поиск. Он не позволяет устанавливать решения для защиты от вредоносных программ в своих ресурсах. Для базовой платформы Корпорация Майкрософт обрабатывает обновление любых программ и подписей для защиты от вредоносных программ.  
...
**Руководство**. когнитивный Поиск в настоящее время не поддерживает автоматическое резервное копирование данных в службе поиска. их необходимо архивировать с помощью ручного процесса. Вы также можете выполнять резервное копирование ключей, управляемых клиентом, в Azure Key Vault.
...
- [Резервное копирование ключей Key Vault в Azure](https://docs.microsoft.com/powershell/module/az.keyvault/backup-azkeyvaultkey?view=azps-4.8.0&amp;preserve-view=true)
...
- [Восстановление ключей Key Vault в Azure](https://docs.microsoft.com/powershell/module/az.keyvault/restore-azkeyvaultkey?view=azps-4.8.0&amp;preserve-view=true)
