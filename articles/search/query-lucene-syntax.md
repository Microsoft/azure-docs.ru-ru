...
Чтобы использовать полный синтаксис Lucene, задайте для queryType значение Full и передайте выражение запроса, предназначенное для подстановочного знака, нечеткий поиск или одну из других форм запросов, поддерживаемых полным синтаксисом. В остальных выражениях запросов предоставляются в **`search`** параметре запроса [поиска документов (REST API)](/rest/api/searchservice/search-documents) .

## <a name="example-full-syntax"></a>Пример (полный синтаксис)

Следующий пример представляет собой запрос поиска, созданный с использованием полного синтаксиса. В этом конкретном примере показан поиск в поле и повышение термина. Он ищет Гостиницы, в которых поле Category содержит термин "бюджет". Все документы, содержащие фразу «недавно отремонтированных», имеют более высокий приоритет, чем результат «повышение» (3).  
...
**`searchMode`** В этом примере параметр относится не только к типам запросов. Когда операторы находятся в запросе, обычно следует установить параметр `searchMode=all`, чтобы убедиться, что *все* критерии будут удовлетворены.  

Дополнительные примеры см. в разделе [Примеры синтаксиса запросов Lucene](search-query-lucene-examples.md). Дополнительные сведения о запросе и параметрах запроса, включая searchMode, см. в разделе [Поиск документов (REST API)](/rest/api/searchservice/Search-Documents).
