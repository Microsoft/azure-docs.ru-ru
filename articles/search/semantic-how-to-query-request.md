...
Семантические запросы, как правило, работают с индексами поиска, построенными на основе содержимого с большим объемом текста, например с помощью документов PDF или документов с большими фрагментами текста.
...
Семантические запросы предоставляют заголовки и выделение автоматически. Если вы хотите, чтобы ответ включал ответы, можно добавить в запрос необязательный **`answer`** параметр. Этот параметр, а также создание самой строки запроса, выдает ответ в ответе.
...
Порядок searchFields является критически важным. Если вы уже используете searchFields в существующих простых или полных запросах Lucene, не забудьте повторно перейти к этому параметру при переключении на семантический тип запроса.
...
+ Если в индексе есть поле URL-адреса, которое является текстовым (например `www.domain.com/name-of-the-document-and-other-details` , и не имеет компьютера `www.domain.com/?id=23463&param=eis` , например), поместите его вторым в списке (или сначала, если нет поля с кратким названием).
...
Если указано только одно поле, используйте описательные поля, в которых можно найти ответ на семантические запросы, например основное содержимое документа. Выберите поле, которое предоставляет достаточное содержимое.
...
При необходимости добавьте "Answers", если хотите включить дополнительную обработку, которая предоставляет ответ. Ответы (и заголовки) формируются из находящихся в полях, перечисленных в searchFields. Не забудьте включить в searchFields поля с богатым содержимым для получения лучших ответов и заголовков в ответе.

Существуют явные и неявные условия, создающие ответы. 

+ Явные условия включают добавление "Answers = Extracted". Кроме того, чтобы указать количество ответов, возвращаемых в общем ответе, добавьте "Count", за которым следует число: `"answers=extractive|count=3"` .  Значение по умолчанию — One. Максимум — пять.

+ Неявные условия включают в себя конструкцию строки запроса, которая предоставляет ответ. Запрос, состоящий из "что отеля имеет зеленую комнату", скорее всего, будет "отвечен", чем запрос, состоящий из такого оператора, как "Гостиницы" с внутренним внутренней областью. Как вы можете ожидать, запрос не может быть неопределенным или иметь значение null.

Важно отметить, что если запрос не похож на вопрос, обработка ответов пропускается, даже если задан параметр "Answers".
