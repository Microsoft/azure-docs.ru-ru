...
ms.locfileid: "104802902"
---
# <a name="how-to-configure-blob-indexing-in-cognitive-search"></a>Настройка индексирования BLOB-объектов в Когнитивный поиск

Индексатор BLOB-объектов используется для приема содержимого из хранилища BLOB-объектов Azure в индекс Когнитивный поиск. Индексаторы больших двоичных объектов часто используются в [обогащении искусственного интеллекта](cognitive-search-concept-intro.md), где присоединенный набор [навыков](cognitive-search-working-with-skillsets.md) добавляет изображение и обработку естественного языка для создания содержимого с возможностью поиска. Но вы также можете использовать индексаторы больших двоичных объектов без обогащения искусственного интеллекта для приема содержимого из текстовых документов, таких как документы PDF, Microsoft Office документы и форматы файлов.

В этой статье показано, как настроить индексатор BLOB-объектов для любого сценария. Если вы не знакомы с понятиями индексатора, начните с [индексаторов в Azure когнитивный Поиск](search-indexer-overview.md) и [Создайте индексатор поиска](search-howto-create-indexers.md) , прежде чем углубляться в индексирование BLOB-объектов.
...
Основное различие между индексатором больших двоичных объектов и любым другим индексатором — это определение источника данных, назначенное индексатору. Определение источника данных определяет тип источника данных ("Type": "azureblob"), а также другие свойства для проверки подлинности и подключения к индексируемому содержимому.
...
Индекс задает поля в документе, атрибуты, и другие компоненты, которые определяют процедуру поиска. Для всех индексаторов необходимо указать определение индекса поиска в качестве назначения. В следующем примере создается простой индекс, использующий [Создание индекса (REST API)](/rest/api/searchservice/create-index). 
...
**`content`** Поле является общим для содержимого BLOB-объектов. Он содержит текст, извлеченный из больших двоичных объектов. Определение этого поля может выглядеть примерно так, как показано выше. Использовать это имя не обязательно, но это позволяет использовать преимущества неявных сопоставлений полей. Индексатор больших двоичных объектов может передавать содержимое большого двоичного объекта в EDM содержимого. строковое поле в индексе без сопоставления полей.
