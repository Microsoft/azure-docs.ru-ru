...
Если для создания индексатора используется портал Azure, необходимо предоставить порталу входящий доступ к виртуальной машине SQL Azure. Для входящего правила в брандмауэре необходимо указать IP-адрес портала.

Чтобы получить IP-адрес портала, выполните команду ping `stamp2.ext.search.windows.net` , которая является доменом диспетчера трафика. Время ожидания запроса истечет, но IP-адрес будет отображаться в сообщении о состоянии. Например, в сообщении «ping azsyrie.northcentralus.cloudapp.azure.com [52.252.175.48]» IP-адрес — «52.252.175.48».

> [!NOTE]
> Кластеры в разных регионах подключаются к различным диспетчерам трафика. Независимо от имени домена, IP-адрес, возвращенный из проверки связи, является правильным, который используется при определении правила брандмауэра для входящего трафика для портал Azure в вашем регионе.
