...
+ Параметр "searchFields" определяет, какие поля предоставляют маркеры для модели извлечения. Во время принимать токенов используется не более 20 000 маркеров, поэтому следует начать список полей с краткими полями, а затем перейти к полям с расширенным текстом. Точные инструкции по заданию этого поля см. в разделе [Set searchFields](semantic-how-to-query-request.md#searchfields).
