...
Запрос — это запрос только для чтения для коллекции документов одного индекса поиска. Он задает параметр "Search", содержащий выражение запроса, состоящее из терминов, заключенных в кавычки фраз и операторов.

Дополнительные параметры предоставляют больше определений для запроса и ответа. Например, области "searchFields" запрашивают выполнение запросов к конкретным полям, "Select" указывает, какие поля возвращаются в результатах, а "Count" возвращает количество совпадений, найденных в индексе.

В следующем примере дается общее представление о запросе запроса путем отображения подмножества доступных параметров. Дополнительные сведения о составлении запросов см. в разделе [типы запросов и композиции](search-query-overview.md) и [Поиск документов (остальные)](/rest/api/searchservice/search-documents).
...
| JavaScript | [сеарчклиент](/javascript/api/@azure/search-documents/searchclient) | Ожидание. |
