...
|------------------------------|---------------|---------------|
| [Семантический поиск](semantic-search-overview.md) | Использование коллекции функций, связанных с запросами, значительно улучшает релевантность результатов поиска с минимальными усилиями. </br></br>[Семантическое ранжирование](semantic-ranking.md) вычисляет оценки релевантности с помощью семантического значения слов и содержимого. </br></br>[Семантические заголовки](semantic-how-to-query-request.md) — это релевантные фрагменты документа, которые наилучшим образом характеризуют документ с выделением наиболее важных терминов или фраз. </br></br>[Семантические ответы](semantic-answers.md) — это ключевые фрагменты, извлеченные из искомого документа, которые формулируются как прямой ответ на запрос, который выглядит как вопрос. | Общедоступная предварительная версия ([по запросу](https://aka.ms/SemanticSearchPreviewSignup)). </br></br>Используйте [Поиск по документам — REST](/rest/api/searchservice/preview-api/search-documents) api-version=2020-06-30-Preview и [Проводник поиска](search-explorer.md) на портале Azure. </br></br>Применяются ограничения по региону и уровню. |
...
| [Нормализаторы](search-normalizers.md) | Нормализаторы обеспечивают простую предварительную обработку текста, например исправление регистра, удаление диакритических знаков, приведение к ASCII и т. д. без выполнения всей цепочки анализа.| Общедоступная предварительная версия, только REST, api-version=2020-06-30-Preview |
