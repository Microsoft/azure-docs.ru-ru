...
|------------------------------|---------------|---------------|
| [Семантический поиск](semantic-search-overview.md) | Набор функций для выполнения запросов, которые изменяют результаты поиска для получения более релевантных результатов. Внеся небольшие изменения в запрос поиска, вы можете оценить работу этих функций на существующих индексах.</br></br>[Семантический запрос](semantic-how-to-query-request.md) — это новый тип запроса, который использует усовершенствования в обработке естественного языка для улучшения ранжирования и определения намерения запроса, чтобы предоставлять ответы, субтитры и семантическое выделение.</br></br>[Семантическое ранжирование и ответы (ответы, субтитры и выделение)](semantic-how-to-query-response.md) относятся к алгоритму, который определяет семантический запрос, и структуре ответа. | Общедоступная предварительная версия ([по запросу](https://aka.ms/SemanticSearchPreviewSignup)). </br></br>Используйте [Поиск по документам — REST](/rest/api/searchservice/preview-api/search-documents) api-version=2020-06-30-Preview и [Проводник поиска](search-explorer.md) на портале Azure. </br></br>Применяются ограничения по региону и уровню. |
