...
Azure Reserved Virtual Machine Instances обеспечивают гибкость, позволяющую удовлетворить ваши требования. Зарезервированный экземпляр можно обменять на другой зарезервированный экземпляр того же типа. Например, можно вернуть несколько зарезервированных вычислений, включая Выделенный узел Azure, Решение Azure VMware и Виртуальные машины Azure одновременно. Иными словами, зарезервированные продукты взаимозаменяемы друг с другом, если они имеют одинаковый тип резервирования. В другом примере можно обменять несколько типов резервирования базы данных SQL, включая Управляемые экземпляры и Эластичный пул.

Однако нельзя обменивать непохожие резервирования. Например, нельзя обменять резервирование Cosmos DB на Базу данных SQL.

Вы также можете обменять резервирование, чтобы приобрести еще одно резервирование аналогичного типа в другом регионе. Например, можно обменять резервирование, которое находится в западной части США 2, на то, которое находится в Западной Европе.

При обмене резервирования можно изменить соответствующий срок действия с одного года на три.

Вы также можете возместить средства за резервирование, но общая сумма всех отмененных обязательств по резервированию в области выставления счетов (таких как EA, Клиентское соглашение Майкрософт и Соглашение с партнером Майкрософт) не может превышать 50 000 долларов США за 12 месяцев (скользящий интервал).

Зарезервированная емкость Azure Databricks, резервирование решений Azure VMware от CloudSimple, резервирование Open Shift в Azure Red Hat, планы Red Hat и планы SUSE Linux недоступны для возмещения.
...
## <a name="exchange-multiple-reservations"></a>Обмен нескольких резервирований

Аналогичные типы резервирования можно возвратить с помощью одного действия.

При обмене резервирований сумма новой покупки в валюте должна быть больше суммы возврата. Если сумма новой покупки меньше суммы возврата, отобразится сообщение об ошибке. Если отобразится сообщение об ошибке, сократите сумму возврата или увеличьте сумму покупки.

1. Войдите на портал Azure и перейдите к разделу **Резервирования**.
1. В списке резервирований выберите соответствующее поле для каждого резервирования, которое необходимо обменять.
1. В верхней части страницы выберите **Обмен**.
1. При необходимости измените сумму для каждого резервирования, которое необходимо возвратить.
1. Если выбран параметр автозаполнения суммы возврата, выберите **Refund all** (Возместить все), чтобы указать в списке полную сумму для каждого резервирования, или **Optimize for utilization (7-day)** (Оптимизировать для использования (7 дней)), чтобы указать в списке количество, которое будет оптимизировано для использования на основе последних семи дней использования. Выберите **Применить**.
1. В нижней части страницы выберите **Далее: Покупка**.
1. На вкладке "Покупка" выберите доступные продукты, которые необходимо обменять. Вы можете выбрать несколько продуктов различных типов.
1. В области "Выберите продукт, который вы хотите приобрести" выберите необходимые продукты, а затем щелкните **Добавить в корзину** и **Закрыть**.
1. По завершении нажмите **Далее: Отзыв**.
1. Просмотрите резервирования для возврата и новые резервирования для покупки, а затем выберите **Подтвердить обмен**.
