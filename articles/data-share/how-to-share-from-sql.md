...
Чтобы предоставить общий доступ к данным с помощью Azure Active Directory проверки подлинности, ниже приведен список необходимых компонентов.
...
* **Администратор SQL Server Azure Active Directory**
...
Ниже приведен список предварительных требований для совместного использования данных с помощью проверки подлинности SQL. Для настройки необходимых компонентов cм. [ролик с пошаговыми инструкциями](https://youtu.be/hIE-TjJD8Dc).
...
1. Выберите рабочую область SQL Server или синапсе. Если используется проверка подлинности AAD и флажок **Разрешить общему ресурсу данных запускать указанный выше сценарий SQL "создать пользователя" от моего имени** , установите флажок. Если используется проверка подлинности SQL, укажите учетные данные и выполните действия, описанные в разделе необходимые условия для запуска скрипта на экране. Это дает ресурсу Data Share разрешение на чтение из базы данных SQL. 

   Нажмите кнопку **Далее** , чтобы вернуться к объекту, к которому вы хотите предоставить общий доступ, и выберите команду "добавить наборы данных". Вы можете выбрать таблицы и представления из базы данных SQL Azure и Azure синапсе Analytics (ранее — хранилище Azure SQL) или таблиц из выделенного пула SQL Azure синапсе Analytics (Рабочая область). 
...
Чтобы получить данные в SQL Server, где вы являетесь **Azure Active Directory администратором** SQL Server, ниже приведен список необходимых компонентов.
...
Чтобы получить данные в SQL Server, где вы не являетесь **администратором Azure Active Directory**, ниже приведен список необходимых компонентов. Для настройки необходимых компонентов cм. [ролик с пошаговыми инструкциями](https://youtu.be/aeGISgK1xro).
...
1. Выберите целевое хранилище данных, в котором должны находиться данные. Все файлы и таблицы данных в целевом хранилище данных с тем же путем и именем будут перезаписаны. Если данные поступают в целевой объект SQL, а параметр **Разрешить общий ресурс данных для запуска указанного выше флажка "создать скрипт SQL для пользователя"** отображается, установите флажок. В противном случае следуйте инструкциям в статье необходимые условия для запуска сценария на экране. Это предоставит ресурсу общего ресурса данных разрешение на запись в целевую базу данных SQL.

   ![Целевая учетная запись хранения](./media/dataset-map-target-sql.png "Целевое хранилище данных") 

1. Для совместного использования на основе моментальных снимков можно также включить на вкладе **Расписание моментальных снимков** функцию расписания моментальных снимков при условии, что поставщик данных создал такое расписание для регулярного обновления данных. Установите флажок рядом с расписанием моментальных снимков и выберите **+ Включить**. Обратите внимание, что первый запланированный моментальный снимок начнется в течение одной минуты времени, а последующие моментальные снимки будут запускаться в течение нескольких секунд по расписанию.
