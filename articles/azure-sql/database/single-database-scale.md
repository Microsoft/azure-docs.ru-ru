...
[!NOTE]
 Свойство избыточности зоны будет по умолчанию оставаться неизменным при масштабировании от критически важный для бизнеса до уровня общего назначения. Задержка для этого перехода на более раннюю версию при включении избыточности зоны, а также Задержка переключения на избыточность зоны для уровня общего назначения будет пропорционально размеру базы данных.
...
- Если копирование данных требуется для масштабирования базы данных (см. раздел [Задержка](#latency)) при изменении уровня служб, то высокая степень использования ресурсов, параллельная операции масштабирования, может привести к увеличению времени масштабирования. Благодаря [ускоренному восстановлению базы данных (ADR)](/sql/relational-databases/accelerated-database-recovery-concepts)откат длительных транзакций не является значительным источником задержки, но при высоком уровне использования ресурсов может остаться меньше ресурсов вычислений, хранения и пропускной способности сети для масштабирования, особенно для меньших размеров вычислений.
