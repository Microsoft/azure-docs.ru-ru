...
### <a name="how-can-i-obtain-the-ip-address-of-an-smb-volume-via-the-portal"></a>Как получить IP-адрес тома SMB через портал?

Используйте ссылку **представление JSON** на панели Обзор томов и найдите идентификатор **стартип** в разделе **Свойства**  ->  **маунттаржетс**.
...
Для каталога размером 320 МБ количество блоков составляет 655360, а размер каждого блока составляет 512 байт.  (Т. е. 320x1024x1024/512.)  Это число преобразуется в приблизительно 4 000 000 файлов для каталога 320 МБ. Однако фактическое число максимальных файлов может быть меньше, в зависимости от таких факторов, как количество файлов, содержащих символы, не входящие в набор ASCII в каталоге. Таким образом, следует использовать команду, `stat` как показано ниже, чтобы определить, приближается ли к вашему каталогу ограничение.  
