...
* [Сопоставление пользователей доменных служб Active Directory (AD DS) с помощью протокола LDAP для расширенных групп NFS](configure-ldap-extended-groups.md) (предварительная версия)   

    По умолчанию Azure NetApp Files поддерживает до 16 ИД групп при обработке учетных данных пользователей NFS, как описано в [RFC 5531](https://tools.ietf.org/html/rfc5531). Теперь вы можете увеличить это число до 1024, если у вас есть пользователи, которые входят в большее количество групп, чем доступно по умолчанию. Также тома NFS теперь можно добавлять в протокол LDAP AD DS. Благодаря этому пользователи AD DS с расширенным количеством групп (до 1024) могут получать доступ к тому. 

## <a name="march-2021"></a>Март 2021 г.

* [Общие папки непрерывной доступности SMB](azure-netapp-files-create-volumes-smb.md#add-an-smb-volume) (предварительная версия)  

    Прозрачная отработка отказа SMB позволяет выполнять операции обслуживания в службе Azure NetApp Files, не прерывая подключения к серверным приложениям, которые хранят данные в томах SMB и имеют доступ к ним. Для поддержки прозрачной отработки отказа SMB Azure NetApp Files теперь поддерживает параметр общих папок постоянной доступности SMB, который можно использовать для приложений SQL Server в SMB, выполняющемся на виртуальных машинах Azure. В настоящее время эта функция поддерживается в Windows SQL Server. SQL Server Linux в настоящее время не поддерживается. Включение этой функции обеспечивает значительное повышение производительности SQL Server, масштабируемость и экономичность для [одного экземпляра, экземпляра отказоустойчивого кластера Always-On и развертываний Групп доступности Always-On](azure-netapp-files-solution-architectures.md#sql-server). Ознакомьтесь с [преимуществами использования Azure NetApp Files для развертывания SQL Server](solutions-benefits-azure-netapp-files-sql-server.md).

* [Автоматическое изменение размера целевого тома репликации между регионами](azure-netapp-files-resize-capacity-pools-or-volumes.md#resize-a-cross-region-replication-destination-volume)

    В соотношении репликации между регионами целевой том автоматически изменяется в зависимости от размера исходного тома. Таким образом, размер целевого тома не нужно изменять отдельно. Автоматическое изменение размера применяется, если тома находятся в активном отношении репликации или если пиринг репликации разбивается с помощью операции повторной синхронизации. Чтобы эта функция работала, необходимо обеспечить достаточный запас ресурсов в пулах емкости для исходного и целевого томов.

## <a name="december-2020"></a>Декабрь 2020 г.

* [Средство создания моментальных снимков, согласованных с приложениями Azure](azacsnap-introduction.md) (предварительная версия)    
...
* [Репликация между регионами Azure NetApp Files](cross-region-replication-introduction.md) (предварительная версия)
