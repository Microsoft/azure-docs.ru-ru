...
* Виртуальные машины Azure с расширением наблюдателя за сетями отправляют данные в рабочую область и метрики. Монитор подключения делает данные доступными через новые метрики (Чекксфаиледперцент и Раундтриптимемс) вместо старых метрик (Пробесфаиледперцент и Аверажераундтрипмс). Старые метрики будут перенесены в новые метрики как Пробесфаиледперцент-> Чекксфаиледперцент и Аверажераундтрипмс-> Раундтриптимемс.
* Мониторинг данных:
...
С помощью нескольких щелчков вы перенесли существующие мониторы подключений в монитор подключений. После переноса из CM (классическая модель) в CM вы не сможете увидеть монитор в папке CM (классическая модель).
