...
| [**Устройство IoT**](#iot-device) | IoTHub<br>DeviceId<br>DeviceName<br>иотсекуритяжентид<br>DeviceType<br>Источник<br>саурцереф<br>Изготовитель<br>Моделирование<br>OperatingSystem<br>IPAddress<br>MacAddress<br>Протоколы<br>SerialNumber | IoTHub<br>DeviceId | IoTHub + DeviceId |
...
| Категория | Строка | Категория вредоносных программ по поставщику, например Троян. |
...
| ValueType | Перечисления? | Одно из следующих значений:<li>Строка<li>Двоичные данные<li>DWord<li>QWORD<li>MultiString<li>ExpandString<li>None<li>Неизвестно<br>Значения должны соответствовать перечислению Microsoft. Win32. RegistryValueKind. |
|
...
| Моделирование | Строка | Модель устройства. |
...
| RiskLevel | Перечисления? | Уровень риска этого почтового ящика. Возможные значения:<li>None<li>Низкий<li>Средний<li>Высокий |
...
| Сообщения | Дата и время | Дата получения этого сообщения. |
...
| Тема | Строка | Тема данного почтового сообщения. |
...
| сендерип | Строка | IP-адрес отправителя. |
| Тема | Строка | Тема отправки почты. |
