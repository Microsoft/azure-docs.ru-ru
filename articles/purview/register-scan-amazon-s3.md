...
1. Выберите **другую учетную запись AWS** и введите следующие значения:
    ...
    |Поле  |Описание  |
    |---------|---------|
    |**идентификатор учетной записи**;     |    Введите идентификатор учетной записи Майкрософт. Пример: `615019938638`     |
    |**Внешний идентификатор**     |   В разделе Параметры выберите параметр **требовать внешний идентификатор...**, а затем введите внешний идентификатор в указанном поле. <br>Пример: `e7e2b8a3-0a9f-414f-a065-afaf4ac6d994`    <br><br>Этот внешний идентификатор можно найти при выполнении.  |
    | | |
...
1. Найдите и выберите контейнер для просмотра страницы сведения о контейнере, а затем скопируйте имя контейнера в буфер обмена.
    ...
    Пример: `s3://purview-tutorial-bucket`
