...
6. Если сервер VMM находится в отключенном состоянии, скачайте и запустите приведенный ниже скрипт очистки на сервере VMM. Откройте PowerShell с параметром **Запуск от имени администратора** , чтобы изменить политику выполнения для области по умолчанию (LocalMachine). В скрипте укажите идентификатор сервера VMM, который требуется удалить. Скрипт удалит с сервера информацию о регистрации и связывании облаков.
