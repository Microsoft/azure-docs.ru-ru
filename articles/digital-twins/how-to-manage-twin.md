...
Определенные свойства цифровых двойника возвращаются в виде свойств верхнего уровня в цифровом двойника. Метаданные или системные сведения, не входящие в определение ДТДЛ, возвращаются с `$` префиксом. Свойства метаданных включают следующие значения:
* `$dtId`— Идентификатор цифрового двойника в этом экземпляре Azure Digital двойников.
* `$etag`— Стандартное поле HTTP, назначенное веб-сервером. Это значение обновляется до нового значения при каждом обновлении двойника, что может быть полезно для определения того, были ли данные двойника обновлены на сервере с момента предыдущей проверки. Можно использовать `If-Match` для выполнения обновлений и удалений, которые выполняются только в том случае, если ETag сущности совпадает с указанным ETag. Дополнительные сведения об этих операциях см. в документации по [Дигиталтвинс Update](/rest/api/digital-twins/dataplane/twins/digitaltwins_update) и [дигиталтвинс Delete](/rest/api/digital-twins/dataplane/twins/digitaltwins_delete).
* `$metadata`: Набор других свойств, включая:
...
Вы можете создавать исправления с помощью [жсонпатчдокумент](/dotnet/api/azure.jsonpatchdocument)пакета SDK для Azure .NET. Ниже приведен пример.
...
Пример того, как удалить все двойников одновременно, можно скачать с помощью примера приложения, используемого в [*учебнике. Изучите основные сведения с примером клиентского приложения*](tutorial-command-line-app.md). Файл *коммандлуп. CS* делает это в `CommandDeleteAllTwins()` функции.
