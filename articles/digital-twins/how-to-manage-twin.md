...
* `$etag`— Стандартное поле HTTP, назначенное веб-сервером. Это значение обновляется до нового значения при каждом обновлении двойника, что может быть полезно для определения того, были ли данные двойника обновлены на сервере с момента предыдущей проверки. Его также можно использовать в заголовках HTTP следующим образом:
  - с операциями чтения, чтобы избежать выборки содержимого, которое не было изменено
  - с операциями записи для поддержки оптимистичного параллелизма
