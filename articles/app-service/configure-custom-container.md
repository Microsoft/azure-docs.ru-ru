...
Если приложение использует образы из частного реестра или из центра DOCKER, то учетные данные для доступа к репозиторию сохраняются в переменных среды: `DOCKER_REGISTRY_SERVER_URL` `DOCKER_REGISTRY_SERVER_USERNAME` и `DOCKER_REGISTRY_SERVER_PASSWORD` . Из-за угроз безопасности ни одно из этих зарезервированных имен переменных не предоставляется приложению.
