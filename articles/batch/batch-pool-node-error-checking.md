...
Если пул занимает больше времени, чем ожидалось, пакетная обработка будет периодически повторяться до тех пор, пока пул не сможет быть успешно удален. В некоторых случаях задержка происходит из-за сбоя службы Azure или других временных проблем. Другие факторы, которые могут препятствовать удалению пула, могут потребовать выполнения действий по устранению проблемы. К таким факторам относятся следующие.

- Блокировки ресурсов были помещены в ресурсы, созданные пакетной службой, или на сетевые ресурсы, используемые пакетом.
- Созданные ресурсы имеют зависимость от ресурса, созданного пакетом. Например, при [создании пула в виртуальной сети](batch-virtual-network.md)Пакетная служба создает группу безопасности сети (NSG), общедоступный IP-адрес и подсистему балансировки нагрузки. Если вы используете эти ресурсы за пределами пула, удалить пул невозможно, пока зависимость не будет удалена.
- Регистрация поставщика ресурсов Microsoft.BatCH отменена в подписке, содержащей пул.
- "Пакетная служба Microsoft Azure" больше не имеет [роли участника или владельца](batch-account-create-portal.md#allow-azure-batch-to-access-the-subscription-one-time-operation) для подписки, содержащей пул (для учетных записей пакетной службы в режиме подписки пользователя).
...
Кроме того, после запуска узла на диске операционной системы для создания пользователей требуется небольшой объем пространства.

Размер временного диска зависит от размера виртуальной машины. Одним из соображений при выборе размера виртуальной машины является обеспечение достаточного места на временном диске для запланированной рабочей нагрузки.
...
Если на временном диске или в операционной системе не хватает места (или почти заканчивается место на диске), узел перейдет в [неработоспособное](/rest/api/batchservice/computenode/get#computenodestate) состояние, и появится сообщение об ошибке узла о том, что диск заполнен.
