...
>
> Чтобы подключиться к узлу, на котором запущена конфигурация виртуальной машины Linux по протоколу SSH, см. статью [Установка и настройка xrdp для использования удаленный рабочий стол с Ubuntu](../virtual-machines/linux/use-remote-desktop.md). Сведения о подключении к узлам под Windows через RDP см. в статье [подключение и вход в виртуальную машину Azure под Windows](../virtual-machines/windows/connect-logon.md).
>
...
> [!IMPORTANT]
> Пакетная служба версии 2017 01-01.4.0 введено предоставила критическое изменение, которое требует обновления кода для вызова этой версии или более поздней версии. Краткие инструкции по обновлению кода пакетной службы с более ранней версии см. [в статье обновление кода до последней клиентской библиотеки пакетной](#update-your-code-to-the-latest-batch-client-library) службы.
