...
>
>Пулы пакетной службы Azure начнут поддерживать виртуальные машины в будущем, используя новые версии [API и средств пакетной](./batch-apis-tools.md)службы. После того как доступна поддержка малых виртуальных машин, виртуальные машины с низким приоритетом будут устаревшими — они продолжат поддерживаться с использованием текущих API и версий средств в течение не менее 12 месяцев, чтобы обеспечить достаточное время для миграции на виртуальные машины.
>
> Плашечные виртуальные машины будут поддерживаться только для пулов конфигурации виртуальных машин. Для использования плашечных виртуальных машин пулы конфигурации облачных служб потребуется [перенести в пулы конфигураций виртуальных машин](batch-pool-cloud-service-to-virtual-machine-configuration.md).
