...
ms.locfileid: "101739149"
---
# <a name="azure-security-baseline-for-event-grid"></a>Базовый план безопасности Azure для службы "Сетка событий"

Этот базовый план безопасности применяет рекомендации из [теста безопасности Azure версии 1,0](../security/benchmarks/overview-v1.md) к Microsoft Azure сетке событий. Azure Security Benchmark содержит рекомендации по обеспечению безопасности облачных решений в Azure.
Содержимое группируются по **элементам управления безопасностью** , определенным в тестовом производительности системы безопасности Azure, и связанным рекомендациям, применимым к службе "Сетка событий Azure". **Элементы управления** , неприменимые к службе "Сетка событий Azure", были исключены.

 
Сведения о том, как служба "Сетка событий Azure" полностью сопоставляется с производительностью системы безопасности Azure, см. в статье [полный базовый план безопасности службы "Сетка событий Azure](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines)".
...
- [Дополнительные сведения о частных конечных точках сетки событий](https://docs.microsoft.com/azure/event-grid/network-security#private-endpoints)

- [Дополнительные сведения о брандмауэре IP-адресов в сетке событий](https://docs.microsoft.com/azure/event-grid/network-security#ip-firewall)
...
- [Группа безопасности сети Azure](/azure/virtual-network/security-overview)
...
**Руководство**. Используйте центр безопасности Azure и следуйте рекомендациям по защите сети, чтобы помочь защитить ресурсы сетки событий в Azure. При использовании

Виртуальные машины Azure для доступа к ресурсам службы "Сетка событий", включения журналов потоков для группы безопасности сети (NSG) и отправки журналов в учетную запись хранения для аудита трафика.
...
- [Использование тега службы в службе "Сетка событий Azure"](https://docs.microsoft.com/azure/event-grid/network-security#service-tags)
...
Вы также можете использовать встроенные определения политик, связанные со службой "Сетка событий Azure", например:

- Домены Сетки событий Azure должны использовать приватные каналы.

- В разделах службы "Сетка событий Azure" должны использоваться частные ссылки Azure
- [Встроенные политики для ресурсов сетки событий](https://docs.microsoft.com/azure/governance/policy/samples/built-in-policies#event-grid)
...
**Руководство**. Управление доступом к ресурсам сетки событий осуществляется с помощью Azure Active Directory (Azure AD). В Azure AD нет концепции паролей по умолчанию.
...
**Руководство**. Неприменимо. Служба "Сетка событий" не использует Azure Active Directory (Azure AD) для проверки подлинности клиентов публикации событий. Он поддерживает проверку подлинности с помощью ключей SAS.
