...
> [!NOTE]
> Синтаксис выражений во многом идентичен в Java и Котлин. Если у вас есть документация Котлин, но вы видите блоки кода для Java, код будет одинаковым на обоих языках.
...
|------------|-------------|-------------|
| `accumulated()` | Число | Возвращает значение свойства кластера, накопленного до сих пор. |
...
| `length(string | Expression)` | Число | Возвращает длину строки или массива. |
...
| Expression | Возвращаемый тип | Описание |
|------------|-------------|-------------|
| `abs(number | Expression)` | Число | Вычисляет абсолютное значение заданного числа. |
| `acos(number | Expression)` | Число | Вычисляет арккосинус указанного числа. |
| `asin(number | Expression)` | Число | Вычисляет арксинус указанного числа. |
| `atan(number | Expression)` | Число | Вычисляет арктангенс указанного числа. |
| `ceil(number | Expression)` | Число | Округляет число до следующего целого числа. |
| `cos(number | Expression)` | Число | Вычисляет COS указанного числа. |
| `division(number, number)` \| `division(Expression, Expression)` | Число | Делит первое число на второе число. Выражение эквивалента веб-пакета SDK: `/` |
| `e()` | Число | Возвращает математическую константу `e` . |
| `floor(number | Expression)` | Число | Округляет число до предыдущего целого числа. |
| `log10(number | Expression)` | Число | Вычисляет десятичный логарифм указанного числа. |
| `log2(number | Expression)` | Число | Вычисляет логарифм указанного числа с основанием 2. |
| `ln(number | Expression)` | Число | Вычисляет натуральный логарифм заданного числа. |
| `ln2()` | Число | Возвращает математическую константу `ln(2)` . |
| `max(numbers... | expressions...)` | Число | Вычисляет максимальное число в указанном наборе чисел. |
| `min(numbers... | expressions...)` | Число | Вычисляет минимальное число в указанном наборе чисел. |
| `mod(number, number)` \| `mod(Expression, Expression)` | Число | Вычисляет остаток при делении первого числа на второе число. Выражение эквивалента веб-пакета SDK: `%` |
| `pi()` | Число | Возвращает математическую константу `PI` . |
| `pow(number, number)` \| `pow(Expression, Expression)` | Число | Вычисляет значение первого значения, возведенного в степень второго числа. |
| `product(numbers... | expressions...)` | Число | Умножает указанные числа вместе. Выражение эквивалента веб-пакета SDK: `*` |
| `round(number | Expression)` | Число | Округляет число до ближайшего целого. Среднее значение округляется с нуля. Например, `round(-1.5)` принимает значение `-2` . |
| `sin(number | Expression)` | Число | Вычисляет синус указанного числа. |
| `sqrt(number | Expression)` | Число | Вычисляет квадратный корень заданного числа. |
| `subtract(number | Expression` | Число | Вычитает 0 на указанное число. |
| `subtract(number | Expression, number | Expression)` | Число | Вычитает первые цифры по второму числу. |
| `sum(numbers... | expressions...)` | Число | Вычисляет сумму указанных чисел. |
| `tan(number | Expression)` | Число | Вычисляет тангенс указанного числа. |
...
| `number(Expression)` | Число | Утверждает, что входное значение является числом. |
...
| `toNumber(Expression)` | Число | По возможности преобразует входное значение в число. |
...
| `isSupportedScript(string)` \| `isSupportedScript(Expression)`| Логическое | Определяет, использует ли входная строка кодировку, поддерживаемую текущим стеком шрифтов. Пример: `isSupportedScript("ಗೌರವಾರ್ಥವಾಗಿ")` |
