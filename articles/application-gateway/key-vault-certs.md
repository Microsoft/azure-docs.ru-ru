...
1. **Настройка хранилища ключей**
   ...
   Затем импортируйте существующий сертификат или создайте новый в хранилище ключей. Сертификат будет использоваться приложениями, которые выполняются через шлюз приложений. На этом шаге можно также использовать Key Vault секрет, который также позволяет хранить PFX-файл без пароля и с основанием 64. Мы рекомендуем использовать тип "Certificate" из-за возможности автопродления, доступной для объектов этого типа в Key Vault. После создания сертификата или секрета необходимо определить политики доступа в Key Vault, чтобы предоставить удостоверению доступ к секрету.
   
   > [!IMPORTANT]
   > Начиная с 15 марта 2021 Key Vault распознает шлюз приложений Azure как одну из доверенных служб, тем самым позволяя создавать границы защищенной сети в Azure. Это дает возможность запретить доступ к трафику из всех сетей (включая Интернет-трафик) в Key Vault, но по-прежнему сделать его доступным для ресурса шлюза приложений в подписке. 

   > Вы можете настроить шлюз приложений в ограниченной сети Key Vault следующим образом. <br />
   > a) в колонке сети Key Vault <br />
   > б) выберите частная конечная точка и выбранные сети на вкладке "брандмауэр и виртуальные сети". <br/>
   > в) затем использовать виртуальные сети, добавить виртуальную сеть и подсеть шлюза приложений. В процессе также настройте конечную точку службы Microsoft. KeyVault, установив ее флажок. <br/>
   > г. Наконец, выберите "Да", чтобы разрешить доверенным службам обход брандмауэра Key Vault. <br/>
   > 
   > ![Брандмауэр Key Vault](media/key-vault-certs/key-vault-firewall.png)
