...
Настройка шлюза с помощью частного IP-адреса внешнего интерфейса полезна для внутренних бизнес-приложений, которые не доступны в Интернете. Это также полезно для служб и уровней в многоуровневом приложении, которые находятся в границах безопасности, которые не доступны в Интернете, но:

- по-прежнему требуется распределение нагрузки с циклическим перебором
- закрепление сеанса
- или прерывание TLS (ранее известное как SSL (SSL)).
...
В Azure для обмена между создаваемыми ресурсами необходима виртуальная сеть. Либо создайте новую виртуальную сеть, либо используйте существующую. 

В этом примере мы создадим виртуальную сеть. Вы можете создать виртуальную сеть во время создания шлюза приложений. Экземпляры Шлюза приложений создаются в отдельных подсетях. В этом примере есть две подсети: одна для шлюза приложений, а другая — для внутренних серверов.
...
4. В качестве **региона** выберите **Центральная область США**.
...
6. Нажмите кнопку **ОК** , чтобы создать виртуальную сеть и подсети.
...
15. В разделе **правила маршрутизации** выберите **Добавить правило маршрутизации**.
...
21. Для **параметра HTTP** выберите **Добавить новый**.
...
4. Введите следующие значения для виртуальной машины:
   ...
   - Введите *myVM* в качестве **имени виртуальной машины**.
   - Выберите **Windows Server 2019 Datacenter** для **образа**.
   - Введите допустимое **имя пользователя**.
   - Введите допустимый **пароль**.
1. Примите остальные значения по умолчанию и выберите **Далее: диски**.
1. Примите значения по умолчанию и выберите **Далее: сеть**.
1. Убедитесь, что для виртуальной сети выбрано **myVNet** , а подсеть — **myBackendSubnet**.
1. Примите остальные значения по умолчанию и выберите **Далее: Управление**.
1. Выберите **Отключить** , чтобы отключить диагностику загрузки.
1. Выберите **Review + create** (Просмотреть и создать).
1. Просмотрите параметры на странице сводки и нажмите кнопку **Создать**. Создание виртуальной машины может занять несколько минут. Дождитесь успешного завершения развертывания перед переходом к следующему разделу.
...
3. Создайте вторую виртуальную машину и установите службы IIS, следуя только что выполненным инструкциям. Используйте myVM2 для имени виртуальной машины и для `VMName` в `Set-AzVMExtension` .
...
2. Выберите **серверные пулы**, а затем выберите **appGatewayBackendPool**.
...
4. Повторите, чтобы добавить MyVM2.
   ![Область изменения серверного пула с выделенными целевыми типами и целевыми объектами.](./media/configure-application-gateway-with-private-frontend-ip/private-frontendip-4.png)
5. Щелкните **Сохранить**.

## <a name="create-a-client-virtual-machine"></a>Создание клиентской виртуальной машины

Виртуальная машина клиента используется для подключения к внутреннему пулу шлюза приложений.

- Создайте третью виртуальную машину, выполнив предыдущие шаги. Используйте myVM3 для имени виртуальной машины.

## <a name="test-the-application-gateway"></a>Тестирование шлюза приложений

1. На странице myAppGateway выберите **IP-конфигурации переднего плана** , чтобы заметкать частный IP-адрес внешнего интерфейса.
    ![Панель IP-конфигураций переднего плана с выделенным закрытым типом.](./media/configure-application-gateway-with-private-frontend-ip/private-frontendip-5.png)
...
Сведения о мониторинге работоспособности серверного пула см. в статье [резервные журналы работоспособности и диагностики для шлюза приложений](application-gateway-diagnostics.md).
