...
- Входящий TCP-трафик на портах 29876 и 29877 с __тега службы__ __BatchNodeManagement__. Трафик через эти порты шифруется и используется пакетной службой Azure для обмена данными между планировщиком и узлом.
...
- Для вычислительного экземпляра входящий TCP-трафик через порт 44224 из __тега службы__ __AzureMachineLearning__. Трафик через этот порт шифруется и используется Машинное обучение Azure для взаимодействия с приложениями, работающими на экземплярах вычислений.
...
Эта статья является третьей частью серии виртуальных сетей из пяти частей. Ознакомьтесь с остальными статьями, чтобы узнать, как защитить виртуальную сеть.
