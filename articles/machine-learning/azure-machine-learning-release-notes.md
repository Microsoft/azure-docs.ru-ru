...
ms.locfileid: "105562228"
---
# <a name="azure-machine-learning-release-notes"></a>Заметки о выпуске Машинное обучение Azure

В этой статье вы узнаете о выпусках Машинное обучение Azure.  Полное справочное содержимое пакета SDK можно найти на странице справочника по [**основному пакету SDK для Python**](/python/api/overview/azure/ml/intro) машинное обучение Azure.
...
### <a name="azure-machine-learning-sdk-for-python-v1240"></a>Машинное обучение Azure SDK для Python v 1.24.0
+ **Исправления ошибок и улучшения**
  + **azureml-automl-core**
    + Удалены обратные совместимые импорты из `azureml.automl.core.shared` . Ошибка "модуль не найден" в `azureml.automl.core.shared` пространстве имен может быть разрешен путем импорта из `azureml.automl.runtime.shared` .
  + **azureml-от участников сообщества-аутомл-DNN-концепция**
    + Предоставлена Йоло модель обнаружения объектов.
  ...
  + **azureml-contrib-fairness**
    + Включить схему JSON в колесо для `azureml-contrib-fairness`
  + **azureml-от участников сообщества-K8S**
    + Теперь необходимо указать resource_id для присоединения вместо группы ресурсов и имени кластера.
  + **azureml-от участников сообщества-Талха**
    + Если для параметра show_output задано значение true при развертывании моделей, конфигурация определения и конфигурация развертывания будут воспроизводиться перед отправкой запроса на сервер.
  + **azureml-core**
    + Добавлены функции для фильтрации табличных наборов данных по значениям столбцов и файловых наборов данных с помощью метаданных.
    + Раньше было возможно, что пользователи могли создавать конфигурации подготовки для ComputeTarget, которые не удовлетворяют требованиям к надежности пароля для `admin_user_password` поля (т. е. они должны содержать по крайней мере три из следующих символов: 1 строчная буква, 1 прописная буква, 1 цифра и один специальный символ из следующего набора: ``\`~!@#$%^&*()=+_[]{}|;:./'",<>?`` ). Если пользователь создал конфигурацию с нестрогим паролем и запустила задание с такой конфигурацией, задание завершится ошибкой во время выполнения. Теперь вызов метода вызывает `AmlCompute.provisioning_configuration` исключение `ComputeTargetException` с сопутствующим сообщением об ошибке, объясняющим требования к надежности пароля. 
    + Кроме того, в некоторых случаях было возможно указать конфигурацию с отрицательным числом узлов максимального числа. Это невозможно. Теперь `AmlCompute.provisioning_configuration` выдаст исключение, `ComputeTargetException` Если `max_nodes` аргумент является отрицательным целым числом.
    + Если для параметра show_output задано значение true при развертывании моделей, будет отображаться конфигурация определения и конфигурация развертывания.
    + Если параметру show_output присвоено значение true, то при ожидании завершения развертывания модели будет отображен ход выполнения операции развертывания.
    + Разрешить клиенту указанный каталог конфигурации проверки подлинности AzureML с помощью переменной среды: AZUREML_AUTH_CONFIG_DIR
    + Ранее было возможно создать конфигурацию подготовки с минимальным числом узлов, меньшим, чем максимальное число узлов. Задание будет запущено, но во время выполнения произойдет сбой. Теперь эта ошибка исправлена. Если вы попытаетесь создать конфигурацию подготовки с помощью `min_nodes < max_nodes` пакета SDK, будет создано исключение `ComputeTargetException` .
  + **azureml-interpret**
    + Исправление информационной панели, не отображающей статистические сведения о функциях для разреженных объяснений
    + Оптимизация использования памяти Експланатионклиент в пакете azureml-интерпретировать
  + **azureml-train-automl-client**
    +  Исправлена show_output = false, чтобы вернуть управление пользователю при выполнении с помощью Spark.
...
+ **Новые функции**
  + [Собственный терминал (GA)](./how-to-access-terminal.md). Теперь пользователи получат доступ к интегрированному терминалу, а также к операции Git через встроенный терминал.
  ...
  + [Сочетания клавиш](./how-to-run-jupyter-notebooks.md#useful-keyboard-shortcuts). Полный контроль четности с помощью сочетаний клавиш, доступных в Jupyter. 
  + Укажите параметры ячейки. Показывает пользователей, какие ячейки в записной книжке являются ячейками параметров, и могут выполнять параметризованные записные книжки с помощью [папермилл](https://github.com/nteract/papermill) на вычислительном экземпляре.
  + Терминал и диспетчер сеансов ядра. пользователи смогут управлять всеми ядрами и сеансами терминала, выполняемыми в их расчете.
...
+ **Исправления ошибок и улучшения**
  ...
  + Добавлено вращающееся колесо для отображения хода выполнения всех текущих [операций вычислительного экземпляра](./how-to-run-jupyter-notebooks.md#status-indicators).
...
+ **Исправления ошибок и улучшения**
  + **azureml-core**
    ...
    + [Экспериментальный компонент] Добавлена поддержка доступа к данным на основе удостоверений. Пользователи могут регистрировать данные хранилища данных или DataSet без предоставления учетных данных. В таком случае для проверки подлинности будет использоваться маркер AAD пользователя или управляемое удостоверение целевого объекта вычислений. Дополнительные сведения см. [здесь](./how-to-identity-based-data-access.md).
  + **azureml-pipeline-steps**
    + [Экспериментальный компонент] Добавление поддержки для [синапсеспаркстеп](/python/api/azureml-pipeline-steps/azureml.pipeline.steps.synapsesparkstep)
  + **azureml-synapse**
...
+ **Исправления ошибок и улучшения**
  ...
  + **azureml-core**
    ...
    + Добавьте `process_count` в [питорчконфигуратион](/python/api/azureml-core/azureml.core.runconfig.pytorchconfiguration) для поддержки многоузловых заданий PyTorch с несколькими узлами.
  + **azureml-pipeline-steps**
    + [Коммандстеп](/python/api/azureml-pipeline-steps/azureml.pipeline.steps.commandstep) теперь общедоступно и не экспериментально.
    + [Параллелрунконфиг](/python/api/azureml-pipeline-steps/azureml.pipeline.steps.parallelrunconfig): Добавьте allowed_failed_count аргументов и allowed_failed_percent, чтобы проверить пороговое значение ошибки на уровне мини-пакетов. Пороговое значение ошибки теперь имеет 3 варианта:
...
+ **Новые функции**
  ...
  + [Обозреватель переменных](./how-to-run-jupyter-notebooks.md#explore-variables-in-the-notebook). Изучите содержимое переменных и кадров данных во всплывающей панели. Пользователи могут легко проверить тип данных, размер и содержимое.
...
+ **Исправления ошибок и улучшения**
  ...
  + **azureml-core**
    ...
    + Добавьте image_name и image_label параметры в Model. Package (), чтобы включить переименование образа созданного пакета.
    + Новый метод `set_pip_requirements()` для установки всего раздела PIP [`CondaDependencies`](/python/api/azureml-core/azureml.core.conda_dependencies.condadependencies) одновременно.
  ...
  + **azureml-train-automl-runtime**
    ...
    + Исправлены ошибки дочерних запусков, когда данные содержат NaN и добавление признаков отключены.
  + **azureml-train-core**
    + Добавлена поддержка указания параметров PIP (например,--очень-index-URL) в файле требований PIP, переданном в [`Estimator`](/python/api/azureml-train-core/azureml.train.estimator.estimator) `pip_requirements_file` параметре.
...
+ **Исправления ошибок и улучшения**
  ...
  + **azureml-core**
    ...
    + Добавлен параметр [Environment.build_local ()](/python/api/azureml-core/azureml.core.environment.environment) для отправки образа в реестр контейнеров рабочих областей AzureML.
...
+ **Исправления ошибок и улучшения**
  ...
  + **[azureml-datadrift](/python/api/azureml-datadrift)**
    ...
    + Поддержка извлечения результатов смещения данных на основе набора данных.
  + **azureml-explain-model**
    + Добавление поддержки [скоринжексплаинер](/python/api/azureml-interpret/azureml.interpret.scoring.scoring_explainer.scoringexplainer) для создания непосредственно с помощью мимиквраппер
  + **[azureml-pipeline-core](/python/api/azureml-pipeline-core)**
    + Улучшена производительность при создании больших конвейеров.
...
[`PipelineEndpoint`](/python/api/azureml-pipeline-core/azureml.pipeline.core.pipeline_endpoint.pipelineendpoint)Было введено Добавление новой версии опубликованного конвейера с сохранением той же конечной точки.
...
+ **Новые функции**
  ...
  Список версий, поддерживаемых последней версией пакета SDK, см. в документации по средству [оценки DNN](/python/api/azureml-train-core/azureml.train.dnn).
...
+ **Новые функции**
  + Машинное обучение Azure теперь предоставляет поддержку первого класса для популярного каркаса DNN Framework. С помощью [`Chainer`](/python/api/azureml-train-core/azureml.train.dnn.chainer) классов пользователи могут легко обучать и развертывать модели цепочек.
...
+ **Исправления ошибок и улучшения**
  + Мы добавили поддержку в конвейеры Машинное обучение Azure, чтобы задать для свойства source_directory_data_store нужное хранилище данных (например, хранилище BLOB-объектов) на [рунконфигуратионс](/python/api/azureml-core/azureml.core.runconfig.runconfiguration) , предоставленное для [писонскриптстеп](/python/api/azureml-pipeline-steps/azureml.pipeline.steps.python_script_step.pythonscriptstep). По умолчанию в качестве резервного хранилища используется хранилище файлов Azure, которое может выполняться в случае проблем регулирования, когда выполняется большое количество шагов одновременно.
