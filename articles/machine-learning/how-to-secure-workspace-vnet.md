...
> [!Warning]
> Защита рабочей области с помощью частных конечных точек не гарантирует сквозную безопасность. Для защиты отдельных компонентов решения необходимо выполнить действия, описанные в оставшейся части этой статьи, и серии VNet.
...
1. На странице Учетная запись службы хранилища выберите __сеть__.

   ![Область "Сетевые подключения" на странице службы хранилища Azure в портал Azure](./media/how-to-enable-virtual-network/storage-firewalls-and-virtual-networks.png)
...
1. На вкладке __брандмауэры и виртуальные сети__ выполните следующие действия.
    ...
    1. Установите флажок __Разрешить доверенным службам Microsoft доступ к этой учетной записи хранения__. Это изменение не дает всем службам Azure доступ к вашей учетной записи хранения.
...
1. Частная конечная точка с целевым подресурсом **большого двоичного объекта** .
1. Частная конечная точка с целевым подресурсом **файла (общая** папка).
...
Чтобы настроить закрытую конечную точку для учетной записи хранения, которая **не** является хранилищем по умолчанию, выберите **целевой тип подресурсов** , соответствующий добавляемой учетной записи хранения.
...
По умолчанию Машинное обучение Azure выполняет проверку достоверности данных и учетных данных при попытке доступа к данным с помощью пакета SDK. Если данные находятся за виртуальной сетью, Машинное обучение Azure не сможет выполнить эти проверки. Чтобы обойти эту проверку, необходимо создать хранилище данных и наборы данных, пропускаемые проверкой.
...
1. На вкладке __брандмауэры и виртуальные сети__ выполните следующие действия.
    ...
    1. В разделе __Разрешить доверенным службам Microsoft обходить этот брандмауэр__ выберите __Да__.
...
* Рабочая область Машинного обучения Azure должна содержать [кластер вычислительных ресурсов Машинного обучения Azure](how-to-create-attach-compute-cluster.md).
    ...
    > [!IMPORTANT]
    > Кластер, используемый для создания образов DOCKER, должен иметь доступ к репозиториям пакетов, которые используются для обучения и развертывания моделей. Может потребоваться добавить правила безопасности сети, которые разрешают доступ к общедоступной репозиториев, [использовать частные пакеты Python](how-to-use-private-python-packages.md)или [пользовательские образы DOCKER](how-to-train-with-custom-image.md) , которые уже содержат пакеты.

После выполнения этих требований выполните следующие действия, чтобы включить реестр контейнеров Azure.

> [!TIP]
> Если вы не использовали существующий реестр контейнеров Azure при создании рабочей области, возможно, он не существует. По умолчанию Рабочая область не создает экземпляр записи контроля доступа, пока он не понадобится. Чтобы принудительно создать одну, обучить или развернуть модель с помощью рабочей области, прежде чем выполнять действия, описанные в этом разделе.
...
1. Настройте запись контроля доступа для рабочей области, чтобы [Разрешить доступ доверенным службам](../container-registry/allow-access-trusted-services.md).
