...
ForecastTCN (предварительная версия)| Форекастткн — это модель нейронной сети, предназначенная для решения самых ресурсоемких задач прогнозирования. Он фиксирует нелинейные локальные и глобальные тенденции в данных и связи между временными рядами.|Возможность использования сложных тенденций в данных и простота масштабирования до самых больших наборов данных.
...
|`freq`| Частота набора данных временных рядов. Этот параметр представляет период времени, в течение которого события должны выполняться, например ежедневно, еженедельно, ежегодно и т. д. Частота должна быть [псевдонимом смещения Pandas](https://pandas.pydata.org/pandas-docs/stable/user_guide/timeseries.html#dateoffset-objects). Дополнительные сведения о [Frequency]. (#frequency--Target-Data-агрегирование)||
...
|`target_aggregation_function`| Функция, используемая для статистической обработки целевого столбца временных рядов в соответствии с частотой, заданной с помощью `freq` параметра. `freq`Параметр должен быть задан для использования `target_aggregation_function` . По умолчанию `None` имеет значение; для большинства сценариев, использующих `sum` , достаточно.<br> Дополнительные сведения о [статистической обработке целевых столбцов](#frequency--target-data-aggregation). 
...
### <a name="frequency--target-data-aggregation"></a>Частота & целевого агрегирования данных

Используйте частоту, `freq` , параметр, чтобы избежать сбоев, вызванных неправильными данными. это данные, которые не соответствуют заданному ритмичности, например ежечасные или ежедневные данные. 

Для очень нестандартных данных или для различных бизнес-нужд пользователи могут задать нужную частоту прогноза, `freq` а также указать `target_aggregation_function` для статистической обработки целевого столбца временных рядов. Используйте эти два параметра в `AutoMLConfig` объекте, чтобы сократить время на подготовку данных. 

При `target_aggregation_function` использовании параметра
* Значения целевого столбца суммируются на основе указанной операции. Как правило, `sum` подходит для большинства сценариев.

* Числовые столбцы прогноза в данных суммируются по сумме, среднему значению, минимуму и максимальному значению. В результате автоматический ML создает новые столбцы с суффиксом Name статистической функции и применяет выбранную статистическую операцию. 

* Для столбцов прогнозируемого прогноза данные вычисляются по режиму — самой заметной категории в окне.

* Столбцы прогноза дат суммируются по минимальному значению, максимальному значению и режиму. 

Поддерживаемые операции агрегирования для значений целевых столбцов включают:

|Компонент | description;
|---|---
|`sum`| Сумма целевых значений
|`mean`| Среднее или среднее от целевых значений
|`min`| Минимальное значение целевого объекта  
...
Кроме того, можно применить глубокое обучение с глубокими нейронными сетями, DNN, чтобы улучшить оценки модели. Глубокое обучение при автоматизированном машинном обучении позволяет прогнозировать данные одномерных и многомерных временных рядов.
...
Подробный пример кода с помощью DNN см. в [записной книжке произ](https://github.com/Azure/MachineLearningNotebooks/blob/master/how-to-use-azureml/automated-machine-learning/forecasting-beer-remote/auto-ml-forecasting-beer-remote.ipynb) .

### <a name="target-rolling-window-aggregation"></a>Целевая Статистическая обработка окна
Часто наилучшей информацией для прогнозиста оказывается последнее значение целевого объекта.  Целевые агрегаты для скользящих окон позволяют добавить последовательное агрегирование значений данных в виде функций. Создание и использование этих функций в качестве дополнительных контекстных данных помогает с точностью модели обучения.
...
Просмотрите пример кода Python применение [статистической функции целевого последовательного окна](https://github.com/Azure/MachineLearningNotebooks/blob/master/how-to-use-azureml/automated-machine-learning/forecasting-energy-demand/auto-ml-forecasting-energy-demand.ipynb).
...
Повторите необходимые шаги, чтобы загрузить эти будущие данные в объект DataFrame, а затем запустите `best_run.forecast(test_data)` для прогнозирования будущих значений.
