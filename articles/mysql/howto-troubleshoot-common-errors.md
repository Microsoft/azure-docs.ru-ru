...
**Решение**. Следует сбросить значение init_connect на вкладке "Параметры сервера" на портале Azure и задать только поддерживаемые параметры сервера с помощью параметра init_connect. 
