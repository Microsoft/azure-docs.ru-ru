...
4. На вкладке **Основные сведения** введите следующие данные: 
    ...
    Тип рабочей нагрузки| Разработка | Для рабочей нагрузки производства можно выбрать малый, средний или большой размер в зависимости от требований [max_connections](concepts-server-parameters.md#max_connections)|
    Зона доступности| Нет предпочтения | Если ваше приложение на виртуальной машине Azure, масштабируемом наборе виртуальных машин или экземпляре AKS расположено в определенной зоне доступности, можно указать гибкий сервер в той же зоне доступности: таким образом приложение и база данных будут находиться в одной зоне. Это позволит снизить задержку между зонами и повысить производительность.|
    Высокий уровень доступности| Значение по умолчанию | Для рабочих серверов настоятельно рекомендуется включить высокий уровень доступности зоны (HA) для обеспечения непрерывности бизнес-процессов и защиты от сбоев зон.|
    Версия MySQL|**5.7**| Основной номер версии MySQL.|
    ...
    Вычисления и хранилище | **С увеличивающейся производительностью**, **Standard_B1ms**, **10 ГиБ**, **100 операций ввода-вывода в секунду**, **7 дней** | Конфигурации вычислительных ресурсов, хранилища, операций ввода-вывода в секунду и резервного копирования для нового сервера. Щелкните **Настройка сервера**. **С увеличивающейся производительностью**, **Standard_B1ms**, **10 ГиБ**, **100 операций ввода-вывода в секунду** и **7 дней** — это значения по умолчанию для параметров **Уровень вычислительных ресурсов**, **Объем вычислительных ресурсов**, **Размер хранилища**, **Операции ввода-вывода в секунду** и **Период хранения для резервных копий**. Вы можете оставить эти значения как есть или изменить их. Для ускорения загрузки данных во время миграции рекомендуется увеличить число операций ввода-вывода до максимального размера, поддерживаемого для объема вычислительных ресурсов, а затем снова уменьшить его для снижения затрат. Чтобы сохранить настроенные параметры вычислительной среды и хранилища, щелкните **Сохранить** и продолжайте настройку. На приведенном ниже снимке экрана представлены доступные варианты для вычислительной среды и хранилища.|
