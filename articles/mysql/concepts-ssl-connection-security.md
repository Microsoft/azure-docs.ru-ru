...
Для параметра минимальной версии TLS не требуется перезагружать сервер, пока сервер находится в режиме «в сети». Сведения о настройке параметра TLS для базы данных Azure для MySQL см. в разделе [Настройка параметра TLS](howto-tls-configurations.md).
